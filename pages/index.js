...
Vaquilla 1°Invierno (recría post-destete): 14–16 Mcal EM/día, PB 900–1100 g/día — suplementación proteica SIN DISCUSIÓN en invierno (pasto <10% floración no cubre PB en estas categorías por bajo volumen de consumo)
Vaquilla 2°Invierno (recría pre-entore): 12–14 Mcal EM/día, PB 800–1000 g/día — flushing obligatorio 25–30d pre-entore

═══════════════════════════════════════
SUPLEMENTACIÓN ESTRATÉGICA (Peruchena, INTA 2003 · Balbuena, INTA 2003)
═══════════════════════════════════════
Escenario A — mucho pasto, baja calidad (>50% floración): proteico 0.7% PV, 3×/semana. Sin energético. Respuesta: +0.5–1.0 kg GDP, +15–20% preñez vaquillas.
Escenario B — poco pasto, baja calidad: E+P 1% PV diario. Combinación necesaria.
...
Vaquilla 1°Invierno (post-destete, primera etapa crítica):
  ...
  Período crítico: mayo–agosto (invierno). Suplementación proteica sin discusión.
...
1️⃣ DIAGNÓSTICO AMBIENTAL Y FORRAJERO
   Zona · provincia · mes · NDVI · temperatura REAL de campo · precipitaciones · balance hídrico
   Fenología → D-value implícito → consumo voluntario estimado por categoría
   Si zona templada: aclarar que conceptos de megatérmicas no aplican directamente

2️⃣ DIAGNÓSTICO POR CATEGORÍA
   Para CADA categoría presente: requerimiento Mcal + PB vs oferta estimada = déficit cuantificado
   Vacas: estado gestacional × fenología × consumo estimado
   Vaca 2°serv: triple demanda, evaluar días estimados al destete
   Vaquilla 1°Invierno: GDP observado vs objetivo 600 g/d, déficit PB
   Vaquilla 2°Invierno: peso vs objetivo entore, GDP, flushing

3️⃣ ESTADO DEL DESTETE Y PROYECCIÓN CC
   Vaca 2°serv: proyectar pérdida CC SOLO hasta destete estimado → luego recuperación post-destete hasta parto
   Rodeo general: misma lógica si tienen ternero al pie
   CC parto → CC servicio → % preñez → N° terneros en riesgo vs objetivo 75%

4️⃣ BALANCE OFERTA vs DEMANDA
   Cuantificar en Mcal y PB total del sistema

5️⃣ EVALUACIÓN ESTRATÉGICA
   PASO 1: destete (siempre primero)
   PASO 2: suplementación post-destete — escenario A o B, dosis exacta kg/animal/entrega, frecuencia
   PASO 3: ajuste de carga si necesario
   PASO 4: acción por calendario con fechas concretas

6️⃣ IMPACTO A 1–2 AÑOS
   Proyectar % preñez con y sin intervención · terneros adicionales · kg carne/ha

7️⃣ ESTADO DE EMERGENCIA
   Evaluar siempre. Si hay riesgo: prioridades Rosello Brajovich, dosis emergencia.

8️⃣ RECOMENDACIONES FINALES POR CATEGORÍA (SIEMPRE, en todas las consultas)
   Para cada categoría presente, una recomendación concreta orientada a:
   — Aumentar % preñez: acción específica + número concreto esperado
   — Lograr primer entore a 24 meses: estado actual vs meta, brecha
   — Reducir mermas: punto de mayor pérdida identificado
   — Aumentar producción por ha: palanca principal para este sistema
   — Alerta emergencia si aplica
   Formato: tabla simple o lista por categoría. Conciso, operativo, con números.

Citar siempre: (NASSEM, 2010) · (Balbuena, INTA 2003) · (Peruchena, INTA 2003) · (Detmann et al., 2014) · (Rosello Brajovich et al., INTA 2025)`;

const CC_PRENEZ = [
  {ccP:6.5,ccS:5.0,pr:95},{ccP:6.0,ccS:4.5,pr:88},{ccP:5.5,ccS:4.0,pr:75},
  {ccP:5.0,ccS:3.5,pr:55},{ccP:4.5,ccS:3.0,pr:35},{ccP:4.0,ccS:2.5,pr:15},{ccP:3.5,ccS:2.0,pr:5}
];
...
function interpCurva(ccP) {
  ...
  for(let i=0;i<t.length-1;i++){
    if(ccP<=t[i].ccP&&ccP>=t[i+1].ccP){
      ...
      return{ccS:+(t[i+1].ccS+r*(t[i].ccS-t[i+1].ccS)).toFixed(1),pr:Math.round(t[i+1].pr+r*(t[i].pr-t[i+1].pr))};
    }
  }
  return{ccS:2.5,pr:15};
}

function calcCCAlParto(cc,dias,estadoDestete,pastoCal,diasDestete){
  if(!cc||!dias)return null;
  if(estadoDestete==="no_ternero"){
    // Si hay días al destete estimados, calcular en dos tramos
    if(diasDestete>0&&diasDestete<dias){
      const perdidaHastaDestete=0.010*diasDestete;
      const ccPostDestete=Math.max(1.0,cc-perdidaHastaDestete);
      const tasa={excelente:0.022,bueno:0.016,regular:0.009,malo:0.004}[pastoCal]||0.013;
      const diasRecup=dias-diasDestete;
      return Math.min(9.0,ccPostDestete+tasa*diasRecup);
    }
...
function consumoPorFenologia(fen,pvKg){
  const pct={menor_10:0.235,["10_25"]:0.210,["25_50"]:0.190,mayor_50:0.165}[fen]||0.200;
  return pvKg>0?((pvKg*pct/100)).toFixed(1):null;
}

function detectZona(lat,lon){
  if(lat>-24)return"NEA";
  if(lat>-28&&lon>-58)return"NEA";
  if(lat>-30&&lon<-64)return"NOA";
  if(lat>-38&&lat<=-30&&lon>-62)return"Pampa Húmeda";
  if(lat>-38&&lat<=-30&&lon<=-62&&lon>-67)return"Semiárido Pampeano";
...
function detectProvincia(lat,lon){
  ...
  if(lat>-28&&lat<=-24&&lon>-57)return"Corrientes";
  if(lat>-30&&lat<=-28&&lon>-57)return"Misiones";
...
const FENOLOGIAS=[
  {val:"menor_10",label:"<10% Floración",desc:"PB >10% · Dig >65% · CV alto · Consumo ~2.3% PV",warn:""},
  {val:"10_25",label:"10–25% Floración",desc:"PB 7–10% · Dig 60–65% · Consumo ~2.1% PV",warn:""},
  {val:"25_50",label:"25–50% Floración",desc:"PB 5–7% · Inicio lignificación · Consumo ~1.9% PV",warn:"⚠ Déficit proteico — 2°serv y vaquillas"},
  {val:"mayor_50",label:">50% Floración",desc:"PB <5% · Lignificación avanzada · Consumo ~1.6% PV",warn:"🔴 Déficit severo — B-value alto, restricción física"},
];
...
export default function AgroMind(){
  const[form,setForm]=useState({
    nombreProductor:"",estadoDestete:"",pastoCal:"",pVacas:"",ccActual:"",diasParto:"",diasDesteteEst:"",eReprod:"",
    v2sN:"",v2sPV:"",v2sCC:"",v2sTernero:"",v2sDiasDestete:"",
    zona:"",provincia:"",mes:"",clima:"",vegetacion:"",supHa:"",fenologia:"",
    vacasN:"",ternerosN:"",torosN:"",prenezHist:"",pctDestete:"",
    vaq1N:"",vaq1PV:"",vaq1GDP:"",
    vaq2N:"",vaq2PV:"",diasEntore:"",tendPeso:"",pvVacaAdulta:"",
    suplem:"",consulta:"",
  });
  ...
  useEffect(()=>{
    try{const p=JSON.parse(localStorage.getItem("agm_prod")||"[]");setProductores(p);}catch(e){}
  },[]);
  ...
  useEffect(()=>{
    ...
    if(!isNaN(cc)&&cc>0&&dias>0){
      const p=calcCCAlParto(cc,dias,form.estadoDestete,form.pastoCal,diasD);
      setCcParto(p?parseFloat(p.toFixed(2)):null);
      setCurva(p?interpCurva(p):null);
  ...
  useEffect(()=>{
    if(form.v2sTernero==="si"){
      ...
      if(form.v2sDiasDestete){
        const dd=parseInt(form.v2sDiasDestete);
        const ccV=parseFloat(form.v2sCC);
        if(!isNaN(dd)&&!isNaN(ccV)&&dd>0){
          const perdida=Math.min(ccV-1.0,0.010*dd).toFixed(1);
          msg+=`\nCon destete en ${dd} días: pérdida estimada hasta destete = ${perdida} pts CC. Luego recuperación post-destete.`;
        }
      }
      setAlerta2serv(msg);
    }else setAlerta2serv("");
  ...
  const applyLocation=async(lat,lon,src)=>{
    const zona=detectZona(lat,lon);
    const prov=detectProvincia(lat,lon);
    const mes=MESES[new Date().getMonth()];
    setCoords({lat,lon,src,zona,prov});
    setForm(f=>({...f,zona,provincia:prov,mes}));
    await fetchSat(lat,lon,zona,mes);
  };
  ...
  const getGPS=()=>{
    ...
    navigator.geolocation.getCurrentPosition(
      p=>{setGpsLoading(false);applyLocation(p.coords.latitude,p.coords.longitude,"GPS");},
      e=>{setGpsLoading(false);alert("Error GPS: "+e.message);},
  ...
  const setManual=()=>{
    ...
    if(isNaN(lat)||isNaN(lon)||lat>-20||lat<-56||lon>-52||lon<-74){
      alert("Coordenadas inválidas.\nNEA ejemplo: Lat -27.45 / Lon -59.12");return;
    }
  ...
  const fetchSat=async(lat,lon,zona,mes)=>{
    ...
    try{
      ...
      // Temperatura real — promedio de max+min últimos 7 días
      const tMax=d.temperature_2m_max.slice(-7);
      const tMin=d.temperature_2m_min.slice(-7);
      const tempArr=tMax.map((t,i)=>(t+(tMin[i]||t))/2);
      const temp=tempArr.reduce((a,b)=>a+b,0)/tempArr.length;
      const tMaxR=Math.max(...tMax).toFixed(1);
      const tMinR=Math.min(...tMin).toFixed(1);
      ...
      // NDVI estimado más conservador — solo para referencia
  ...
  const saveProductor=(informe)=>{
    ...
    const r={
      ...
      diasParto:form.diasParto,diasDesteteEst:form.diasDesteteEst,pVacas:form.pVacas,eReprod:form.eReprod,
      vacasN:form.vacasN,ternerosN:form.ternerosN,torosN:form.torosN,prenezHist:form.prenezHist,pctDestete:form.pctDestete,
      v2sN:form.v2sN,v2sPV:form.v2sPV,v2sCC:form.v2sCC,v2sTernero:form.v2sTernero,v2sDiasDestete:form.v2sDiasDestete,
      pvVacaAdulta:form.pvVacaAdulta,vaq1N:form.vaq1N,vaq1PV:form.vaq1PV,vaq1GDP:form.vaq1GDP,
      vaq2N:form.vaq2N,vaq2PV:form.vaq2PV,diasEntore:form.diasEntore,
      suplem:form.suplem,ccParto:ccParto||"",prenezEst:curva?.pr||"",
      informe:(informe||"").slice(0,400),fechaVisita:new Date().toLocaleDateString("es-AR"),
    };
    const upd=[r,...productores.filter(p=>p.nombre!==form.nombreProductor)];
    setProductores(upd);
    try{localStorage.setItem("agm_prod",JSON.stringify(upd));}catch(e){}
  };

  const exportCSV=()=>{
    if(!productores.length){alert("Sin datos aún.");return;}
    const cols=["nombre","zona","provincia","mes","clima","ndvi","supHa","vegetacion","fenologia","estadoDestete","ccActual","diasParto","diasDesteteEst","pVacas","eReprod","vacasN","ternerosN","torosN","prenezHist","pctDestete","v2sN","v2sPV","v2sCC","v2sTernero","v2sDiasDestete","pvVacaAdulta","vaq1N","vaq1PV","vaq1GDP","vaq2N","vaq2PV","diasEntore","suplem","ccParto","prenezEst","informe","fechaVisita"];
  ...
  const buildPrompt=()=>{
    ...
    if(sat)t+=`DATOS METEOROLÓGICOS REALES (Open-Meteo): Temp promedio 7d: ${sat.temp}°C · Tmax: ${sat.tMax}°C · Tmin: ${sat.tMin}°C · Precip 7d: ${sat.p7}mm · 30d: ${sat.p30}mm · Balance hídrico: ${sat.deficit>0?'+':''}${sat.deficit}mm · ET₀: ${sat.et}mm/d · NDVI estimado: ${sat.ndvi} (${sat.cond}) · Clima: ${sat.clima}\n`;
    ...
    if(form.fenologia&&form.pVacas){
      const cons=consumoPorFenologia(form.fenologia,parseFloat(form.pVacas));
      if(cons)t+=`Consumo voluntario estimado vaca (${form.fenologia}): ~${cons} kg MS/día\n`;
    }
    ...
    t+=`VAQ 1°INVIERNO (post-destete, recría): N° ${form.vaq1N||'—'} · PV ${form.vaq1PV||'—'} kg · GDP obs ${form.vaq1GDP||'—'} g/d (objetivo 600 g/d)\n`;
    t+=`VAQ 2°INVIERNO (pre-entore): N° ${form.vaq2N||'—'} · PV ${form.vaq2PV||'—'} kg · Días al entore ${form.diasEntore||'—'} · Tendencia ${form.tendPeso||'—'}\n`;
    ...
    if(form.consulta)t+=`\nCONSULTA ESPECÍFICA: ${form.consulta}\n`;
    t+="\n→ Estructura 8 secciones obligatorias. Usar datos meteorológicos REALES para temperatura. Proyectar CC vaca 2°serv SOLO hasta destete estimado. Consumo voluntario según fenología. Recomendaciones finales por categoría siempre en sección 8.";
    return t;
  };
  ...
  const renderResult=(t)=>t
    ...
    .replace(/(1️⃣|2️⃣|3️⃣|4️⃣|5️⃣|6️⃣|7️⃣|8️⃣)\s*([^\n]+)/g,'<div style="font-family:monospace;font-size:.88rem;color:#7ec850;margin:18px 0 7px;border-left:3px solid #7ec850;padding-left:10px;letter-spacing:1px">$1 $2</div>')
    .replace(/\n\n/g,"<br/><br/>").replace(/\n/g,"<br/>");
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"0 16px 80px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))",gap:7,marginBottom:16}}>
          {[
            ...
            ["Temp °C",sat?(sat.tMin+"–"+sat.tMax+"°C"):"—","#d4952a"],
            ["Precip 30d",sat?(sat.p30+" mm"):"—","#3a8fb5"],
        ...
        {tab==="analisis"&&(
          <div>
            ...
            <div style={P}>
              ...
              {coords&&(
                <div>
                  <iframe src={`https://www.openstreetmap.org/export/embed.html?bbox=${coords.lon-0.15}%2C${coords.lat-0.15}%2C${coords.lon+0.15}%2C${coords.lat+0.15}&layer=mapnik&marker=${coords.lat}%2C${coords.lon}`}
                    style={{width:"100%",height:220,border:"1px solid rgba(126,200,80,.2)",borderRadius:6,marginBottom:10}} loading="lazy"/>
            ...
            <div style={{...P,border:"1px solid rgba(212,149,42,.28)",background:"rgba(212,149,42,.03)"}}>
              ...
              <div style={{fontFamily:"monospace",fontSize:".52rem",color:"rgba(212,149,42,.55)",lineHeight:1.7,marginBottom:14}}>
                Destete Feb → −6–8 Mcal/día + rebrote otoñal → recuperación CC al parto. El expeller no reemplaza el destete.
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10}}>
                ...
                {form.estadoDestete==="no_ternero"&&(
                  <div><label style={L}>Días estimados al destete</label>
                    <input type="number" value={form.diasDesteteEst} onChange={e=>set("diasDesteteEst",e.target.value)} placeholder="Ej: 30" style={I}/></div>
                )}
              </div>
              {ccParto&&curva&&(
                <div style={{marginTop:14,background:"rgba(0,0,0,.25)",border:"1px solid rgba(126,200,80,.2)",borderRadius:6,padding:"12px 14px",fontFamily:"monospace",fontSize:".65rem",lineHeight:2}}>
                  ...
                  {form.diasDesteteEst&&<><br/><span style={{color:"#d4952a"}}>Cálculo en 2 tramos: pérdida hasta destete ({form.diasDesteteEst}d) → recuperación post-destete hasta parto</span></>}
                </div>
              )}
              {form.estadoDestete==="no_ternero"&&(
            ...
            <div style={{...P,border:"1px solid rgba(136,102,204,.28)",background:"rgba(136,102,204,.03)"}}>
              ...
              <div style={{fontFamily:"monospace",fontSize:".52rem",color:"rgba(136,102,204,.55)",lineHeight:1.7,marginBottom:14}}>
                Triple demanda: Lactando + Creciendo + Quedar preñada. ~20–22 Mcal/día requeridos.<br/>
                <strong style={{color:"#8866cc"}}>Destete precoz = manejo estándar. CC proyecta solo hasta destete, luego recuperación.</strong>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:10}}>
                ...
                {form.v2sTernero==="si"&&(
                  <div><label style={L}>Días estimados al destete</label>
                    <input type="number" value={form.v2sDiasDestete} onChange={e=>set("v2sDiasDestete",e.target.value)} placeholder="Ej: 20" style={I}/></div>
                )}
              </div>
              {alerta2serv&&(
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(165px,1fr))",gap:10,marginBottom:14}}>
                ...
                <div><label style={L}>Mes</label>
                  <select value={form.mes} onChange={e=>set("mes",e.target.value)} style={I}>
                    {["","..."].concat(MESES).map(o=><option key={o} value={o}>{o||"Seleccionar..."}</option>)}
                  </select></div>
                ...
                <div><label style={L}>PV Vaca Adulta Rodeo (kg)</label>
                  <input type="number" value={form.pvVacaAdulta} onChange={e=>set("pvVacaAdulta",e.target.value)} placeholder="Ej: 420" style={I}/></div>
              </div>
              <div style={{fontFamily:"monospace",fontSize:".5rem",color:"#7a9668",textTransform:"uppercase",letterSpacing:1.5,marginBottom:8}}>Estado Fenológico — Determina consumo voluntario estimado</div>
            ...
            <div style={P}>
              ...
              <div style={{fontFamily:"monospace",fontSize:".75rem",color:"#d4952a",marginTop:14,marginBottom:8}}>Vaquilla 1° Invierno — Recría post-destete (primera etapa crítica)</div>
              <div style={{fontFamily:"monospace",fontSize:".52rem",color:"rgba(212,149,42,.5)",marginBottom:10}}>Suplementación proteica sin discusión en invierno · Objetivo: 600 g/d GDP · Primer entore a 24 meses</div>
              ...
              <div style={{fontFamily:"monospace",fontSize:".75rem",color:"#d4952a",marginBottom:8}}>Vaquilla 2° Invierno — Recría pre-entore (flushing)</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(155px,1fr))",gap:10}}>
                ...
                <div><label style={L}>Tendencia de Peso</label>
              ...
              {form.pvVacaAdulta&&(
                <div style={{marginTop:10,fontFamily:"monospace",fontSize:".62rem",color:"#d4952a",background:"rgba(212,149,42,.05)",border:"1px solid rgba(212,149,42,.2)",borderRadius:4,padding:"8px 12px"}}>
                  ...
                  {form.vaq2PV&&parseFloat(form.vaq2PV)<parseFloat(form.pvVacaAdulta)*0.75&&(
                    <span style={{color:"#c04820"}}> · ⚠️ Vaq 2°inv ({form.vaq2PV} kg) NO alcanza el mínimo</span>
                  )}
                </div>
              )}
            </div>
          ...
          <div>
            <div style={P}>
              ...
              <div style={{fontFamily:"monospace",fontSize:".55rem",color:"#7a9668",marginBottom:14}}>
                {productores.length} productor{productores.length!==1?"es":""} · 1 fila por productor · Se actualiza automáticamente al analizar
              </div>
              {productores.length===0?(
                <div style={{textAlign:"center",padding:"40px 20px",fontFamily:"monospace",fontSize:".7rem",color:"#3e5230"}}>
                  Sin registros aún. Completá el formulario con nombre del productor y analizá.
                </div>
              ):(
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",fontFamily:"monospace",fontSize:".6rem"}}>
                    <thead>
                      <tr style={{borderBottom:"1px solid rgba(126,200,80,.2)"}}>
                        {["Productor","Zona","Prov","Mes","Clima","NDVI","ha","Fenología","Destete","CC","D.Parto","D.Dest","Vacas","Preñez%","Dest%","V2s N°","Vaq1 N°","Vaq2 N°","CC Parto","Preñez Est","Visita"].map(h=>(
                    ...
                    <tbody>
                      {productores.map((p,i)=>(
                        <tr key={i} style={{borderBottom:"1px solid rgba(126,200,80,.07)",cursor:"pointer"}} onClick={()=>{setForm(f=>({...f,...p}));setTab("analisis");}}>
                          {[p.nombre,p.zona,p.provincia,p.mes,p.clima,p.ndvi,p.supHa,p.fenologia,p.estadoDestete,p.ccActual,p.diasParto,p.diasDesteteEst,p.vacasN,p.prenezHist,p.pctDestete,p.v2sN,p.vaq1N,p.vaq2N,p.ccParto,p.prenezEst?p.prenezEst+"%":"",p.fechaVisita].map((v,j)=>(
        ...
        {tab==="marco"&&(
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:12}}>
            {[
              ["📅 Destete — La Palanca Central","#d4952a","rgba(212,149,42,.3)","Destete en febrero = caída −6–8 Mcal/día + rebrote otoñal. La velocidad de recuperación CC depende del pastizal y el tamaño del animal. El expeller potencia después del destete, no lo reemplaza.","Balbuena INTA (2003)"],
              ["⚠️ Vaca 2° Servicio","#8866cc","rgba(136,102,204,.35)","Triple demanda: lactando + creciendo + quedar preñada. ~20–22 Mcal/día. Déficit estructural 12–17 Mcal. Destete precoz = manejo ESTÁNDAR independiente de la CC. Proyectar pérdida CC SOLO hasta destete, luego recuperación.",""],
              ["📊 Curva CC → Preñez","#7ec850","rgba(126,200,80,.2)","CC 5.5 parto → 4.0 serv → 75% · CC 5.0 → 3.5 → 55% · CC 4.5 → 3.0 → 35% · CC 4.0 → 2.5 → 15%. Pérdida post-parto NEA: 1.5 puntos. La palanca está en el parto.","Datos campo NEA"],
              ["🌾 Consumo Voluntario × Fenología","#7ec850","rgba(126,200,80,.15)","<10% flor: ~2.3% PV · 10–25%: ~2.1% · 25–50%: ~1.9% · >50%: ~1.6% PV. A mayor lignificación, el B-value sube y el animal no puede compensar con más cantidad. La restricción física domina.","Detmann et al. (2014) J.Anim.Sci."],
              ["🐄 Vaquilla 1° Invierno","#d4952a","rgba(212,149,42,.25)","Recría post-destete, primera etapa crítica. GDP objetivo: 600 g/d. Meta: ≥280 kg al inicio del 2°invierno. Suplementación proteica sin discusión en invierno. Objetivo: primer entore a 24 meses.","NASSEM (2010)"],
              ["📚 Fuentes Técnicas","#3a8fb5","rgba(58,143,181,.25)","NASSEM (2010) · Balbuena INTA CB (2003) · Peruchena INTA Ctes (2003) · Fernández Mayer INTA (2008) · Detmann et al. J.Anim.Sci. (2014) · Rosello Brajovich et al. INTA (2025) · NRC Beef (2000)",""],
