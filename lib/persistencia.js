...
import { sanitizeForm } from "./schema";

const STORAGE_KEY_FORM    = "calfai_form_draft";
const STORAGE_KEY_HIST    = "calfai_historial";
const MAX_HISTORIAL       = 20;
// Incrementar cuando FORM_DEF agrega campos que cambian la semántica de datos viejos
const FORM_VERSION        = "17";

function usePersistencia(form, setForm) {
  // Guardar borrador automáticamente
  const guardarBorrador = React.useCallback(() => {
    try {
      const draft = { ...form, _v: FORM_VERSION, _guardadoEn: new Date().toISOString() };
  ...
  const restaurarBorrador = React.useCallback(() => {
    try {
      ...
      // Merge con FORM_DEF primero para rellenar campos nuevos de versiones anteriores,
      // luego sanitizar con zod para garantizar arrays y tipos correctos.
      const draft = sanitizeForm({ ...FORM_DEF, ...parsed });
      if (!draft.vacasN) return false;
      setForm(draft);
      return draft;
  ...
  const guardarEnHistorial = React.useCallback((formData, motor, resultado, potreros = [], sat = null) => {
    try {
      ...
      const entrada = {
        ...
        mesesDeficit: [4,5,6].filter(i => (motor?.balanceMensual?.[i]?.balance ?? 0) < 0).length,
        ...
        // Snapshot climático del momento de la visita (no se puede reconstruir después:
        // el clima de hoy cambia, así que hay que guardarlo aquí o se pierde para siempre)
        sat: sat ? {
          temp: sat.temp, tMax: sat.tMax, tMin: sat.tMin,
          p7: sat.p7, p30: sat.p30, p90: sat.p90, deficit: sat.deficit,
          ndvi: sat.ndvi, ndviHist: sat.ndviHist, ndviDelta: sat.ndviDelta, ndviCateg: sat.ndviCateg,
  ...
  const cargarVisita = React.useCallback((entrada) => {
    try {
      if (entrada?.form) {
        // Mismo patrón: merge con FORM_DEF + sanitización
        setForm(sanitizeForm({ ...FORM_DEF, ...entrada.form }));
        return true;
      }
      return false;
    } catch(e) { return false; }
...
export { usePersistencia, PanelHistorial, calcConfianzaDiagnostico,
         STORAGE_KEY_FORM, STORAGE_KEY_HIST, FORM_VERSION };
