...
function interpretarCCEnTiempo(tray, mesesDeficit, peorDeficitMcal) {
  if (!tray) return null;
  ...
  // Erosión anual por déficit invernal — magnitud calibrada por severidad del peor mes
  const erosionAnual = mesesDeficit >= 2
    ? (Math.abs(peorDeficitMcal) > 100 ? 0.40 : 0.20)
...
function interpretarMomentoOptimo(form, motor, tray, sat) {
  ...
  const ccServ       = tray?.ccServ || 0;
  const mesesDeficit = motor.mesesDeficit ?? 0;
...
function interpretarAmplitudParicion(form, motor, tray) {
  ...
  // Ventana de recuperación = desde el destete hasta el PRÓXIMO servicio.
  // El parto ocurre ~280 días DESPUÉS del inicio del servicio, así que la
  // recuperación se mide contra el servicio del año siguiente (ini + 365d),
  // no contra el servicio actual (eso daba siempre recuperación 0 → brecha 0).
  const proxServ = cadena.ini ? new Date(cadena.ini.getTime() + 365*86400000) : null;
  const diasRecupCabeza = partoTemp && proxServ
    ? Math.max(0, Math.round((proxServ - new Date(partoTemp.getTime() + diasLactModal*86400000)) / 86400000))
    : null;
  const diasRecupCola = partoTard && proxServ
    ? Math.max(0, Math.round((proxServ - new Date(partoTard.getTime() + diasLactModal*86400000)) / 86400000))
    : null;

  // ── RECONCILIACIÓN CON EL MODELO PRINCIPAL ────────────────────
  // En vez de calcular la CC de cabeza/cola desde cero (con una tasa fija que no
  // respondía al pasto), se ANCLAN al ccServ ponderado del rodeo (calcTrayectoriaCC)
  // y se separan por la amplitud de la parición. Así los tres números son coherentes:
  // cabeza ≥ promedio del rodeo ≥ cola, siempre.
  const ccServRodeo = tray?.ccServ || 4.5;
  const techoCC     = tray?.ccParto || 6.0;
  // Misma tasa de recuperación ajustada por pasto/NDVI/agua que usa el modelo principal.
  const factorForr    = tray?._factorForrajero ?? 1.0;
  const tasaRecupEff  = 0.010 * factorForr; // CC/día efectivo
  // El gap de recuperación cabeza-cola ≈ duración del servicio (la cola pare diasServ
  // más tarde → tiene diasServ menos días para recuperar).
  const gapDias  = (diasRecupCabeza !== null && diasRecupCola !== null)
    ? Math.max(0, diasRecupCabeza - diasRecupCola)
    : diasServ;
  const spreadCC = Math.min(2.0, +(gapDias * tasaRecupEff).toFixed(2)); // CC entre extremos
  const ccServCabeza = diasServ > 0
    ? Math.min(techoCC, +(ccServRodeo + spreadCC / 2).toFixed(2))
    : null;
  const ccServCola = diasServ > 0
    ? Math.max(1.5, +(ccServRodeo - spreadCC / 2).toFixed(2))
    : null;

  // Preñez estimada por posición — misma tabla CC→preñez que el modelo principal (interpCC).
  const prenezCabeza = ccServCabeza !== null ? interpCC(ccServCabeza).pr : null;
  const prenezCola   = ccServCola   !== null ? interpCC(ccServCola).pr   : null;
  ...
  let recAmplitud = "";
  if (diasServ > 90) {
    const esContinuo = cadena.esContinuo || diasServ >= 180;
    const mP = partoTemp?.getMonth(), mT = partoTard?.getMonth();
    const mStr = mP != null && mT != null ? ` (${MESES[mP]}–${MESES[mT]})` : "";
    recAmplitud = esContinuo
      ? `Servicio de ${diasServ}d — continuo/sin estacionar${mStr}. Sin períodos secos ni manejo por categorías. Meta inmediata: 120d → objetivo 90d. `
      : diasServ > 120
      ? `Servicio de ${diasServ}d — parición muy extendida${mStr}. Acortar con urgencia a 90d. `
      : `Servicio de ${diasServ}d — parición extendida${mStr}. Acortar a 90d al próximo ciclo. `;
  }
...
function interpretarSistemaCompleto(form, motor, sat, tray) {
  ...
  const ccTiempo   = interpretarCCEnTiempo(tray, motor.mesesDeficit ?? 0, motor.peorDeficitMcal ?? 0);
...
// ─── INTERPRETACIÓN DE POTREROS INDIVIDUALES ──────────────────────────────
function interpretarPotreros(potreros, form, motor) {
  const PB_MAP  = { menor_10:12, "10_25":9, "25_50":6, mayor_50:4 };
  const EM_MAP  = { menor_10:2.20, "10_25":2.10, "25_50":1.95, mayor_50:1.80 };
  const MS_MAP  = { menor_10:1800, "10_25":1500, "25_50":1200, mayor_50:900 };
  const EST_MAP = {
    menor_10: { label:"Macollaje / inicio flor", color:"#7ec850" },
    "10_25":  { label:"10–25% floración",        color:"#a0c840" },
    "25_50":  { label:"25–50% espigas",           color:"#e8a030" },
    mayor_50: { label:">50% encañado",            color:"#e05530" },
  };

  const activos = (potreros || []).filter(p => parseFloat(p.ha) > 0 || p.veg);
  if (activos.length === 0) {
    return { sinDatos:true, conclusion:"Sin datos de potreros individuales — diagnóstico forrajero global." };
  }

  let pbSum = 0, haConFenol = 0;
  const haTotal = activos.reduce((s,p) => s + (parseFloat(p.ha)||0), 0);

  const potrerosDx = activos.map((p, idx) => {
    const ha    = parseFloat(p.ha) || 0;
    const fenol = p.fenol || "25_50";
    const veg   = p.veg || "Pastizal natural";
    const alt   = parseFloat(p.altPasto) || null;
    const pb    = PB_MAP[fenol]  || 8;
    const em    = EM_MAP[fenol]  || 1.95;
    const ms    = MS_MAP[fenol]  || 1200;
    const est   = EST_MAP[fenol] || EST_MAP["25_50"];

    if (ha > 0) { pbSum += pb * ha; haConFenol += ha; }

    const esEncañado = pb < 7;
    const esBajaCant = ms < 1200;

    let recom = "";
    if (esEncañado && ha >= 5)
      recom = "Diferir — retirar el rodeo para permitir rebrote. Rotación: 35–45 días descanso en C4.";
    else if (esBajaCant)
      recom = "Disponibilidad baja — reducir carga en este lote.";
    else if (pb >= 10)
      recom = "Buen estado — aprovechar la ventana de alta calidad.";
    else
      recom = "Estado moderado — monitorear fenología.";

    return {
      idx:idx+1, ha, fenol, veg, altPasto:alt,
      pb, em, msHa:ms, estado:est.label, colorEstado:est.color,
      esEncañado, esBajaCant, recom,
    };
  });

  const pbPonderado    = haConFenol > 0 ? Math.round(pbSum / haConFenol * 10) / 10 : 8;
  const haCritica      = potrerosDx.filter(p => p.esEncañado).reduce((s,p) => s+p.ha, 0);
  const pctCritico     = haTotal > 0 ? Math.round(haCritica / haTotal * 100) : 0;
  const potreroCritico = potrerosDx.reduce((mn,p) => p.pb < mn.pb ? p : mn, potrerosDx[0]);
  const mejorPotrero   = potrerosDx.reduce((mx,p) => p.pb > mx.pb ? p : mx, potrerosDx[0]);
  const ordenPastoreo  = [...potrerosDx].sort((a,b) => b.pb - a.pb);

  let conclusion = `${activos.length} potrero${activos.length>1?"s":""} · ${haTotal} ha · PB ponderado ${pbPonderado}%. `;
  if (haCritica > 0) {
    conclusion += `${Math.round(haCritica)} ha encañadas (${pctCritico}%) — sin valor proteico funcional. `;
    conclusion += `Diferir P${potreroCritico.idx} (${potreroCritico.ha} ha) hasta nuevo rebrote.`;
  } else if (pbPonderado >= 10) {
    conclusion += "Campo en buena condición. Aprovechar la ventana de calidad.";
  } else {
    conclusion += "Estado moderado. Mantener rotación con 35–45 días de descanso.";
  }

  return { potrerosDx, pbPonderado, haTotal, haCritica, pctCritico,
           potreroCritico, mejorPotrero, ordenPastoreo, sinDatos:false, conclusion };
}

// ─── INTERPRETACIÓN DE VAQUILLONA 1° Y 2° INVIERNO ────────────────────────
function interpretarVaquillona(motor, form) {
  const pvAdulto = parseFloat(form?.pvVacaAdulta) || 320;
  const nVaq1    = motor?.nVaq1 || 0;
  const nVaq2    = parseInt(form?.vaq2N) || motor?.nVaq2 || 0;
  const e1       = motor?.vaq1E || null;
  const e2       = motor?.vaq2E || null;

  // Identifica el factor limitante principal del sistema
  function detectarLimitante(e, gdpN) {
    if (!e) return "sin_datos";
    const gdpR = e.gdpReal ?? e.gdpInv ?? 0;
    if (e.nivelMS === "baja" && e.calidadBaja) return "escasez_calidad";
    if (e.nivelMS === "baja")                  return "escasez";
    if ((e.pbPastoAct || 0) < 7 || e.calidadBaja) return "proteina";
    if (gdpR < gdpN)                           return "energia";
    return "ok";
  }

  function labelLimitante(id, e) {
    const pb = e?.pbPastoAct ?? "< 7";
    const ms = e?.msHa ?? "?";
    switch (id) {
      case "proteina":        return `Proteína — PB pasto ${pb}% (C4 dormante), microflora ruminal sin activar`;
      case "escasez":         return `Disponibilidad de pasto — ${ms} kg MS/ha insuficiente`;
      case "escasez_calidad": return `Escasez + calidad baja — ${ms} kg MS/ha y PB < 7%: doble restricción`;
      case "energia":         return `Energía — proteína cubierta pero déficit energético neto`;
      default:                return `Sin limitante — sistema en condición`;
    }
  }

  function construirDx(e, cat, pvObjetivo) {
    if (!e) return null;
    const n        = cat === "vaq1" ? nVaq1 : nVaq2;
    const gdpN     = e.gdpNecesario || (cat === "vaq1" ? 533 : 330);
    const gdpReal  = cat === "vaq1" ? (e.gdpReal ?? 0) : (e.gdpReal ?? e.gdpInv ?? 0);
    const gdpPasto = cat === "vaq1" ? (e.gdpPasto ?? 0) : (e.gdpPastoInv ?? 0);
    const pvActual = cat === "vaq1" ? (e.pvSal ?? 0) : (e.pvEntore ?? 0);
    const brechaPV = Math.max(0, Math.round(pvObjetivo - pvActual));
    const llega    = pvActual >= pvObjetivo;
    const pctAdulto = pvAdulto > 0 ? Math.round(pvActual / pvAdulto * 100) : 0;
    const suplCarg  = form?.[`supl_${cat}`] || "";
    const dosisCarg = parseFloat(form?.[`dosis_${cat}`]) || 0;
    const limId     = detectarLimitante(e, gdpN);

    // BALANCE: demanda vs oferta en g/d GDP
    const gdpSuplAporte = Math.round(Math.max(0, gdpReal - gdpPasto));
    const balance = {
      gdpDemanda:  gdpN,
      gdpPasto,
      gdpSuplAporte,
      gdpReal,
      deficit:  Math.max(0, gdpN - gdpReal),
      surplus:  Math.max(0, gdpReal - gdpN),
      pvActual, pvObjetivo, brechaPV,
    };

    // ESCENARIO con el sistema cargado
    const pvLabel = cat === "vaq1" ? "PV agosto" : "PV entore";

    // Para vaq2: prefija con el crecimiento primavera-verano estimado desde los potreros
    const prefijoVerano = cat === "vaq2" && e.gdpVeranoProm
      ? `Primavera-verano (ago–may): ${e.gdpVeranoProm} g/d promedio (+${e.gananciaVerano} kg) → PV entrada 2° inv: ${e.pvMayo2Inv} kg. `
      : "";

    let escenario;
    if (e.sinSupl && limId !== "ok") {
      escenario = prefijoVerano +
                  `Sin suplemento invernal: pasto aporta ${gdpPasto >= 0 ? "+" : ""}${gdpPasto} g/d. ${pvLabel} ${pvActual} kg (${pctAdulto}% PV adulto). ` +
                  (llega ? `Dentro del objetivo (${pvObjetivo} kg).` : `Brecha ${brechaPV} kg vs objetivo ${pvObjetivo} kg.`);
    } else if (suplCarg && dosisCarg > 0) {
      escenario = prefijoVerano +
                  `Con ${dosisCarg} kg/cab ${suplCarg}: GDP ${gdpReal} g/d. ${pvLabel} ${pvActual} kg (${pctAdulto}% PV adulto). ` +
                  (llega ? "Objetivo cumplido." : `Brecha ${brechaPV} kg vs objetivo ${pvObjetivo} kg.`);
    } else {
      escenario = prefijoVerano +
                  `${pvLabel} proyectado ${pvActual} kg (${pctAdulto}% PV adulto, objetivo ${pvObjetivo} kg). ` +
                  (llega ? "Objetivo cumplido." : `Brecha ${brechaPV} kg.`);
    }

    // RECOMENDACIÓN: qué hacer para cerrar la brecha
    const dur = "90 días (jun–ago)";
    let recomendacion;

    if (llega && e.sinSupl) {
      recomendacion = `Sistema en condición sin suplemento. Control de pesadas en agosto.`;
    } else if (llega && !e.sinSupl) {
      recomendacion = `Mantener ${dosisCarg} kg/cab ${suplCarg}, ${e.freq || "3×/semana"}, ${dur}.`;
    } else if (limId === "proteina" || (e.calidadBaja && e.sinSupl)) {
      const pvConRec = cat === "vaq1" ? (e.pvSalRecom ?? "—") : "—";
      recomendacion = `${e.protKg || 0.5} kg/cab/día expeller girasol o soja (PB ≥ 30%), ${e.freq || "3 veces/semana"}, ${dur}.` +
                      (e.energKg > 0 ? ` + ${e.energKg} kg/cab grano energético (pasto escaso).` : "") +
                      (pvConRec !== "—" ? ` GDP esperado: ${e.gdpRecom || "?"} g/d → ${pvLabel} ${pvConRec} kg.` : "");
    } else if (limId === "escasez" || limId === "escasez_calidad") {
      recomendacion = `${e.protKg || 0.5} kg proteico + ${e.energKg || 0.5} kg grano/cab, diario.` +
                      (e.fuenteEnerg ? ` ${e.fuenteEnerg}.` : "") +
                      ` Evaluar rollo o heno como fibra base.`;
    } else if (limId === "energia") {
      recomendacion = `${e.energKg || 0.3} kg sorgo o maíz molido/cab/día + ${e.protKg || 0.3} kg proteico, ${e.freq || "3×/semana"}.`;
    } else if (!e.sinSupl && balance.deficit > 0) {
      recomendacion = `Aumentar dosis: GDP actual ${gdpReal} g/d, déficit ${balance.deficit} g/d. Incrementar proteico a ${e.protKg || Math.round((dosisCarg + 0.3)*10)/10} kg/cab.`;
    } else {
      recomendacion = `GDP ${gdpReal} g/d vs mínimo ${gdpN} g/d. Revisar disponibilidad de pasto y suplemento.`;
    }

    if (cat === "vaq2") {
      recomendacion += ` Flushing 25 días pre-servicio: 0.5 kg grano/cab/día.`;
      if (e.aptaEntoreAntic)
        recomendacion += ` PV agosto ${e.pvV2Agosto} kg (${e.pctPVAgosto}% PV adulto) — evaluar entore a 24 meses.`;
    }

    return {
      n, balance,
      limitante: limId, limitanteDetalle: labelLimitante(limId, e),
      escenario, recomendacion, llega,
      // campos legacy que tabs.js todavía puede usar
      gdpReal, gdpMin: gdpN,
      pvSalida:    cat === "vaq1" ? pvActual : undefined,
      pvObjetivo,
      pvEntore:    cat === "vaq2" ? e.pvEntore : undefined,
      pvMinEntore: cat === "vaq2" ? (e.pvMinEntore ?? pvObjetivo) : undefined,
      pct: pctAdulto,
      aptaEntoreAntic: cat === "vaq2" ? (e.aptaEntoreAntic || false) : undefined,
    };
  }

  const pvObj1 = Math.round(pvAdulto * 0.65);
  const pvObj2 = e2?.pvMinEntore || Math.round(pvAdulto * 0.75);
  const vaq1   = nVaq1 > 0 && e1 ? construirDx(e1, "vaq1", pvObj1) : null;
  const vaq2   = nVaq2 > 0 && e2 ? construirDx(e2, "vaq2", pvObj2) : null;
  const hayVaq1 = !!vaq1;
  const hayVaq2 = !!vaq2;

  let conclusionGeneral = "Sin datos de reposición.";
  if (hayVaq1 && !vaq1.llega)      conclusionGeneral = `Vaq1: brecha ${vaq1.balance.brechaPV} kg — invierno actual es la ventana de intervención.`;
  else if (hayVaq2 && !vaq2.llega) conclusionGeneral = `Vaq2: brecha ${vaq2.balance.brechaPV} kg al entore — ajustar suplementación invernal.`;
  else if (hayVaq1 || hayVaq2)     conclusionGeneral = "Reposición proyectada dentro de parámetros. Control pesadas mensuales.";

  return { vaq1, vaq2, hayVaq1, hayVaq2, conclusionGeneral };
}

function calcCerebro(motor, form, sat, potreros = []) {
  ...
  // ── INTERPRETACIONES PREVIAS — única fuente de verdad ─────────
  ...
  const mesesDeficit = motor.mesesDeficit ?? 0;
  const deficitAcumInv = motor.deficitAcumInv ?? 0;
  ...
  // PB del pasto — promedio ponderado por potrero si hay datos, sino fenología global
  const PB_MAP = { menor_10:12, "10_25":9, "25_50":6, mayor_50:4 };
  const potrerosConDatos = (potreros || []).filter(p => parseFloat(p.ha) > 0);
  const totalHaPot = potrerosConDatos.reduce((s,p) => s + (parseFloat(p.ha)||0), 0);
  const PB_PASTO_HOY = totalHaPot > 0
    ? potrerosConDatos.reduce((s,p) => s + (PB_MAP[p.fenol]||8) * (parseFloat(p.ha)||0), 0) / totalHaPot
    : (PB_MAP[fenologia] || 8);
  ...
  // pvSalidaVaq1 llega como string desde el motor → parseFloat para evitar
  // que "185" + 21 concatene en "18521" al proyectar el PV de agosto.
  const pvVaq1Sal    = parseFloat(pvSalidaVaq1) || vaq1E?.pvSal || 0;
  ...
  // Agua — usa interpretarAgua como fuente de verdad
  const aguaSal        = interp?.agua?.hayProblema || false;
  const reduccionDMI   = interp?.agua?.pctReduc    || 0;
  ...
  // Erosión y ciclos — delega en interpretarCCEnTiempo como fuente de verdad
  const erosionCCAnual  = interp?.ccTiempo?.erosionAnual    ?? 0;
  const ciclosAlColapso = interp?.ccTiempo?.ciclosAlColapso ?? null;
  ...
  // Sanidad — delega en interpretarSanidadReproductiva como fuente de verdad
  const penalidadSan = interp.sanidad;
  ...
  // amplitudParicion ya fue calculado por interpretarSistemaCompleto
  const amplitudCalc    = interp.amplitud;
  ...
  // Detecta problemas estructurales que los indicadores globales no capturan
  if (potrerosConDatos.length > 0) {
    const haTotal       = Math.max(totalHaPot, supHa || 1);
    const mesInvierno   = [4,5,6,7,8].includes(mesHoy);

    // ── 1. Encañado dominante (>50% ha en fenol "mayor_50") ──────
    const haEncanado = potrerosConDatos
      .filter(p => p.fenol === "mayor_50" && (p.veg||"").toLowerCase().includes("pastizal"))
      .reduce((s,p) => s + (parseFloat(p.ha)||0), 0);
    const pctEncanado = haEncanado / haTotal;
    if (pctEncanado >= 0.5) {
      const haySuplProt = suplVacas && dosisVacas > 0 &&
        suplMeses.some(m => [4,5,6,7,8].includes(m));
      añadir({
        id: "pot_encanado",
        prioridad: mesInvierno && !haySuplProt ? "P1" : "P2",
        icono: "🌿",
        titulo: `${Math.round(pctEncanado*100)}% del área en pastizal encañado — proteína limitante`,
        impacto: `Pastizal C4 encañado: PB 3-4% (mínimo ruminal 7%). Consumo voluntario cae 30-40% aunque haya pasto en cantidad. Ningún suplemento energético compensa sin proteína.`,
        solucion: haySuplProt
          ? `Suplemento proteico activo — confirmar dosis ≥0.4 kg/cab/día 3×/semana. Planificar quema o rolado de ${Math.round(haEncanado)} ha en Ago-Sep para renovación.`
          : `Iniciar suplemento proteico [P]: expeller girasol 0.4 kg/cab/día 3×/sem · ${fmtMes(4)}-${fmtMes(7)}. Quemar/rodar ${Math.round(haEncanado)} ha en agosto para rebrote de calidad.`,
        cuandoActuar: mesInvierno ? "AHORA — pastizal ya encañado" : `Antes de ${fmtMes(4)}`,
        mesAccion: mesInvierno ? mesHoy : 4,
        categoria: "Pastizal",
        cuantifica: `${Math.round(haEncanado)} ha encañadas · ${nVacasN} cab`,
      });
    }

    // ── 2. Megatérmicas C4 dominantes sin suplemento invernal ────
    const haC4 = potrerosConDatos
      .filter(p => (p.veg||"").includes("Megatérmicas"))
      .reduce((s,p) => s + (parseFloat(p.ha)||0), 0);
    const pctC4 = haC4 / haTotal;
    const haySuplInv = suplVacas && dosisVacas > 0 &&
      suplMeses.some(m => [5,6,7].includes(m));
    if (pctC4 >= 0.6 && !haySuplInv) {
      añadir({
        id: "pot_c4_sin_supl",
        prioridad: [5,6,7].includes(mesHoy) ? "P1" : "P2",
        icono: "🌱",
        titulo: `${Math.round(pctC4*100)}% del área en C4 sin suplemento invernal — riesgo estructural`,
        impacto: `Megatérmicas C4 en invierno (jun-ago): PB cae a 4-6%, digestibilidad 45-50%. Sin proteína suplementaria la fermentación ruminal colapsa aunque haya masa forrajera.`,
        solucion: `[P] Expeller girasol 0.4 kg/cab/día · 3×/sem · jun-ago. Costo/vaca/campaña ≈ 36 kg × precio girasol. Impacto: microflora activa → +30-40% aprovechamiento del pasto disponible.`,
        cuandoActuar: `${fmtMes(4)} — antes de que la C4 deje de crecer`,
        mesAccion: 4,
        categoria: "Pastizal",
        cuantifica: `${Math.round(haC4)} ha de C4 · ${nVacasN + nV2s + nToros} cab`,
      });
    }

    // ── 3. Monte/bosque como % relevante del área ────────────────
    const haMonte = potrerosConDatos
      .filter(p => (p.veg||"").includes("Bosque"))
      .reduce((s,p) => s + (parseFloat(p.ha)||0), 0);
    const pctMonte = haMonte / haTotal;
    if (pctMonte >= 0.20) {
      const haEfectiva   = Math.round(haTotal - haMonte);
      const cargaReal    = nVacasN > 0 && haEfectiva > 0
        ? (nVacasN / haEfectiva).toFixed(2) : null;
      añadir({
        id: "pot_monte",
        prioridad: "P2",
        icono: "🌳",
        titulo: `${Math.round(pctMonte*100)}% del área es monte/bosque — carga real ${cargaReal ? cargaReal+" EV/ha efectiva" : "subestimada"}`,
        impacto: `Superficie ganadera efectiva = ${haEfectiva} ha (sin los ${Math.round(haMonte)} ha de monte). La carga calculada sobre supHa total subestima el impacto real en el pastizal.`,
        solucion: `Usar ${haEfectiva} ha como base de cálculo. El monte aporta sombra, refugio y frutos estacionales pero no como forraje base. Si la carga efectiva supera 1.0 EV/ha → revisar rodeo.`,
        cuandoActuar: `Próximo tacto/manejo`,
        mesAccion: mesHoy,
        categoria: "Pastizal",
        cuantifica: `${Math.round(haMonte)} ha de monte · carga real ${cargaReal || "recalcular"} EV/ha`,
      });
    }

    // ── 4. Potrero con pastizal bajo (<12 cm) ────────────────────
    const potrerosbajos = potrerosConDatos.filter(p =>
      parseFloat(p.altPasto) > 0 &&
      parseFloat(p.altPasto) < 12 &&
      (p.veg||"").includes("Pastizal")
    );
    if (potrerosbajos.length > 0) {
      const haBaja = potrerosbajos.reduce((s,p) => s + (parseFloat(p.ha)||0), 0);
      const altMin = Math.min(...potrerosbajos.map(p => parseFloat(p.altPasto)));
      añadir({
        id: "pot_bajo",
        prioridad: altMin < 8 ? "P1" : "P2",
        icono: "📉",
        titulo: `${potrerosbajos.length > 1 ? potrerosbajos.length + " potreros" : "Potrero"} con pastizal a ${altMin}-${Math.round(haBaja / potrerosbajos.length > 0 ? haBaja/potrerosbajos.length : 10)} cm — oferta comprometida`,
        impacto: `A <12 cm el animal consume más energía pastando que lo que ingiere neto. ${altMin < 8 ? "⚠ <8 cm: sobrepastoreo severo — daño a corona de raíces irreversible en C4." : "Diferir para recuperación."}`,
        solucion: `Cerrar estos potreros 30-45 días mínimo. Objetivo ≥20 cm antes de reingreso. ${altMin < 8 ? "Suspender inmediatamente para evitar daño permanente al pastizal." : "Concentrar la hacienda en otros potreros durante la recuperación."}`,
        cuandoActuar: altMin < 8 ? "INMEDIATO" : `Próximo movimiento de tropa`,
        mesAccion: mesHoy,
        categoria: "Pastizal",
        cuantifica: `${Math.round(haBaja)} ha comprometidas · altura mín. ${altMin} cm`,
      });
    }

    // ── 5. Potreros dispares sin rotación aparente ───────────────
    const alturas = potrerosConDatos
      .filter(p => parseFloat(p.altPasto) > 0 && (p.veg||"").includes("Pastizal"))
      .map(p => parseFloat(p.altPasto));
    if (alturas.length >= 2) {
      const altMax = Math.max(...alturas);
      const altMin2 = Math.min(...alturas);
      if (altMax / Math.max(1, altMin2) >= 3 && altMin2 < 15) {
        añadir({
          id: "pot_disparo",
          prioridad: "P2",
          icono: "🔄",
          titulo: `Potreros dispares: ${altMin2}-${altMax} cm — rotación comprometida`,
          impacto: `Diferencia de ${altMax-altMin2} cm entre potreros indica pastoreo continuo o rotación irregular. Potreros altos maduran y pierden calidad; los bajos se sobrepastorean.`,
          solucion: `Rotación NEA: 7-14 días pastoreo + 35-45 días descanso en verano (C4). Potrero >${Math.round(altMax*0.7)} cm → diferir para confección de rollos o quema controlada de encañado.`,
          cuandoActuar: `Próxima semana — corregir rotación antes de invierno`,
          mesAccion: mesHoy,
          categoria: "Pastizal",
          cuantifica: `Rango: ${altMin2}–${altMax} cm · ratio ${(altMax/Math.max(1,altMin2)).toFixed(1)}x`,
        });
      }
    }

    // ── 6. Discrepancia supHa vs suma potreros (>15%) ────────────
    if (supHa > 0 && totalHaPot > 0) {
      const diff = Math.abs(totalHaPot - supHa) / supHa;
      if (diff > 0.15) {
        añadir({
          id: "pot_ha_mismatch",
          prioridad: "P3",
          icono: "📐",
          titulo: `Superficie total (${supHa} ha) ≠ suma potreros (${Math.round(totalHaPot)} ha) — diferencia ${Math.round(diff*100)}%`,
          impacto: `El balance forrajero usa la superficie total declarada. Una diferencia >15% puede subestimar o sobreestimar la carga y la oferta calculada.`,
          solucion: `Verificar si hay potreros sin cargar o si la superficie total incluye monte, caminos y casco. Actualizar el dato de mayor error para que el balance sea preciso.`,
          cuandoActuar: `Antes de la próxima consulta`,
          mesAccion: mesHoy,
          categoria: "Datos",
          cuantifica: `Diff: ${Math.round(Math.abs(totalHaPot - supHa))} ha (${Math.round(diff*100)}%)`,
        });
      }
    }
  }
  ...
  if (hayVaq1) {
    ...
    // Solo tratar el verdeo como recurso de la vaq1 si realmente está destinado a
    // ella (destino "si"). Antes bastaba con que no fuera "todo" → recomendaba verdeo
    // aunque estuviera asignado a V2S/ternero o el productor lo hubiera sacado.
    const verdeoParaVaq = tieneVerdeo && verdeoDestVaq === "si" && verdeoHa > 0;
  ...
  // Nota: la sal mineral base (30–50 g Ca/P/Mg/Zn/Se a libre acceso todo el año)
  // es una práctica general del NEA/NOA — figura en el manual como recomendación
  // permanente. No se emite como "cuello de botella" para no diluir el plan con una
  // tarjeta genérica que aparecía en todos los informes.
  ...
  const tarjetas = puedeAlcanzarMeta
  ...
  const balancePorCategoria = {
    vacas: balanceMensual.map(m => m.balance ?? 0),
    vaq1: balanceMensual.map(m => m.balVaq1 ?? 0),
    vaq2: balanceMensual.map(m => m.balVaq2 ?? 0),
  };
  const gdpPorCategoria = {
    vaq1: balanceMensual.map(m => m.gdpVaq1Mes ?? null),
    vaq2: balanceMensual.map(m => m.gdpVaq2Mes ?? null),
  };
  const problemas = limitantes.map(l => ({
  ...
  return {
    diagnostico: {
      ...
      balancePorCategoria,
      gdpPorCategoria,
      potreros:   interpretarPotreros(potreros, form, motor),
      vaquillona: interpretarVaquillona(motor, form),
      eficiencia: interp?.eficiencia || null,
      momento:    interp?.momento    || null,
      verdeo:     interp?.verdeo     || null,
    },
    prescripciones: {
      tarjetas,
      tarjetasOptim: optimizacion.oportunidades,
      recomendaciones: limitantes.map(l => ({
        prioridad: l.prioridad,
        categoria: l.categoria,
        accion: l.titulo,
        dosis: l.cuantifica || "",
        momento: l.cuandoActuar || "",
        duracion: "",
        resultadoEsperado: l.impacto || "",
        deltaProyectado: { cuantifica: l.cuantifica || "" },
        fuente: "",
      })),
      escenarios: [],
      cronograma,
...
- CC servicio optima: 4.5 => 80% prenez. Riesgo: 4.0-4.4. Critica: <4.0 => anestro profundo (Selk 1988; Kunkle 2000).
...
- V2S: hiperprecoz si CC < 4.0. Requerimiento +10% vs adulta (NASEM 2016).
- Anestro maximo 55d para llegar al servicio. Energia preparto determina onset LH (Wiltbank et al. 1962).

REQUERIMIENTOS POR CATEGORIA (NASEM 2016 — referencia para cuantificar brechas):
- Vaca adulta seca-gestante (NEA invierno C4): ~7-8 Mcal EM/d segun PV y temperatura
- Vaca lactando pico: ~12-14 Mcal EM/d (V2S hasta 15 Mcal EM/d por crecimiento simultaneo)
- Vaquillona 1 inv (crecimiento 400g/d): ~9-10 Mcal EM/d
- Toro activo: 10-12 Mcal EM/d en servicio
- C4 encanado (>50% espigas): PB 4%, EM 1.8-2.0 Mcal/kg MS — limita consumo voluntario (Detmann 2010)

SUPLEMENTACION — TIPOS Y EFECTOS (Moore et al. 1999):
- [P] PROTEICO: cuando PB pasto <7%. Activa microflora ruminal — boost CV real hasta 30%. Dosis 0.3-0.5 kg/cab/d expeller girasol. Frecuencia 2-3x/semana suficiente.
- [E] ENERGETICO: cuando pasto escaso (<1200 kgMS/ha). Compensa EM faltante. Grano maiz/sorgo 1-3 kg/cab/d DIARIO para evitar acidosis. No mejora digestibilidad del forraje base.
- [EP] ENERGETICO+PROTEICO: deficit mixto (PB<7% Y cantidad baja). Expeller + grano o DDGS. Maximo impacto pero mayor costo.
- PREPARTO: suplementacion 45-60d antes del parto. Evita caida CC >1 punto. Impacto directo en duracion anestro pospartum (Short et al. 1990).
...
4. Balance invernal: el deficit erosiona CC ciclo a ciclo (Wiltbank et al. 1962).
...
function buildPromptFull(motor, form, sat, cerebro_data, potreros = []) {
  ...
  if (sat?.ndvi) {
    const ndviDeltaStr = sat.ndviDelta !== undefined ? " | delta vs hist: " + (sat.ndviDelta > 0 ? "+" : "") + sat.ndviDelta + " (" + (sat.ndviCateg||"") + ")" : "";
    const p90str = sat.p90 !== undefined ? " | Lluvia 90d: " + sat.p90 + "mm" : "";
    lines.push("NDVI: " + sat.ndvi + " (" + (sat.condForr||"ND") + ")" + ndviDeltaStr + " | Temp: " + sat.temp + "C | Lluvia 30d: " + (sat.p30||"ND") + "mm" + p90str);
    const pronostico = [];
  ...
  if (potreros && potreros.filter(p => p.ha || p.veg).length > 0) {
    ...
    // Diagnósticos forrajeros per-potrero ya calculados por cerebro
    const dxPot = (cerebro_data?.prescripciones?.tarjetas || [])
      .filter(t => t.categoria === "Pastizal" || t.categoria === "Datos");
    if (dxPot.length > 0) {
      lines.push("DIAGNOSTICO FORRAJERO POR POTRERO:");
      dxPot.forEach(t => lines.push("  [" + t.prioridad + "] " + t.titulo + " — " + (t.impacto||"") + " → " + (t.solucion||"")));
    }
    lines.push("");
  }
  ...
  lines.push("");
  if (motor.mesesDeficit > 0) {
    lines.push("DEFICIT JUN-AGO: " + defInv.map(m=>MC[m.i]+" "+Math.round(m.balance)+"Mcal").join(" | "));
    const dAcum = motor.deficitAcumInv ?? 0;
    lines.push("Acumulado: " + Math.round(dAcum) + " Mcal = " + (nVacas>0?(dAcum/nVacas/5.6).toFixed(1):"ND") + " puntos CC movilizados/vaca sin correccion");
  ...
  if (motor?.vaq1E && !motor.vaq1E.mensaje) {
    ...
    const gdpVaq1Arr = bm.map(m => m.gdpVaq1Mes ?? null);
    if (gdpVaq1Arr.some(v => v !== null)) {
      lines.push("GDP vaq1 mes a mes [g/d]: " + MC.map((m,i) => m+":" + (gdpVaq1Arr[i]??"-")).join(" | "));
    }
  }
  if (motor?.vaq2E) {
    ...
    const gdpVaq2Arr = bm.map(m => m.gdpVaq2Mes ?? null);
    if (gdpVaq2Arr.some(v => v !== null)) {
      lines.push("GDP vaq2 mes a mes [g/d]: " + MC.map((m,i) => m+":" + (gdpVaq2Arr[i]??"-")).join(" | "));
    }
  }
  lines.push("");
  ...
  if (cerebro_data) {
    const cxDx  = cerebro_data.diagnostico   || {};
    const cxPrx = cerebro_data.prescripciones || {};
    lines.push("=== ANALISIS PRE-CALCULADO ===");

    // Diagnóstico de sustentabilidad
    if (cxDx.diagnosticoSustentabilidad?.resumen) lines.push("Diagnostico sustentabilidad: " + cxDx.diagnosticoSustentabilidad.resumen);
    if (cxDx.diagnosticoSustentabilidad?.factoresLimitantes?.length > 0) {
      lines.push("Factores limitantes: " + cxDx.diagnosticoSustentabilidad.factoresLimitantes.join(" | "));
    }

    // Resumen cuantitativo
    if (cxPrx.resumen) {
      const r = cxPrx.resumen;
    ...
    const limitantes = cxPrx.limitantes || [];
    if (limitantes.length > 0) {
      lines.push("LIMITANTES JERARQUIZADOS:");
      limitantes.slice(0, 9).forEach((t, i) => {
        const impact = t.impacto || t.cuantifica || "";
        const sol    = t.solucion || "";
        lines.push("  " + (i+1) + ". [" + (t.prioridad||"") + "] [" + (t.categoria||"") + "] " + t.titulo + (impact ? " — " + impact : "") + (sol ? " → " + sol : ""));
      });
    }

    // Contexto climático
    if (cxDx.campoSeco)     lines.push("ALERTA CLIMATICA: campo seco — impacto en oferta forrajera y consumo voluntario");
    if (cxDx.callorExtremo) lines.push("ALERTA CLIMATICA: calor extremo — deprime consumo y fertilidad (NRC 2000)");
    if (cxDx.faltaAgua)     lines.push("ALERTA AGUA: TDS critico — reduccion DMI confirmada");
...
// ─── Prompt corto para consulta bibliográfica ─────────────────────
// Solo pide citas y evidencia — NO diagnosis, NO plan, NO reescritura.
function buildPromptBiblio(cb) {
  const limitantes = (cb?.prescripciones?.limitantes || []).slice(0, 4);
  const resumen    = cb?.prescripciones?.resumen || {};
  const dx         = cb?.diagnostico || {};

  const lines = [];
  lines.push("=== DIAGNÓSTICO DEL SISTEMA (calculado algorítmicamente) ===");
  if (dx.resumen) lines.push(dx.resumen);
  if (resumen.prenez) lines.push(`Preñez estimada: ${resumen.prenez}% → potencial con mejoras: ${resumen.prenezPot || "ND"}%`);
  if (resumen.ternerosDif > 0) lines.push(`Brecha cuantificada: +${resumen.ternerosDif} terneros potenciales`);
  lines.push("");

  if (limitantes.length > 0) {
    lines.push("LIMITANTES IDENTIFICADOS:");
    limitantes.forEach((l, i) => {
      lines.push(`${i + 1}. [${l.prioridad}] ${l.titulo}${l.impacto ? " — " + l.impacto : ""}`);
    });
  }
  lines.push("");
  lines.push("=== INSTRUCCIÓN ===");
  lines.push("Para cada limitante listado arriba, aportá el respaldo bibliográfico científico:");
  lines.push("- Cita autor, año, revista/fuente y conclusión clave con magnitud del efecto");
  lines.push("- Fuentes válidas: NRC 2000, Detmann/NASSEM 2010, Wiltbank, INTA, CREA, revistas indexadas");
  lines.push("- Máximo 3 citas por limitante, ordenadas por relevancia");
  lines.push("- NO repetir el diagnóstico — solo bibliografía y evidencia");
  lines.push("- NO sugerir nuevas intervenciones — el plan ya está calculado");

  return lines.join("\n");
}

const SYS_BIBLIO = `Sos un asistente de bibliografía científica veterinaria especializado en ganadería de cría extensiva (NEA/Chaco, Bos indicus, pastizal natural).
Tu único rol es aportar evidencia científica que respalde diagnósticos ya calculados algorítmicamente.
No diagnosticás, no planificás, no repetís información del sistema — solo citás fuentes con rigor académico.
Respondé en español técnico. Cada cita: Autor (año) — Título o conclusión clave — Efecto cuantificado si existe.`;
...
export {
  ...
  interpretarPotreros, interpretarVaquillona,
  SYS_FULL, SYS_BIBLIO, buildPromptFull, buildPromptBiblio, getSysFull,
};
