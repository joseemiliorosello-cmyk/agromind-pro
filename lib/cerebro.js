...
import { SUPLEMENTOS, MESES_NOM, CALIDAD_C4_CALIBRADA, getBiotipo, BIOTIPOS } from "./constantes";
import { recomendarSuplemento, calcFaseCiclo, calcImpactoCola, getClima,
         calcPenalidadSanidad, calcImpactoAguaCalibrado,
         calcConsumoPasto, calcDisponibilidadMS, calcGEI, calcScore,
         calcSupervivencia, calcV2S, fmtFecha, mcalSuplemento } from "./motor";
...
function calcCerebro(motor, form, sat) {
  ...
  // ══════════════════════════════════════════════════════════════
  // REGLAS BIOLÓGICAS FIJAS — el cerebro siempre las aplica
  // independientemente de lo que el productor haga o no haga
  // ══════════════════════════════════════════════════════════════
  const REGLAS = {
    // Vacas adultas
    CC_PARTO_MIN:      5.5,   // Peruchena 2003 — CC al parto mínima
    CC_SERV_MIN:       4.5,   // CC al servicio → 80% preñez
    CC_MIN_LACT:       3.5,   // No bajar de esto en lactación
    ANESTRO_MAX:       55,    // días — para llegar al servicio

    // Vaquillona 1° invierno
    VAQ1_GDP_MIN:      400,   // g/día mayo-agosto mínimo
    VAQ1_PCT_PV_SAL:   0.40,  // PV salida agosto = 40% PV adulto mínimo

    // Vaquillona 2° invierno
    VAQ2_GDP_MIN:      300,   // g/día mínimo
    VAQ2_PCT_PV_ENT:   0.75,  // PV entore = 75% PV adulto mínimo
    VAQ2_FLUSHING_DIAS: 25,   // días pre-servicio siempre

    // V2S
    V2S_CC_HIPER:      4.0,   // hiperprecoz si CC < 4.0
    V2S_REQ_FACTOR:    1.10,  // +10% requerimiento vs adulta (NRC 2000)
  };

  // Diagnóstico contra las reglas
  const pvAdulto = parseFloat(form.pvVacaAdulta) || 380;
  const brechaCC_parto  = Math.max(0, REGLAS.CC_PARTO_MIN - (tray?.ccParto || 0));
  const brechaCC_serv   = Math.max(0, REGLAS.CC_SERV_MIN  - (tray?.ccServ  || 0));
  const brechaAnestro   = Math.max(0, (tray?.anestro?.dias || 0) - REGLAS.ANESTRO_MAX);

  const vaq1GDP    = motor.vaq1E?.gdpReal || 0;
  const vaq1pvSal  = motor.pvSalidaVaq1 || 0;
  const vaq1Obj    = Math.round(pvAdulto * REGLAS.VAQ1_PCT_PV_SAL);
  const vaq1BrechaPv = Math.max(0, vaq1Obj - vaq1pvSal);
  const vaq1BrechaGdp= Math.max(0, REGLAS.VAQ1_GDP_MIN - vaq1GDP);
  const vaq1Critico  = vaq1GDP < REGLAS.VAQ1_GDP_MIN || vaq1pvSal < vaq1Obj;

  const vaq2pvEnt  = motor.vaq2E?.pvEntore || 0;
  const vaq2Obj    = Math.round(pvAdulto * REGLAS.VAQ2_PCT_PV_ENT);
  const vaq2LlegaR = motor.vaq2E?.llegas ?? (vaq2pvEnt >= vaq2Obj);
  const vaq2Critico= !vaq2LlegaR;

  const ccV2sPond  = (() => {
    const d = form.cc2sDist || [];
    let s=0,t=0;
    d.forEach(x=>{const p=parseFloat(x.pct)||0,c=parseFloat(x.cc)||0;s+=p*c;t+=p;});
    return t>0?s/t:0;
  })();
  const v2sNecesitaHiper = ccV2sPond > 0 && ccV2sPond < REGLAS.V2S_CC_HIPER;
  ...
  // ═══════════════════════════════════════════════════════════════
  // ESCENARIOS COMPARATIVOS — base del informe
  // Calcula qué pasa con el balance si se aplican las recomendaciones
  // ═══════════════════════════════════════════════════════════════
  const calcEscenario = (ajustes) => {
    // ajustes: { destete, suplMcalExtra, verdeoHa, reducCarga }
    return balanceMensual.map(m => {
      let ofExtra = 0;
      // Destete: libera 6-8 Mcal/día en meses de lactación (Short 1990)
      if (ajustes.destete && m.enLact) ofExtra += 6.5 * nVacas;
      // Suplemento extra: Mcal/día adicionales en meses invernales
      if (ajustes.suplMcalExtra && [5,6,7].includes(m.i)) ofExtra += ajustes.suplMcalExtra;
      // Verdeo: aporte adicional en meses configurados
      if (ajustes.verdeoMcal && [6,7,8].includes(m.i)) ofExtra += ajustes.verdeoMcal;
      // Reducción carga: menos demanda por reducir categorías improductivas
      const demExtra = ajustes.reducCarga ? -ajustes.reducCarga : 0;
      const balanceEsc = m.balance + ofExtra + demExtra;
      return {
        i: m.i, mes: m.mes,
        balance: +balanceEsc.toFixed(0),
        deficit: balanceEsc < 0,
        ofExtra: +ofExtra.toFixed(0),
      };
    });
  };

  // Escenario 1: Solo destete anticipado en vacas con CC < 4.5
  const pctConDestAntic = balanceMensual.some(m => m.enLact) ? 0.4 : 0; // 40% con dest. anticip.
  const escDestete = calcEscenario({ destete: pctConDestAntic > 0 });

  // Escenario 2: Suplementación invernal óptima
  // Calcular cuánto suplemento proteico necesita el rodeo en junio (mes más crítico)
  const balJunEsc = balanceMensual[5]?.balance || 0;
  const suplNecesarioMcal = balJunEsc < 0 ? Math.abs(balJun) : 0;
  const escSuplemento = calcEscenario({ suplMcalExtra: suplNecesarioMcal });

  // Escenario 3: Combinado — destete + suplementación
  const escCombinado = calcEscenario({
    destete: pctConDestAntic > 0,
    suplMcalExtra: suplNecesarioMcal * 0.6, // menos suplemento si hay destete
  });

  // Mejora en preñez para cada escenario (Peruchena 2003)
  const interpPrenezCC = (cc) => cc >= 5.5 ? 93 : cc >= 5.0 ? 88 : cc >= 4.5 ? 80 : cc >= 4.0 ? 70 : cc >= 3.5 ? 50 : 28;
  const ccMejoraDestete = ccServ ? Math.min(7, ccServ + 0.4) : 0;
  const ccMejoraSupl    = ccServ ? Math.min(7, ccServ + 0.3) : 0;
  const ccMejoraComb    = ccServ ? Math.min(7, ccServ + 0.6) : 0;
  ...
  const escenarios = [
    {
      ...
      prenez: prenezEst,
      ccServ: ccServ,
      descripcion: "Balance con el manejo actual",
    },
    {
      id: "destete",
      label: "Con destete anticipado",
      color: "#4a9eff",
      balance: escDestete,
      prenez: interpPrenezCC(ccMejoraDestete),
      ccServ: +ccMejoraDestete.toFixed(1),
      descripcion: `Destete al 40% anticipado en vacas CC < 4.5 → +${(interpPrenezCC(ccMejoraDestete)-prenezEst)}pp preñez`,
      dosisRecomendada: null,
    },
    {
      id: "suplemento",
      label: "Con suplementación invernal",
      color: "#7ec850",
      balance: escSuplemento,
      prenez: interpPrenezCC(ccMejoraSupl),
      ccServ: +ccMejoraSupl.toFixed(1),
      descripcion: suplNecesarioMcal > 0
        ? `Cubrir déficit invernal (${Math.round(suplNecesarioMcal/nVacas || 0)} Mcal/vaca/d) → +${(interpPrenezCC(ccMejoraSupl)-prenezEst)}pp preñez`
        : "Balance invernal cubierto — no requiere suplemento",
      dosisRecomendada: suplNecesarioMcal > 0
        ? `${(suplNecesarioMcal / Math.max(1,nVacas) / 2.6).toFixed(1)} kg expeller girasol/vaca/día`
        : null,
    },
    {
      id: "combinado",
      label: "Destete + suplementación",
      color: "#c084fc",
      balance: escCombinado,
      prenez: interpPrenezCC(ccMejoraComb),
      ccServ: +ccMejoraComb.toFixed(1),
      descripcion: `Máxima mejora → +${(interpPrenezCC(ccMejoraComb)-prenezEst)}pp preñez`,
    },
  ];

  // Bibliografía por recomendación
  const bibliografiaRecom = {
    destete: [
      "Short et al. (1990): destete reduce anestro posparto de 90 a 14 días en vacas con CC < 4.5",
      "Wiltbank et al. (1990): retiro del ternero restablece pulsatilidad LH en 7-14 días",
      "Peruchena INTA (2003): CC 4.5 → preñez 80%; CC 4.0 → preñez 70%",
    ],
    suplemento: [
      "Detmann & NASSEM (2010): proteína degrada fibra → activa microflora → +GDP y +CV",
      "Balbuena INTA (2003): suplemento proteico 2-3x/semana en pasto <8% PB",
      "Lippke (1980): CV% PV por fenología: <10%flor→2.8%; >50%flor→1.6% PV",
    ],
    agua: [
      "López et al. (2021, JAS 99): TDS >3000 mg/L reduce DMI 2-5% y consumo agua 8%",
      "Winchester & Morris (1956): base del cálculo de consumo hídrico bovino",
    ],
    vaquillona: [
      "Bavera (2005): umbral 65% PV adulto para ciclicidad en vaquillona",
      "Rosello Brajovich et al. (2025): suplementación invernal 2 ciclos NEA/NOA",
      "NRC (2000): requerimientos energéticos por categoría y estado fisiológico",
    ],
    gei: [
      "IPCC (2019) Tier 2, Ec.10.21: CH4 entérico por categoría",
      "Gere et al. (2019, NZ J. Agric. Res.): Ym calibrado pastizal neotropical",
      "Wang et al. (2023, Nature Comms): ineficiencia reproductiva = mayor fuente emisiones",
    ],
  };
...
const SYS_FULL = `Sos el cerebro analítico de Calf AI — sistema de diagnóstico ganadero para cría bovina extensiva en NEA, NOA, Paraguay y Brasil.

TU FORMA DE RAZONAR — estos 5 pasos en orden:

1. CUELLO DE BOTELLA PRINCIPAL
   Una causa raíz con número. La variable que más limita la preñez final.
   Preguntate: si corrigiese solo UNA cosa, cuál tiene el mayor impacto?

2. CADENA CAUSAL COMPLETA
   Siempre: [NDVI/clima] → [oferta Mcal] → [balance] → [CC] → [anestro] → [preñez] → [kg ternero]
   Con los números reales del sistema, no genérico.

3. REGLAS DE PRIORIDAD — siempre en este orden:
   a) DESTETE antes que cualquier suplemento a la vaca adulta
      El destete libera 0.75-1.0 Mcal/día que la leche consume. Es gratis.
      Hiperprecoz (50d) en CC < 4.0. Anticipado (90d) en CC 4.0-4.4. Trad en CC >= 4.5.
   b) V2S (vacas 2° servicio) — categoría MÁS CRÍTICA
      Crecimiento + lactación + gestación simultáneos. Req +10% vs adulta (NRC 2000).
      Hiperprecoz en V2S ANTES que en cualquier adulta.
   c) VAQUILLONA 1° INVIERNO — irreversible
      Si no gana 400g/día este invierno, no llega al 2° servicio en condición.
      El déficit de este invierno NO se recupera en la primavera siguiente.
      Suplemento proteico cuando PB pasto < 7% (Balbuena INTA 2003).
   d) VAQUILLONA 2° INVIERNO — control obligatorio
      Pesar en abril-mayo ANTES del invierno. Decidir suplementación EP o P según déficit real.
      FLUSHING siempre 25 días pre-servicio — aumenta tasa ovulatoria.
   e) CREEP FEEDING — cuando se quiere aumentar peso al destete
      El ternero come sin competir con la madre. GDP 750 vs 700 g/día.

4. MOMENTO DEL AÑO
   ¿Cuántos días al servicio? ¿La ventana está abierta o cerrada?
   Cada mes de demora es CC que no se recupera.

5. CUANTIFICAR SIEMPRE
   Preñez actual → preñez con corrección. CC actual → CC al servicio.
   Terneros adicionales = nVacas × (prenezMejora - prenezActual) / 100 × 0.95.

REGLAS DE ESCRITURA:
- Párrafos directos, sin listas
- Sin insumos específicos si no están cargados en el sistema
- Bibliografía solo cuando justifica un número: Peruchena 2003, Selk 1988, Short 1990, Wiltbank 1990, NRC 2000, Detmann/NASSEM 2010, Balbuena INTA 2003
- El técnico ya sabe ganadería — no explicar conceptos básicos
`


function buildPromptFull(motor, form, sat, cerebro_data) {
  ...
  const esc  = cerebro_data?.escenarios || [];
  const nVacas = motor?.nVacas || parseInt(form?.vacasN) || 0;

  const diasAlServicio = motor?.cadena?.ini
    ? Math.max(0, Math.round((new Date(motor.cadena.ini) - hoy) / 86400000))
    : null;

  const ventana = diasAlServicio === null ? "sin fechas de servicio"
    : diasAlServicio < 30  ? "CRITICA - menos de 30 dias al servicio"
    : diasAlServicio < 60  ? "URGENTE - menos de 60 dias al servicio"
    : diasAlServicio < 120 ? "IMPORTANTE - menos de 4 meses al servicio"
    : "AMPLIA - mas de 4 meses para actuar";

  const defInv = [5,6,7].map(i => bm[i]).filter(m => m?.balance < 0);
  const defAcum = defInv.reduce((s,m) => s + Math.abs(m.balance), 0);
  const defPorVaca = nVacas > 0 && defAcum > 0 ? (defAcum / nVacas).toFixed(1) : "0";

  const escActual = esc.find(e => e.id === "actual");
  const escMejor  = esc.filter(e => e.id !== "actual")
    .reduce((a,b) => b.prenez > a.prenez ? b : a, esc[1] || { prenez: 0 });
  const gananciaMax = (escMejor?.prenez || 0) - (escActual?.prenez || 0);
  const ternerosExtra = nVacas > 0 && gananciaMax > 0 ? Math.round(nVacas * gananciaMax / 100) : 0;

  const lines = [];

  lines.push("=== SISTEMA A ANALIZAR ===");
  lines.push("ESTABLECIMIENTO: " + (form?.nombreProductor || "sin nombre"));
  lines.push("UBICACION: " + ([form?.localidad, form?.provincia, form?.pais].filter(Boolean).join(", ") || "sin especificar"));
  lines.push("BIOTIPO: " + (form?.biotipo || "sin especificar"));
  lines.push("FECHA: " + ML[mesHoy] + " " + hoy.getFullYear() + " | VENTANA: " + ventana);
  if (diasAlServicio !== null) lines.push("DIAS AL SERVICIO: " + diasAlServicio + "d");
  lines.push("");

  lines.push("=== ESTADO DEL SISTEMA ===");
  lines.push("Vacas: " + (form?.vacasN || "ND") + " cab | PV adulto: " + (form?.pvVacaAdulta || "ND") + " kg");
  lines.push("CC ponderada HOY: " + (motor?.ccPondVal?.toFixed(1) || "ND") + " (escala 1-9 INTA)");
  lines.push("CC al SERVICIO proyectada: " + (tray?.ccServ || "ND") + " -> prenez estimada: " + (tray?.pr || "ND") + "%");
  lines.push("Anestro posparto: " + (tray?.anestro?.dias || "ND") + " dias");
  if (sat?.ndvi) lines.push("NDVI hoy: " + sat.ndvi + " (" + (sat.condForr || "") + ") | Temp: " + sat.temp + "C | Lluvia 30d: " + sat.p30 + "mm");
  if (form?.aguaTDS) lines.push("Agua: TDS " + form.aguaTDS + " mg/L (" + (form.aguaTipoSal || "tipo desconocido") + ")");
  lines.push("");

  lines.push("=== BALANCE FORRAJERO MES A MES (Mcal/dia rodeo) ===");
  bm.forEach(m => {
    const bal = Math.round(m.balance || 0);
    const signo = bal >= 0 ? "+" : "";
    lines.push(
      MC[m.i] + ": bal " + signo + bal +
      " | pasto " + Math.round(m.ofPastoTotal||0) +
      " supl " + Math.round(m.suplAporte||0) +
      " cc " + Math.round(m.ccAporte||0) +
      " vs dem " + Math.round(m.demanda||0)
    );
  });
  lines.push("");
  if (defInv.length === 0) {
    lines.push("DEFICIT INVERNAL: Sin deficit - balance cubierto en invierno");
  } else {
    lines.push("DEFICIT INVERNAL (Jun-Jul-Ago):");
    defInv.forEach(m => lines.push("  " + MC[m.i] + ": " + Math.round(m.balance) + " Mcal/dia rodeo"));
    lines.push("  Deficit acumulado: " + Math.round(defAcum) + " Mcal = " + defPorVaca + " Mcal/vaca en esos meses");
    if (nVacas > 0) lines.push("  Equivale a ~" + (defAcum / nVacas / 5.6).toFixed(1) + " unidades CC movilizadas por vaca sin correccion");
  }
  lines.push("");

  lines.push("=== OFERTA - FUENTES DE ENERGIA ===");
  lines.push("Vegetacion: " + (form?.vegetacion || form?.potreros?.[0]?.veg || "Pastizal natural"));
  lines.push("Fenologia actual: " + (form?.fenologia || "no especificada"));
  if (motor?.disponMS) lines.push("Disponibilidad pasto: " + motor.disponMS.msHa + " kgMS/ha (" + motor.disponMS.nivel + ")");
  if ((motor?.verdeoAporteMcalMes || 0) > 0) lines.push("Verdeo activo: " + motor.verdeoAporteMcalMes + " Mcal/dia (" + (form?.verdeoTipo || "") + ")");
  if ((motor?.suplRodeoMcalDia || 0) > 0) lines.push("Suplementacion: " + Math.round(motor.suplRodeoMcalDia) + " Mcal/dia");
  else lines.push("Sin suplementacion cargada");
  if (motor?.factorAgua < 1) lines.push("ALERTA - Agua salobre reduce consumo de pasto -" + Math.round((1-(motor?.factorAgua||1))*100) + "%");
  lines.push("");

  lines.push("=== CADENA REPRODUCTIVA ===");
  if (form?.iniServ && form?.finServ) lines.push("Servicio: " + form.iniServ + " a " + form.finServ + " (" + (motor?.cadena?.diasServ || "?") + "d)");
  lines.push("CC: hoy " + (tray?.ccHoy || "ND") + " -> parto " + (tray?.ccParto || "ND") + " -> min lactacion " + (tray?.ccMinLact || "ND") + " -> servicio " + (tray?.ccServ || "ND"));
  lines.push("Meses lactacion: " + (tray?.mesesLact || "ND") + " | Caida CC: -" + (tray?.caidaLact || "ND"));
  lines.push("");

  lines.push("=== ESCENARIOS DE MEJORA ===");
  esc.forEach(e => {
    const base = esc[0]?.prenez || 0;
    const diff = e.id !== "actual" ? " (+" + (e.prenez - base) + "pp)" : " (base)";
    lines.push(e.label + ": CC serv " + e.ccServ + " -> prenez " + e.prenez + "%" + diff);
  });
  if (gananciaMax > 0) lines.push("Mejora maxima alcanzable: +" + gananciaMax + "pp prenez = +" + ternerosExtra + " terneros adicionales");
  lines.push("");

  if (motor?.vaq1E && !motor.vaq1E.mensaje) {
    lines.push("=== VAQUILLONA 1 INVIERNO ===");
    const v1pv = motor.pvEntVaq1 || "ND";
    const v1gdp = motor.vaq1E.gdpPasto || 0;
    const v1gdpR = motor.vaq1E.gdpReal || 0;
    const v1pvSal = motor.vaq1E.pvSal || 0;
    const v1obj = Math.round((parseFloat(form?.pvVacaAdulta)||380) * 0.40);
    const v1ok = v1pvSal >= v1obj;
    lines.push("PV entrada mayo: " + v1pv + " kg | GDP pasto solo: " + v1gdp + " g/d | Con suplemento: " + v1gdpR + " g/d");
    lines.push("PV salida agosto: " + v1pvSal + " kg | Objetivo: " + v1obj + " kg (40% PV adulto)");
    lines.push(v1ok ? "Llega al objetivo OK" : "NO LLEGA — deficit " + (v1obj-v1pvSal) + " kg. IRREVERSIBLE: si no gana estos kg este invierno no los recupera. No llega al 75% PV al entore de 2 anos.");
    lines.push("Regla clave: sin 400g/dia en 1 invierno, la vaquillona no llega al 2 servicio en condicion. Balbuena INTA 2003.");
    lines.push("");
  }
  if (motor?.vaq2E) {
    lines.push("=== VAQUILLONA 2 INVIERNO ===");
    const v2pv = motor.pvEntradaVaq2 || 0;
    const v2pvEnt = motor.vaq2E.pvEntore || 0;
    const v2min = motor.vaq2E.pvMinEntore || Math.round((parseFloat(form?.pvVacaAdulta)||380)*0.75);
    const v2ok = motor.vaq2E.llegas;
    lines.push("PV entrada mayo: " + v2pv + " kg | PV al entore estimado: " + v2pvEnt + " kg | Minimo: " + v2min + " kg (75% PV adulto)");
    lines.push(v2ok ? "Llega al entore en condicion OK" : "NO LLEGA — deficit " + (v2min-v2pvEnt) + " kg. Revisar suplementacion EP segun tipo de pasto.");
    lines.push("FLUSHING: siempre 25 dias pre-servicio con suplemento — aumenta tasa ovulatoria y prenez primer celo.");
    lines.push("Control de peso obligatorio en abril-mayo ANTES del 2 invierno para decidir suplementacion real.");
    lines.push("");
  }

  // V2S — categoría crítica separada
  const nV2s = motor?.nV2s || 0;
  if (nV2s > 0) {
    const v2sMotor = motor?.dist?.grupos || [];
    const ccV2s = form?.cc2sDist?.length > 0
      ? form.cc2sDist.reduce((s,d)=>{const p=parseFloat(d.pct)||0,c=parseFloat(d.cc)||0;return{s:s.s+p*c,t:s.t+p}},{s:0,t:0})
      : null;
    const ccV2sVal = ccV2s && ccV2s.t > 0 ? (ccV2s.s/ccV2s.t).toFixed(1) : "ND";
    lines.push("=== V2S (VACAS 2 SERVICIO) ===");
    lines.push(nV2s + " cab | CC: " + ccV2sVal + " | PV estimado: " + Math.round((parseFloat(form?.pvVacaAdulta)||380)*0.85) + " kg");
    lines.push("V2S es la categoria MAS CRITICA: crecimiento + lactacion + gestacion simultaneos. Req +10% vs adulta.");
    lines.push("SIEMPRE hiperprecoz en V2S antes que en cualquier adulta — anestro mas prolongado, menor reserva.");
    lines.push("");
  }

  // Creep feeding
  if (form?.tieneCreep === "si" || form?.creepFeeding) {
    lines.push("=== CREEP FEEDING ===");
    lines.push("Activo — GDP ternero estimado: 750 g/dia vs 700 sin creep.");
    lines.push("Libera energia de la madre sin comprometer el ternero. Util cuando vaca tiene CC baja.");
    lines.push("");
  }

  lines.push("=== SANIDAD ===");
  lines.push("Aftosa: " + (form?.sanAftosa === "si" ? "Al dia" : "SIN VACUNAR"));
  lines.push("Brucelosis: " + (form?.sanBrucelosis === "si" ? "Al dia" : "SIN VACUNAR"));
  lines.push("IBR/DVB: " + (form?.sanVacunas === "si" ? "Al dia" : "Sin vacunar"));
  lines.push("Toros: " + (form?.sanToros === "con_control" ? "Con revision" : "Sin revision pre-servicio"));
  if (form?.torosCC) lines.push("CC toros: " + form.torosCC + " (objetivo 5.5 al servicio)");
  lines.push("");

  // Diagnóstico contra reglas biológicas
  const pvAdP = parseFloat(form?.pvVacaAdulta)||380;
  const ccParto = motor?.tray?.ccParto || 0;
  const ccServ2 = motor?.tray?.ccServ || 0;
  const anestro2 = motor?.tray?.anestro?.dias || 0;
  const v1gdp = motor?.vaq1E?.gdpReal || 0;
  const v1pvS = motor?.pvSalidaVaq1 || 0;
  const v2pvE = motor?.vaq2E?.pvEntore || 0;

  lines.push("=== DIAGNÓSTICO CONTRA REGLAS BIOLÓGICAS ===");
  lines.push("REGLAS FIJAS (siempre aplican):");
  lines.push("CC parto mínima: 5.5 | CC serv. mínima: 4.5 | Anestro máx: 55d");
  lines.push("Vaq 1°inv GDP mín: 400g/d | PV salida: " + Math.round(pvAdP*0.40) + "kg (40% PV adulto)");
  lines.push("Vaq 2°inv GDP mín: 300g/d | PV entore: " + Math.round(pvAdP*0.75) + "kg (75% PV adulto)");
  lines.push("V2S hiperprecoz si CC < 4.0 | Flushing vaq2: siempre 25d pre-servicio");
  lines.push("");
  lines.push("BRECHAS DETECTADAS:");
  if(ccParto < 5.5) lines.push("CC PARTO: " + ccParto.toFixed(2) + " < 5.5 → BRECHA " + (5.5-ccParto).toFixed(2) + " CC. Sin correccion el parto ya esta comprometido.");
  else lines.push("CC PARTO: " + ccParto.toFixed(2) + " ≥ 5.5 OK");
  if(ccServ2 < 4.5) lines.push("CC SERVICIO: " + ccServ2.toFixed(2) + " < 4.5 → BRECHA " + (4.5-ccServ2).toFixed(2) + " CC → preñez actual " + (motor?.tray?.pr||0) + "% vs meta 75%");
  else lines.push("CC SERVICIO: " + ccServ2.toFixed(2) + " ≥ 4.5 OK");
  if(anestro2 > 55) lines.push("ANESTRO: " + anestro2 + "d > 55d máximo → " + (anestro2-55) + "d de retraso en la ciclicidad");
  if(v1gdp < 400 && v1gdp > 0) lines.push("VAQ 1°INV GDP: " + v1gdp + "g/d < 400g/d mínimo → brecha " + (400-v1gdp) + "g/d → no llega al objetivo de PV. IRREVERSIBLE.");
  if(v1pvS > 0 && v1pvS < Math.round(pvAdP*0.40)) lines.push("VAQ 1°INV PV: " + v1pvS + "kg < " + Math.round(pvAdP*0.40) + "kg objetivo → déficit " + (Math.round(pvAdP*0.40)-v1pvS) + "kg");
  if(v2pvE > 0 && !motor?.vaq2E?.llegas) lines.push("VAQ 2°INV: " + v2pvE + "kg < " + Math.round(pvAdP*0.75) + "kg mínimo entore → NO LLEGA");
  lines.push("");

  lines.push("=== INSTRUCCIONES ===");
  lines.push("Con esta informacion, actua como tecnico ganadero experto. Tu analisis debe:");
  lines.push("1. IDENTIFICAR el cuello de botella principal — UNA causa raiz, no una lista");
  lines.push("2. EXPLICAR la cadena causal completa con los numeros reales del sistema");
  lines.push("3. CALCULAR que pasa con la prenez en cada escenario de correccion");
  lines.push("4. PRIORIZAR acciones por impacto Y momento del año — que hacer en los proximos 30, 60 y 90 dias");
  lines.push("5. CUANTIFICAR el resultado: prenez esperada, kg ternero adicionales, fechas");
  lines.push("NO menciones insumos especificos a menos que esten cargados en el sistema.");
  lines.push("NO expliques conceptos basicos. SI cita bibliografia cuando justifica un numero concreto.");
  lines.push("Formato: secciones con titulo claro, parrafos cortos, numeros en negrita cuando son criticos.");
