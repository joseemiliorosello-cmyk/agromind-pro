...
import { recomendarSuplemento, calcFaseCiclo, calcImpactoCola, getBiotipo, interpCC, calcPenalidadSanidad, getClima } from "./motor";
...
ESTRUCTURA TU RESPUESTA EN ESTOS 4 BLOQUES:
...
function buildPromptFull(motor, form, sat, cerebro_data) {
  ...
  if (form?.aguaTDS) lines.push("Agua: TDS " + form.aguaTDS + "mg/L | impacto DMI: -" + Math.round(motor?.evalAgua?.pctReducDMI||0) + "%");
  lines.push("");

  lines.push("SUPLEMENTACION:");
  const supls = [["Vacas",form?.supl_vacas,form?.dosis_vacas],["V2S",form?.supl_v2s,form?.dosis_v2s],["Toros",form?.supl_toros,form?.dosis_toros],["Vaq1",form?.supl_vaq1,form?.dosis_vaq1],["Vaq2",form?.supl_vaq2,form?.dosis_vaq2],["Invernada",form?.supl1,form?.dosis1]].filter(([,s,d])=>s&&parseFloat(d)>0);
  if (supls.length > 0) supls.forEach(([c,s,d]) => lines.push("  " + c + ": " + s + " " + d + "kg/d"));
  else lines.push("  Sin suplementacion cargada");
  if (form?.tieneVerdeo==="si") lines.push("  Verdeo: " + (form?.verdeoTipo||"ND") + " " + (form?.verdeoHa||"ND") + "ha -> " + (form?.verdeoDestinoVaq==="si"?"vaquillona":"rodeo"));
  lines.push("");
  ...
  bm.forEach(m => {
    ...
    lines.push(MC[m.i]+": "+(bal>=0?"+":"")+bal+" | p:"+Math.round(m.ofPastoTotal||0)+" cc:"+Math.round(m.ccAporte||0)+" s:"+Math.round(m.suplAporte||0)+" dem:"+Math.round(m.demanda||0)+" "+inv+" "+hito);
  });
  lines.push("");
  if (defInv.length > 0) {
    lines.push("DEFICIT JUN-AGO: " + defInv.map(m=>MC[m.i]+" "+Math.round(m.balance)).join(" | "));
    lines.push("Acumulado: " + Math.round(defAcum) + " Mcal = " + (nVacas>0?(defAcum/nVacas/5.6).toFixed(1):"ND") + " CC movilizadas/vaca sin correccion");
  ...
  if (anestro>0) lines.push("Anestro: " + anestro + "d" + (anestro>55?" BRECHA "+(anestro-55)+"d":" OK"));
  if (v1gdp>0) lines.push("Vaq1 GDP: " + v1gdp + "g/d" + (v1gdp<400?" BRECHA "+(400-v1gdp)+"g/d IRREVERSIBLE":" OK"));
  if (v1pvSal>0) lines.push("Vaq1 PV: " + v1pvSal + "kg / obj " + v1obj + "kg" + (v1pvSal<v1obj?" BRECHA "+(v1obj-v1pvSal)+"kg":" OK"));
  if (v2pvEnt>0) lines.push("Vaq2 PV entore: " + v2pvEnt + "kg / min " + v2obj + "kg " + (v2ok?"OK":"NO LLEGA") + " | Flushing 25d: siempre");
  lines.push("");

  if (esc.length > 0) {
    lines.push("=== PASO 3: ESCENARIOS ===");
    esc.forEach(e => {
      const b = esc[0]?.prenez||0;
      lines.push(e.label + ": CC " + e.ccServ + " => " + e.prenez + "%" + (e.id!=="actual"?" (+"+(e.prenez-b)+"pp)":""));
    });
    const mej = esc.slice(1).reduce((a,b)=>b.prenez>a.prenez?b:a,esc[1]||{prenez:0});
    const gan = (mej?.prenez||0)-(esc[0]?.prenez||0);
    if (gan>0&&nVacas>0) lines.push("Potencial: +" + gan + "pp = +" + Math.round(nVacas*gan/100*.95) + " terneros");
    lines.push("");
  }
  ...
  if (motor?.vaq2E) {
    lines.push("VAQ 2 INV: entrada mayo " + (motor.pvEntradaVaq2||"ND") + "kg | entore " + v2pvEnt + "kg / min " + v2obj + "kg " + (v2ok?"OK":"NO LLEGA") + " | FLUSHING 25d: siempre sin excepcion");
  }
  lines.push("");

  lines.push("SANIDAD: Aftosa " + (form?.sanAftosa==="si"?"OK":"SIN") + " | Brucelosis " + (form?.sanBrucelosis==="si"?"OK":"SIN") + " | Toros " + (form?.sanToros==="con_control"?"OK":"SIN revision") + (form?.torosCC?" | CC toros "+form.torosCC:""));
  lines.push("");

  lines.push("=== INSTRUCCION ===");
  lines.push("Estructura tu respuesta en 4 bloques: 1.DIAGNOSTICO INTEGRADO 2.PUNTOS CRITICOS POR IMPACTO 3.ESCENARIOS CON CORRECCIONES 4.PLAN DE ACCION CON FECHAS");
  lines.push("Prioridad: destete antes que suplemento. V2S categoria separada. Vaq1 irreversible si no llega.");
  lines.push("Sin insumos especificos salvo que esten en los datos. Parrafos directos sin listas.");
