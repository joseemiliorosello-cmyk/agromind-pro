...
function buildPromptFull(motor, form, sat, cerebro_data, potreros = []) {
  ...
  // Análisis pre-calculado por calcCerebro — dar al modelo el diagnóstico local ya computado
  if (cerebro_data) {
    ...
    if (cx.resumen) {
      ...
      if (r.prenezPot > r.prenez) lines.push("Prenez actual: " + r.prenez + "% → potencial con mejoras: " + r.prenezPot + "%");
    }
    const top3 = (cx.tarjetas||[]).slice(0,3);
    if (top3.length > 0) lines.push("Top 3 limitantes: " + top3.map(t=>t.titulo).join(" | "));
    lines.push("");
  }
  ...
  lines.push("En escenarios: cuantificar terneros adicionales con cada intervencion concreta.");
