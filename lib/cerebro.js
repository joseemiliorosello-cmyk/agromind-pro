...
const SYS_FULL = `Sos el cerebro analítico de Calf AI — un sistema de diagnóstico ganadero para cría bovina extensiva en NEA, NOA, Paraguay y Brasil.

TU FORMA DE RAZONAR — seguí siempre este orden:

1. IDENTIFICAR EL CUELLO DE BOTELLA REAL
   No listés síntomas. Encontrá la causa raíz. El sistema ya calculó todos los números — tu trabajo es encontrar cuál variable limita más el resultado final (preñez, kg ternero, CC al servicio).
   Preguntate: ¿si corrigiese solo UNA cosa, cuál tendría el mayor impacto?

2. CADENA CAUSAL — conectar todo
   El ganadero necesita entender POR QUÉ. Siempre explicá: 
   [causa] → [efecto en energía] → [efecto en CC] → [efecto en preñez] → [kg ternero]
   Ejemplo: "Pasto invernal PB <5% → microflora ruminal inactiva → digestibilidad cae 20% → vaca no puede cubrir mantenimiento → moviliza CC → CC al servicio 3.2 → anestro prolongado → preñez 36%"

3. MOMENTO DEL AÑO — es crítico
   El mismo problema tiene soluciones distintas según el momento. Siempre contextualizá:
   - ¿Cuántos meses faltan para el servicio?
   - ¿La ventana de acción todavía está abierta?
   - ¿Qué pasa si no se actúa en los próximos 30 días?

4. CUANTIFICAR LAS CORRECCIONES
   Para cada recomendación, calculá con los datos del sistema:
   - Qué cambia en el balance (Mcal/día)
   - Cuántos puntos de preñez gana
   - Cuántos terneros adicionales representa
   - En qué fecha hay que actuar

5. PRIORIZAR POR IMPACTO Y URGENCIA
   No todas las correcciones son iguales. Ordená:
   - URGENTE: sin esto el ciclo ya está comprometido
   - IMPORTANTE: impacto alto, ventana de 30-60 días
   - RECOMENDADO: mejora marginal, puede esperar

REGLAS DE ESCRITURA:
- Párrafos cortos, directos, con números reales del sistema
- Nunca mencionar insumos específicos (expeller, maíz) a menos que el usuario los haya cargado — si hay déficit, decí "suplemento proteico" o "suplemento energético"
- Citar bibliografía solo cuando justifica un número concreto
- El técnico ya sabe ganadería — no explicar conceptos básicos
- Estructurar en secciones claras con títulos

BIBLIOGRAFÍA DE REFERENCIA (usar cuando el número lo justifica):
Peruchena INTA 2003 · Selk 1988 · Short et al. 1990 · Wiltbank 1990 · Detmann/NASSEM 2010 · NRC 2000 · Balbuena INTA 2003 · López et al. 2021 · Bavera 2005 · Rosello Brajovich et al. 2025
``;
...
function buildPromptFull(motor, form, sat, cerebro_data) {
  ...
  // ── 1. CONTEXTO: dónde está el sistema HOY ──────────────────────
  const mesHoyNom = MESES[mesHoy];
  const diasAlServicio = motor?.cadena?.ini
    ? Math.max(0, Math.round((new Date(motor.cadena.ini) - hoy) / 86400000))
    : null;
  const ventana = diasAlServicio !== null
    ? diasAlServicio < 30  ? "CRÍTICA — menos de 30 días al servicio"
    : diasAlServicio < 60  ? "URGENTE — menos de 60 días al servicio"
    : diasAlServicio < 120 ? "IMPORTANTE — menos de 4 meses al servicio"
    : "AMPLIA — más de 4 meses para actuar"
    : "sin fechas de servicio";

  // ── 2. BALANCE: cuál es el déficit real ────────────────────────
  const defInv = [5,6,7].map(i => bm[i]).filter(m => m?.balance < 0);
  const peorDef = defInv.length > 0
    ? Math.min(...defInv.map(m => m.balance))
    : 0;
  ...
  // ── 3. OFERTA: de dónde viene la energía ───────────────────────
  const ofertaActual = bm.map(m => ({
    mes: MESES_C[m.i],
    pasto: m.ofPastoTotal || 0,
    cc: m.ccAporte || 0,
    supl: m.suplAporte || 0,
    verdeo: m.verdeoAporte || 0,
    demanda: m.demanda || 0,
    balance: m.balance || 0,
  }));

  // ── 4. ESCENARIOS: qué pasa si se corrige ──────────────────────
  ...
  const gananciaMax = escMejor?.prenez && escActual?.prenez
    ? escMejor.prenez - escActual.prenez : 0;
  const ternerosExtra = nVacas > 0 && gananciaMax > 0
    ? Math.round(nVacas * gananciaMax / 100) : 0;

  // ── CONSTRUCCIÓN DEL PROMPT ─────────────────────────────────────
  let prompt = `=== SISTEMA A ANALIZAR ===

ESTABLECIMIENTO: ${form?.nombreProductor || "sin nombre"}
UBICACIÓN: ${[form?.localidad, form?.provincia, form?.pais].filter(Boolean).join(", ") || "sin especificar"}
BIOTIPO: ${form?.biotipo || "sin especificar"}
FECHA: ${mesHoyNom} ${hoy.getFullYear()} | VENTANA DE ACCIÓN: ${ventana}
${diasAlServicio !== null ? `DÍAS AL SERVICIO: ${diasAlServicio}d (inicio ${motor?.cadena?.ini ? new Date(motor.cadena.ini).toLocaleDateString("es-AR",{month:"short",year:"numeric"}) : "?"})` : ""}

=== ESTADO DEL SISTEMA ===
Vacas: ${form?.vacasN || "ND"} cab | PV adulto: ${form?.pvVacaAdulta || "ND"} kg | Biotipo: ${form?.biotipo || "ND"}
CC ponderada HOY: ${motor?.ccPondVal?.toFixed(1) || "ND"} (escala 1-9 INTA)
CC proyectada al SERVICIO: ${tray?.ccServ || "ND"} → preñez estimada: ${tray?.pr || "ND"}%
Anestro posparto: ${tray?.anestro?.dias || "ND"} días
${sat?.ndvi ? `NDVI hoy: ${sat.ndvi} (${sat.condForr}) | Temp: ${sat.temp}°C | Lluvia 30d: ${sat.p30}mm` : "Sin datos satelitales"}
${form?.aguaTDS ? `Agua: TDS ${form.aguaTDS} mg/L (${form?.aguaTipoSal || "tipo desconocido"})` : ""}

=== BALANCE FORRAJERO MES A MES ===
Rodeo completo (Mcal/día) | + = superávit · - = déficit
${ofertaActual.map(m =>
  `${m.mes.padEnd(4)}: bal ${(m.balance > 0 ? "+" : "") + Math.round(m.balance).toString().padStart(6)} | pasto ${Math.round(m.pasto).toString().padStart(5)} + supl ${Math.round(m.supl).toString().padStart(4)} + cc ${Math.round(m.cc).toString().padStart(4)} vs dem ${Math.round(m.demanda).toString().padStart(5)}`
).join("
")}

DÉFICIT INVERNAL (Jun-Jul-Ago):
${defInv.length === 0
  ? "Sin déficit — balance cubierto en invierno ✓"
  : defInv.map(m => `  ${MESES_C[m.i]}: ${Math.round(m.balance)} Mcal/día rodeo`).join("
")}
${defAcum > 0 ? `  Déficit acumulado: ${Math.round(defAcum)} Mcal totales = ${defPorVaca} Mcal/vaca en esos meses` : ""}
${defAcum > 0 && nVacas > 0 ? `  Equivale a ~${(defAcum / nVacas / 5.6).toFixed(1)} unidades CC movilizadas por vaca sin corrección` : ""}

=== OFERTA — DE DÓNDE VIENE LA ENERGÍA ===
Vegetación: ${form?.vegetacion || form?.potreros?.[0]?.veg || "Pastizal natural"}
Fenología actual: ${form?.fenologia || "no especificada"}
${motor?.disponMS ? `Disponibilidad pasto: ${motor.disponMS.msHa} kgMS/ha (${motor.disponMS.nivel})` : ""}
${motor?.verdeoAporteMcalMes > 0 ? `Verdeo activo: ${motor.verdeoAporteMcalMes} Mcal/día (${form?.verdeoTipo || ""})` : ""}
${motor?.suplRodeoMcalDia > 0 ? `Suplementación cargada: ${Math.round(motor.suplRodeoMcalDia)} Mcal/día en meses ${(form?.suplMeses || []).map(m => MESES_C[m]).join("-")}` : "Sin suplementación cargada"}
${motor?.factorAgua < 1 ? `⚠ Agua salobre reduce consumo de pasto -${Math.round((1-motor.factorAgua)*100)}%` : ""}

=== CADENA REPRODUCTIVA ===
${form?.iniServ && form?.finServ ? `Servicio: ${new Date(form.iniServ+"T12:00").toLocaleDateString("es-AR",{month:"short",year:"numeric"})} → ${new Date(form.finServ+"T12:00").toLocaleDateString("es-AR",{month:"short",year:"numeric"})} (${motor?.cadena?.diasServ || "?"}d)` : "Sin fechas de servicio"}
${motor?.cadena?.partoTemp ? `Parto temprano estimado: ${new Date(motor.cadena.partoTemp).toLocaleDateString("es-AR",{month:"short",year:"numeric"})}` : ""}
CC hoy → parto: ${tray?.ccParto || "ND"} | mín lactación: ${tray?.ccMinLact || "ND"} | al servicio: ${tray?.ccServ || "ND"}
Meses lactación: ${tray?.mesesLact || "ND"} | Caída CC: -${tray?.caidaLact || "ND"}
Destete: ${((form?.destTrad||0) > 0 ? `Trad ${form.destTrad}%` : "")} ${((form?.destAntic||0) > 0 ? `Antic ${form.destAntic}%` : "")} ${((form?.destHiper||0) > 0 ? `Hiperprecoz ${form.destHiper}%` : "")}

=== ESCENARIOS DE MEJORA ===
${esc.map(e => `${e.label}: CC serv ${e.ccServ} → preñez ${e.prenez}% ${e.id !== "actual" ? `(+${e.prenez - (esc[0]?.prenez||0)}pp)` : "(base)"}`).join("
")}
${gananciaMax > 0 ? `
Mejora máxima alcanzable: +${gananciaMax}pp preñez = +${ternerosExtra} terneros en ${nVacas} vacas` : ""}

=== VAQUILLONA ===
${motor?.vaq1E && !motor.vaq1E.mensaje ? `1° inv: PV entrada ${motor.pvEntVaq1 || "ND"} kg | GDP pasto solo: ${motor.vaq1E.gdpPasto} g/d | ${motor.vaq1E.gdpReal < 400 ? "⚠ GDP insuficiente para objetivo" : "GDP adecuado"}` : ""}
${motor?.vaq2E ? `2° inv: ${motor.vaq2E.llegas ? "✓ Llega al entore" : `⚠ Déficit ${(motor.vaq2E.pvMinEntore||0)-(motor.vaq2E.pvEntore||0)} kg para entore`}` : ""}

=== SANIDAD ===
Aftosa: ${form?.sanAftosa === "si" ? "Al día ✓" : "⚠ SIN VACUNAR"}
Brucelosis: ${form?.sanBrucelosis === "si" ? "Al día ✓" : "⚠ SIN VACUNAR"}
IBR/DVB: ${form?.sanVacunas === "si" ? "Al día ✓" : "⚠ Sin vacunar"}
Toros: ${form?.sanToros === "con_control" ? "Con revisión ✓" : "⚠ Sin revisión pre-servicio"}
${form?.torosCC ? `CC toros: ${form.torosCC} (objetivo ≥5.5 al servicio)` : ""}

=== INSTRUCCIONES ===

Con toda esta información, actuá como técnico ganadero experto. Tu análisis debe:

1. IDENTIFICAR EL CUELLO DE BOTELLA PRINCIPAL del sistema — UNA causa raíz, no una lista
2. EXPLICAR LA CADENA CAUSAL completa con los números reales del sistema
3. CALCULAR qué pasa con la preñez en cada escenario de corrección
4. PRIORIZAR las acciones por impacto Y por momento del año — qué hay que hacer en los próximos 30, 60 y 90 días
5. CUANTIFICAR el resultado: preñez esperada, kg ternero adicionales, meses de implementación

NO menciones insumos específicos (expeller, maíz, etc.) a menos que estén cargados en el sistema.
NO expliques conceptos básicos.
SÍ citá bibliografía cuando justifica un número concreto.

Formato: secciones con título claro, párrafos cortos, números en negrita cuando son críticos.`;
