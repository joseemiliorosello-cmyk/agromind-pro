...
Recibs datos en 4 secciones: datos cargados, balance y brechas, escenarios con mejoras, sanidad.
...
ESTRUCTURA TU RESPUESTA EN ESTOS 4 BLOQUES SIEMPRE:

1. DIAGNOSTICO INTEGRADO
...
2. PUNTOS CRITICOS POR IMPACTO
Cuantos pp de prenez afecta cada uno. Cuando actuar. Destete primero siempre.

3. ESCENARIOS CON CORRECCIONES
Balance con mejoras aplicadas. Prenez actual -> prenez con correccion -> terneros adicionales.

4. PLAN DE ACCION CON FECHAS
Que, cuando exactamente, en que orden. Sin insumos especificos salvo que esten en los datos.

Citar solo cuando justifica un numero: Peruchena 2003, Selk 1988, Short 1990, Wiltbank 1990, NRC 2000, Detmann/NASSEM 2010, Balbuena INTA 2003.
...
function buildPromptFull(motor, form, sat, cerebro_data) {
  ...
  const ventana = diasAlServ === null ? "sin fechas" :
    diasAlServ < 30 ? "CRITICA — menos de 30 dias" :
    diasAlServ < 60 ? "URGENTE — menos de 60 dias" :
    diasAlServ < 120 ? "IMPORTANTE — menos de 4 meses" :
    "AMPLIA — mas de 4 meses";

  // ── Balance ────────────────────────────────────────────
  const defInv = [5,6,7].map(i => bm[i]).filter(m => m?.balance < 0);
  const defAcum = defInv.reduce((s,m) => s + Math.abs(m?.balance||0), 0);
  const peorMes = defInv.length > 0 ? defInv.reduce((a,b) => a.balance < b.balance ? a : b) : null;

  // ── Reglas biológicas — brechas calculadas ─────────────
  const ccParto = tray?.ccParto || 0;
  const ccServ  = tray?.ccServ  || 0;
  const prenez  = tray?.pr      || 0;
  const anestro = tray?.anestro?.dias || 0;
  ...
  // ══════════════════════════════════════════════════════
  lines.push("=== PASO 1: DATOS DEL ESTABLECIMIENTO ===");
  lines.push("Establecimiento: " + (form?.nombreProductor || "sin nombre"));
  lines.push("Ubicacion: " + [form?.localidad, form?.provincia, form?.pais].filter(Boolean).join(", "));
  lines.push("Biotipo: " + (form?.biotipo || "ND") + " | Vacas: " + nVacas + " | V2S: " + (motor?.nV2s||0) + " | PV adulto: " + pvAdulto + "kg");
  lines.push("Superficie: " + (form?.supHa || "ND") + "ha | Carga: " + (motor?.cargaEV_ha?.toFixed(2) || "ND") + " EV/ha");
  lines.push("");

  lines.push("REPRODUCTIVO:");
  lines.push("Servicio: " + (form?.iniServ||"sin fecha") + " a " + (form?.finServ||"sin fecha") + " (" + (motor?.cadena?.diasServ||"?") + "d)");
  lines.push("Dias al servicio: " + (diasAlServ !== null ? diasAlServ+"d" : "sin dato") + " | Ventana: " + ventana);
  lines.push("Destete: Trad " + (form?.destTrad||0) + "% | Anticipado " + (form?.destAntic||0) + "% | Hiperprecoz " + (form?.destHiper||0) + "%");
  if (form?.ccAnterior) lines.push("CC tacto anterior: " + form.ccAnterior + " (fecha: " + (form.fechaCCAnterior||"ND") + ")");
  lines.push("");

  lines.push("CC RODEO HOY (distribucion por grupo):");
  const dist = form?.distribucionCC || [];
  if (dist.length > 0 && dist.some(d => parseFloat(d.pct) > 0)) {
    dist.filter(d => parseFloat(d.pct) > 0).forEach(d =>
      lines.push("  CC " + d.cc + ": " + d.pct + "%")
    );
    lines.push("CC ponderada: " + (motor?.ccPondVal?.toFixed(2) || "0.00"));
  } else {
  ...
  lines.push("Vegetacion: " + (form?.vegetacion || "ND") + " | Fenologia: " + (form?.fenologia || "ND"));
  if (sat?.ndvi) {
    lines.push("NDVI: " + sat.ndvi + " (" + (sat.condForr||"ND") + ") | Temp: " + sat.temp + "C | Lluvia 30d: " + (sat.p30||"ND") + "mm");
    if (sat.p30 < 30) lines.push("ALERTA: precipitacion muy baja — campo bajo estres hidrico");
  } else {
    lines.push("NDVI: sin datos satelitales — usando historial climatico de la zona");
  }
  lines.push("ENSO: " + (form?.enso||"neutro") + " | Pronostico: " + (sat?.pronoResumen||"sin dato"));
  lines.push("");

  lines.push("SUPLEMENTACION POR CATEGORIA:");
  const supls = [
    ["Vacas",  form?.supl_vacas,  form?.dosis_vacas],
    ["V2S",    form?.supl_v2s,    form?.dosis_v2s],
    ["Toros",  form?.supl_toros,  form?.dosis_toros],
    ["Vaq 1°", form?.supl_vaq1,   form?.dosis_vaq1],
    ["Vaq 2°", form?.supl_vaq2,   form?.dosis_vaq2],
    ["Invernada", form?.supl1,    form?.dosis1],
  ].filter(([,s,d]) => s && parseFloat(d) > 0);
  if (supls.length > 0) {
    supls.forEach(([cat,s,d]) => lines.push("  " + cat + ": " + s + " " + d + "kg/dia"));
    lines.push("  Meses activos: " + (form?.suplMeses || []).map(m => MC[m]).join("-"));
  } else {
    lines.push("  Sin suplementacion cargada");
  }
  if (form?.tieneVerdeo === "si") {
    lines.push("Verdeo: " + (form?.verdeoTipo||"ND") + " " + (form?.verdeoHa||"ND") + "ha — destinado a: " + (form?.verdeoDestinoVaq==="si"?"vaquillona prioritaria":"rodeo general"));
  }
  if (form?.aguaTDS) lines.push("Agua: TDS " + form.aguaTDS + "mg/L (" + (form.aguaTipoSal||"ND") + ") — impacto DMI: -" + Math.round((motor?.evalAgua?.pctReducDMI||0)) + "%");
  lines.push("");

  // ══════════════════════════════════════════════════════
  // PASO 2 — ANÁLISIS: balance y brechas
  // ══════════════════════════════════════════════════════
  lines.push("=== PASO 2: ANALISIS — BALANCE Y BRECHAS ===");
  lines.push("BALANCE MES A MES (Mcal/dia rodeo completo):");
  bm.forEach(m => {
    ...
    lines.push("  " + MC[m.i] + ": " + sig + bal + " | pasto " + Math.round(m.ofPastoTotal||0) + " cc " + Math.round(m.ccAporte||0) + " supl " + Math.round(m.suplAporte||0) + " vs dem " + Math.round(m.demanda||0) + inv + hito);
  });
  lines.push("");
  if (defInv.length > 0) {
    lines.push("DEFICIT INVERNAL:");
    defInv.forEach(m => lines.push("  " + MC[m.i] + ": " + Math.round(m.balance) + " Mcal/dia"));
    lines.push("  Acumulado: " + Math.round(defAcum) + " Mcal totales Jun-Ago");
    lines.push("  Por vaca: " + (nVacas > 0 ? (defAcum/nVacas).toFixed(1) : "ND") + " Mcal en esos meses sin correccion");
    lines.push("  Equivale a: " + (nVacas > 0 ? (defAcum/nVacas/5.6).toFixed(1) : "ND") + " CC movilizadas por vaca");
  } else {
    lines.push("BALANCE INVERNAL: sin deficit — OK");
  }
  lines.push("");

  lines.push("BRECHAS CONTRA REGLAS BIOLOGICAS:");
  lines.push("Regla: CC parto minima 5.5 | CC serv minima 4.5 | Anestro maximo 55d");
  lines.push("Regla: Vaq1 GDP minimo 400g/d | PV salida: " + v1obj + "kg (40% PV adulto)");
  lines.push("Regla: Vaq2 PV entore: " + v2obj + "kg (75% PV adulto) | Flushing 25d pre-serv: siempre");
  lines.push("");

  if (ccParto > 0) {
    const bParto = (5.5 - ccParto).toFixed(2);
    lines.push("CC PARTO: " + ccParto.toFixed(2) + (ccParto < 5.5 ? " < 5.5 BRECHA " + bParto + " — sin correccion el parto ya esta comprometido" : " >= 5.5 OK"));
  }
  if (ccServ > 0) {
    const bServ = (4.5 - ccServ).toFixed(2);
    lines.push("CC SERVICIO: " + ccServ.toFixed(2) + (ccServ < 4.5 ? " < 4.5 BRECHA " + bServ + " => prenez " + prenez + "% vs meta 75%" : " >= 4.5 OK => prenez " + prenez + "%"));
  }
  if (anestro > 0) lines.push("ANESTRO: " + anestro + "d" + (anestro > 55 ? " > 55d BRECHA " + (anestro-55) + "d de retraso en ciclicidad" : " <= 55d OK"));
  if (v1gdp > 0) lines.push("VAQ 1°INV GDP: " + v1gdp + "g/d" + (v1gdp < 400 ? " < 400g/d BRECHA " + (400-v1gdp) + "g/d — IRREVERSIBLE si no se corrige" : " >= 400g/d OK"));
  if (v1pvSal > 0) lines.push("VAQ 1°INV PV: " + v1pvSal + "kg" + (v1pvSal < v1obj ? " < " + v1obj + "kg BRECHA " + (v1obj-v1pvSal) + "kg" : " >= " + v1obj + "kg OK"));
  if (v2pvEnt > 0) lines.push("VAQ 2°INV: " + v2pvEnt + "kg al entore" + (!v2ok ? " < " + v2obj + "kg NO LLEGA" : " >= " + v2obj + "kg OK") + " — flushing 25d pre-serv: siempre");
  lines.push("");

  // ══════════════════════════════════════════════════════
  // PASO 3 — ESCENARIOS CON MEJORAS
  // ══════════════════════════════════════════════════════
  if (esc.length > 0) {
    lines.push("=== PASO 3: ESCENARIOS CON MEJORAS APLICADAS ===");
    esc.forEach(e => {
    ...
    const mejorEsc = esc.slice(1).reduce((a,b) => b.prenez > a.prenez ? b : a, esc[1]||{prenez:0});
    const ganancia = (mejorEsc?.prenez||0) - (esc[0]?.prenez||0);
    if (ganancia > 0 && nVacas > 0) {
      lines.push("POTENCIAL: +" + ganancia + "pp = +" + Math.round(nVacas*ganancia/100*0.95) + " terneros en " + nVacas + " vacas");
    }
    lines.push("");
  }

  if (motor?.vaq1E && !motor.vaq1E.mensaje) {
    lines.push("=== VAQ 1°INV ===");
    lines.push("PV entrada: " + (motor.pvEntVaq1||"ND") + "kg | GDP pasto: " + (motor.vaq1E.gdpPasto||0) + "g/d | Con supl: " + (motor.vaq1E.gdpReal||0) + "g/d");
    lines.push("PV salida: " + (v1pvSal) + "kg | Objetivo: " + v1obj + "kg | " + (v1pvSal >= v1obj ? "OK" : "DEFICIT " + (v1obj-v1pvSal) + "kg — IRREVERSIBLE"));
    lines.push("Si no llega este invierno NO lo recupera en primavera — Balbuena INTA 2003");
    lines.push("");
  }
  if (motor?.vaq2E) {
    lines.push("=== VAQ 2°INV ===");
    lines.push("PV entrada mayo: " + (motor.pvEntradaVaq2||"ND") + "kg | PV entore: " + v2pvEnt + "kg | Minimo: " + v2obj + "kg | " + (v2ok?"OK":"NO LLEGA"));
    lines.push("FLUSHING: siempre 25 dias pre-servicio — aumenta tasa ovulatoria (nunca omitir)");
    lines.push("Control de peso obligatorio en abril-mayo ANTES del invierno");
    lines.push("");
  }

  lines.push("=== SANIDAD ===");
  lines.push("Aftosa: " + (form?.sanAftosa==="si"?"OK":"SIN VACUNAR"));
  lines.push("Brucelosis: " + (form?.sanBrucelosis==="si"?"OK":"SIN VACUNAR"));
  ...
  lines.push("Con toda esta informacion, actua como tecnico ganadero experto.");
  lines.push("ESTRUCTURA TU RESPUESTA EN EXACTAMENTE ESTOS 4 BLOQUES:");
  lines.push("");
  lines.push("1. DIAGNOSTICO INTEGRADO");
  lines.push("   Identifica UNA causa raiz — la variable que mas limita la prenez.");
  lines.push("   Cadena causal completa con numeros reales: [clima/NDVI] -> [oferta] -> [balance] -> [CC] -> [anestro] -> [prenez]");
  lines.push("   Por que no se puede llegar al objetivo sin correccion.");
  lines.push("");
  lines.push("2. PUNTOS CRITICOS ORDENADOS POR IMPACTO");
  lines.push("   Para cada uno: que es, cuanto impacta en pp de prenez, cuando actuar.");
  lines.push("   Prioridad: destete ANTES que cualquier suplemento a la vaca adulta.");
  lines.push("   V2S como categoria separada y mas critica que las adultas.");
  lines.push("   Vaquillona 1°inv: si no llega al objetivo es irreversible — decirlo.");
  lines.push("");
  lines.push("3. ESCENARIOS CON CORRECCIONES");
  lines.push("   Mostrar que pasa con el balance y la prenez en cada escenario.");
  lines.push("   Cuantificar: prenez actual -> prenez con correccion -> terneros adicionales.");
  lines.push("");
  lines.push("4. PLAN DE ACCION CON FECHAS");
  lines.push("   Que hacer, cuando exactamente, en que orden.");
  lines.push("   Sin insumos especificos salvo que esten en los datos cargados.");
  lines.push("   Citar bibliografia solo cuando justifica un numero: Peruchena 2003, Selk 1988, Short 1990, Wiltbank 1990, NRC 2000, Detmann/NASSEM 2010, Balbuena INTA 2003.");
  lines.push("");
  lines.push("Escribi en parrafos cortos y directos. Sin listas. El tecnico ya sabe ganaderia.");
