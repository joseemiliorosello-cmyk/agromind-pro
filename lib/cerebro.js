...
function interpretarVaquillona(motor, form) {
  ...
  const pvSalV1  = motor?.pvSalidaVaq1 || vaq1E?.pvSal || 0;
  const pvEntV1  = motor?.pvEntVaq1 || Math.round(pvAdulto * 0.40);

  // ── VAQ1 — 1° invierno ─────────────────────────────────────────
  let vaq1 = null;
  if (nVaq1 > 0 && vaq1E) {
    const gdpReal   = vaq1E.gdpReal   || 0;
    const gdpRecom  = vaq1E.gdpRecom  || 300;
    const GDP_MIN   = 400;
    const pvObj     = Math.round(pvAdulto * 0.65);
    const llega     = pvSalV1 >= pvObj;
    const brechaGDP = Math.max(0, GDP_MIN - gdpReal);
    const brechaPV  = Math.max(0, pvObj - pvSalV1);

    let sugerencia = "";
    if (!llega || gdpReal < GDP_MIN) {
      sugerencia = brechaGDP > 150
        ? "Suplemento [EP] 1–1.5 kg/cab/día · diario · 90 días — sin esto no llega al entore."
        : "Suplemento [P] 0.4–0.6 kg expeller girasol/cab/día · 3×/sem · 90 días.";
    }

    let conclusion = `Vaq1 (${nVaq1} cab) GDP ${gdpReal} g/d`;
    if (gdpReal < GDP_MIN || !llega) {
      conclusion += ` — bajo mínimo (${GDP_MIN} g/d). PV salida ${pvSalV1} kg vs obj ${pvObj} kg. `;
      conclusion += !llega
        ? `Brecha ${brechaPV} kg. Ventana IRREVERSIBLE (Balbuena INTA 2003). ${sugerencia}`
        : `Llega por margen ajustado — monitorear. ${sugerencia}`;
    } else {
      conclusion += ` — en condición. PV salida ${pvSalV1} kg / obj ${pvObj} kg.`;
      if (pvSalV1 >= Math.round(pvAdulto * 0.65))
        conclusion += " Evaluar entore a 24 meses para adelantar producción.";
    }

    vaq1 = { n:nVaq1, pvEntrada:pvEntV1, pvSalida:pvSalV1, pvObjetivo:pvObj,
             gdpReal, gdpRecom, gdpMin:GDP_MIN, llega, brechaGDP, brechaPV, sugerencia, conclusion };
  }

  // ── VAQ2 — 2° invierno / entore ────────────────────────────────
  let vaq2 = null;
  if (nVaq2 > 0 && vaq2E) {
    const pvEntore  = vaq2E.pvEntore    || 0;
    const pvMinEnt  = vaq2E.pvMinEntore || Math.round(pvAdulto * 0.75);
    const llega2    = vaq2E.llegas ?? (pvEntore >= pvMinEnt);
    const brecha2   = Math.max(0, pvMinEnt - pvEntore);
    const pct       = pvAdulto > 0 ? Math.round(pvEntore / pvAdulto * 100) : 0;
    const gdpInv2   = vaq2E.gdpInv || 0;
    const flush     = "Flushing 25d pre-servicio: 0.5 kg maíz/sorgo/cab/día.";

    let conclusion2 = `Vaq2 (${nVaq2} cab) PV entore ${pvEntore} kg`;
    if (!llega2) {
      conclusion2 += ` — faltan ${brecha2} kg (mín ${pvMinEnt} kg = 75% PV adulto). Supl invernal [EP] + ${flush}`;
    } else {
      conclusion2 += ` — llega (${pct}% PV adulto). `;
      conclusion2 += gdpInv2 < 400 ? `GDP bajo (${gdpInv2} g/d) — ${flush}` : `GDP adecuado. ${flush}`;
    }
    if (vaq2E.aptaEntoreAntic)
      conclusion2 += " Peso agosto ≥65% PV adulto — evaluar entore anticipado.";

    vaq2 = { n:nVaq2, pvEntore, pvMinEntore:pvMinEnt, llega:llega2, brecha:brecha2, pct,
             gdpInv:gdpInv2, aptaEntoreAntic:vaq2E.aptaEntoreAntic||false,
             sugerenciaFlush:flush, conclusion:conclusion2 };
  }

  const hayVaq1 = !!vaq1;
  const hayVaq2 = !!vaq2;
  let conclusionGeneral = "Sin datos de reposición.";
  if (hayVaq1 && vaq1 && !vaq1.llega)
    conclusionGeneral = "CRÍTICO: vaq1 no llega al entore — actuar este invierno.";
  else if (hayVaq2 && vaq2 && !vaq2.llega)
    conclusionGeneral = "Vaq2 con brecha de peso — suplementar + flushing pre-servicio.";
  else if (hayVaq1 || hayVaq2)
    conclusionGeneral = "Reposición dentro de parámetros. Monitorear pesadas mensuales.";
