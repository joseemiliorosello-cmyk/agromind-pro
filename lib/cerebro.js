...
function interpretarAmplitudParicion(form, motor, tray) {
  ...
  // Techo = CC al parto (no puede recuperar por encima de donde arrancó). Antes se
  // topaba al promedio del rodeo (tray.ccServ), lo que dejaba a la cabeza —la mejor
  // vaca— por debajo del promedio, sin diferencia con la cola.
  const techoCC = tray?.ccParto || 6.0;
  const ccServCabeza = diasRecupCabeza !== null
    ? Math.min(techoCC, +(ccMinLact + diasRecupCabeza * tasaRecup).toFixed(2))
    : null;
