...
function buildPromptFull(motor, form, sat, cerebro_data) {
  ...
  const est = form.nombreProductor || "Establecimiento";
  const loc = [form.localidad, form.provincia, form.pais].filter(Boolean).join(", ");
  const tray = motor?.tray;
  const bm   = motor?.balanceMensual || [];
  const esc  = cerebro_data?.escenarios || [];
  const bib  = cerebro_data?.bibliografiaRecom || {};
  const MESES = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];

  // ── SECCIÓN 1: IDENTIFICACIÓN ──────────────────────────────────
  let prompt = `INFORME TÉCNICO CALF AI — ${hoy}
Establecimiento: ${est}
Ubicación: ${loc || "sin especificar"}
Biotipo: ${form.biotipo || "sin especificar"}
Vacas: ${form.vacasN || "ND"} cab | PV adulto: ${form.pvVacaAdulta || "ND"} kg
Clima hoy: ${sat?.temp ? sat.temp+"°C" : "ND"} | NDVI: ${sat?.ndvi || "ND"} (${sat?.condForr || "ND"}) | Lluvia 30d: ${sat?.p30 ? sat.p30+"mm" : "ND"}

`;

  // ── SECCIÓN 2: ESTADO DEL SISTEMA ─────────────────────────────
  prompt += `=== ESTADO DEL SISTEMA ===
CC ponderada hoy: ${motor?.ccPondVal?.toFixed(1) || "ND"} (escala 1-9 INTA)
CC al servicio proyectada: ${tray?.ccServ || "ND"}
Preñez estimada: ${tray?.pr || "ND"}%
Anestro posparto: ${tray?.anestro?.dias || "ND"} días
Score sistema: ${motor?.scoreRiesgo || "ND"}/100

`;

  // ── SECCIÓN 3: BALANCE FORRAJERO ──────────────────────────────
  const defInv = [5,6,7].map(i => bm[i]).filter(m => m?.balance < 0);
  prompt += `=== BALANCE FORRAJERO MENSUAL (Mcal/día rodeo) ===
${bm.map(m => `${MESES[m.i]}: ${m.balance > 0 ? "+" : ""}${m.balance} Mcal (oferta: ${m.ofPastoTotal}+supl:${m.suplAporte}+cc:${m.ccAporte} vs demanda:${m.demanda})`).join("\n")}

Meses con déficit invernal: ${defInv.length === 0 ? "Ninguno ✓" : defInv.map(m => MESES[m.i]+" "+m.balance+" Mcal").join(", ")}

`;

  // ── SECCIÓN 4: ESCENARIOS COMPARATIVOS ────────────────────────
  if (esc.length > 1) {
    prompt += `=== ESCENARIOS COMPARATIVOS ===
`;
    esc.forEach(e => {
      const defE = [5,6,7].map(i => e.balance[i]).filter(m => m?.balance < 0);
      prompt += `${e.label}:
  Preñez proyectada: ${e.prenez}% | CC servicio: ${e.ccServ}
  Déficit invernal: ${defE.length === 0 ? "Sin déficit ✓" : defE.map(m => MESES[m.i]).join("+")}
  ${e.descripcion}
  ${e.dosisRecomendada ? "Dosis: "+e.dosisRecomendada : ""}
`;
    });
    const mejorEsc = esc.slice(1).reduce((a,b) => b.prenez > a.prenez ? b : a, esc[1]);
    const prenezBase = esc[0]?.prenez || 0;
    prompt += `
Escenario más efectivo: ${mejorEsc.label} → +${mejorEsc.prenez - prenezBase}pp preñez
`;
  }

  // ── SECCIÓN 5: VAQUILLONA ──────────────────────────────────────
  if (motor?.vaq1E || motor?.vaq2E) {
    prompt += `
=== VAQUILLONA ===
`;
    if (motor.vaq1E && !motor.vaq1E.mensaje) {
      prompt += `1° Invierno: PV entrada ${motor.pvEntVaq1 || "ND"} kg | GDP pasto: ${motor.vaq1E.gdpPasto} g/d | Con supl: ${motor.vaq1E.gdpReal} g/d | PV agosto est: ${motor.vaq1E.pvSal} kg
`;
    }
    if (motor.vaq2E) {
      prompt += `2° Invierno: PV mayo ${motor.pvEntradaVaq2 || "ND"} kg | ${motor.vaq2E.llegas ? "Llega al entore ✓" : "No llega — déficit "+(motor.vaq2E.pvMinEntore - motor.vaq2E.pvEntore)+" kg"}
`;
    }
  }
...
Toros: ${form.sanToros === "con_control" ? "Con revisión ✓" : "Sin revisión ⚠"}
`;

  // ── SECCIÓN 7: INSTRUCCIONES AL MODELO ────────────────────────
  prompt += `
=== INSTRUCCIONES PARA EL INFORME ===
Generá un informe técnico completo con las siguientes secciones:

1. DIAGNÓSTICO INTEGRADO (3-4 párrafos): Analizá el sistema completo. Relacioná clima/NDVI con oferta forrajera, CC con preñez, y cómo cada variable afecta al resultado final. Identificá la causa raíz del problema principal.

2. PRIORIDADES DE ACCIÓN (máximo 5, ordenadas por impacto):
   Para cada una: acción concreta | impacto cuantificado | cuándo implementar | dosis/cantidades si aplica

3. BALANCE FORRAJERO — ANÁLISIS:
   Explicá qué meses son críticos y por qué. Cuantificá el déficit. Describí el escenario recomendado vs actual.

4. PROYECCIÓN CON RECOMENDACIONES:
   Si se implementan las recomendaciones prioritarias: preñez esperada, CC al servicio, kg ternero adicionales.

5. BIBLIOGRAFÍA APLICADA:
   Citá las fuentes específicas que sustentan cada recomendación.
   Incluí: Peruchena INTA 2003, Selk 1988, Short 1990, Wiltbank 1990, Detmann/NASSEM 2010, NRC 2000, Balbuena INTA 2003, López et al. 2021 según corresponda.

Usá lenguaje técnico pero claro. Sé preciso con los números. No repitas datos — analizalos.
`;
