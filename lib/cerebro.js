...
function interpretarVaquillona(motor, form) {
  ...
  function construirDx(e, cat, pvObjetivo) {
    ...
    if (e.sinSupl && limId !== "ok") {
      escenario = `Sin suplemento: pasto aporta ${gdpPasto >= 0 ? "+" : ""}${gdpPasto} g/d. ${pvLabel} ${pvActual} kg (${pctAdulto}% PV adulto). ` +
                  (llega ? `Dentro del objetivo (${pvObjetivo} kg).` : `Brecha ${brechaPV} kg vs objetivo ${pvObjetivo} kg.`);
    } else if (suplCarg && dosisCarg > 0) {
      escenario = `Con ${dosisCarg} kg/cab ${suplCarg}: GDP ${gdpReal} g/d. ${pvLabel} ${pvActual} kg (${pctAdulto}% PV adulto). ` +
                  (llega ? "Objetivo cumplido." : `Brecha ${brechaPV} kg vs objetivo ${pvObjetivo} kg.`);
    } else {
      escenario = `${pvLabel} proyectado ${pvActual} kg (${pctAdulto}% PV adulto, objetivo ${pvObjetivo} kg). ` +
