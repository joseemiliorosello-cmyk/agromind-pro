...
import { calcGEI, calcYm, calcGE_kgMS, calcCH4_kgMS, calcCO2PastizalAnual,
         GWP_CH4, GWP100, AR6 } from "./gei";
...
function calcScore(motor, form, faseCiclo) {
  ...
  // ── TOTAL PONDERADO ──────────────────────────────────────────
  const dim = [
    { id:"cc",      nombre:"CC al servicio",    score:Math.round(scoreCC),    peso:25, descripcion:descCC,    color:scoreCC>=75?"#7ec850":scoreCC>=50?"#e8a030":"#e05530" },
    ...
    { id:"gei",     nombre:"Eficiencia GEI",     score:Math.round(scoreGEI),   peso:5,  descripcion:descGEI,   color:scoreGEI>=75?"#7ec850":scoreGEI>=50?"#e8a030":"#e05530" },
  ];
...
function elegirSuplEnergetico(form, emMin) {
...
function elegirSuplProteico(form, pbMin) {
  // Elegir el suplemento con mayor PB disponible según lo que cargó el usuario
  const opciones = ['supl_vacas', 'supl1', 'supl2', 'supl3'];
  for (const key of opciones) {
    const nombre = form[key];
  ...
  // Fallback: expeller girasol
  return 'Expeller girasol';
}
...
getBiotipo,
  ...
  calcYm, calcGE_kgMS, calcCH4_kgMS,
  GWP_CH4, GWP100, AR6,
};
