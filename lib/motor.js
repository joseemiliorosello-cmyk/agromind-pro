...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  for (let i = 0; i < MESES; i++) {
    ...
    // Suplementacion proteica de esta categoria (si la hay)
    let suplKey_s1, suplKey_d1, suplKey_s2, suplKey_d2;
    if (cat === "vacas_v2s") { suplKey_s1="supl_vacas"; suplKey_d1="dosis_vacas"; suplKey_s2="supl_v2s"; suplKey_d2="dosis_v2s"; }
    else if (cat === "vaq1") { suplKey_s1="supl_vaq1"; suplKey_d1="dosis_vaq1"; }
    else if (cat === "vaq2") { suplKey_s1="supl_vaq2"; suplKey_d1="dosis_vaq2"; }
    else { suplKey_s1="supl_vacas"; suplKey_d1="dosis_vacas"; }

    const s1 = form[suplKey_s1] || "";
    const d1 = parseFloat(form[suplKey_d1]) || 0;
    const s2 = suplKey_s2 ? (form[suplKey_s2] || "") : "";
    const d2 = suplKey_d2 ? (parseFloat(form[suplKey_d2]) || 0) : 0;

    const ALIM = (typeof ALIMS !== 'undefined') ? ALIMS : {};
    const a1 = ALIM[s1] || null;
    const a2 = ALIM[s2] || null;
    ...
    const pbSuplDia = supleActivo ? ((a1?.pb || 0) * d1 * 10 + (a2?.pb || 0) * d2 * 10) : 0; // g PB/animal/dia
    const mcalSuplDia = supleActivo ? ((a1?.em || 0) * d1 + (a2?.em || 0) * d2) : 0;
