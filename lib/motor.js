...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  const categorias = {
    ...
    "vaq1":       { n: Math.round((parseFloat(form.vacasN)||0) * (parseFloat(form.pctReposicion)||20) / 100),
                    pv: 180, consPot: 2.5, label: "Vaquillona 1° invierno" },
  ...
  for (let i = 0; i < MESES; i++) {
    ...
    // Digestibilidad desde tabla fenologica — NDVI modula dispMS (cantidad), no composicion quimica
    // Cita: Peruchena 2003; Rosello et al. 2025
    const digMap = DIG_POR_VEG_FEN[veg] || DIG_POR_VEG_FEN["Pastizal natural"];
    const digFinal = Math.max(40, Math.min(70, digMap[fenol] || 52));
