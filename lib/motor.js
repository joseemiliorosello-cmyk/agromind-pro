...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  const categorias = {
    ...
    "vaq1":       { n: Math.round((parseFloat(form.vacasN)||0) * (parseFloat(form.pctReposicion)||20) / 100),
                    pv: 180, consPot: 3.0, label: "Vaquillona 1° invierno" },
  ...
  for (let i = 0; i < MESES; i++) {
    ...
    // Estimar dig y PB del pasto por vegetacion+fenologia
    const fenol = form.fenologia || "25_50";
    const digMap = DIG_POR_VEG_FEN[veg] || DIG_POR_VEG_FEN["Pastizal natural"];
    const digMes = (digMap[fenol] || 52) * (ndviMes / 0.7); // modula por NDVI estacional
    ...
    // Demanda — usar reqEM si esta, si no estimar con factor
    const estadoFisiol = motor?.balanceMensual?.[i]?.estado || "Vaca seca sin ternero";
