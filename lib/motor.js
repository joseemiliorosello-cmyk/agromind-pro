...
const CC_MINIMA_MOVILIZACION = 2.5;
...
function calcMovilizacionCC(tray, _mes, pvAdulta, estadoFisiolMes) {
  ...
  if (ccMin < CC_MINIMA_MOVILIZACION) return 0;
...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  for (let i = 0; i < MESES; i++) {
    ...
    let mcalSuplDia;
    if (cat === "vaq1") {
      const a = SUPLEMENTOS[form.supl_vaq1] || null;
      const d = parseFloat(form.dosis_vaq1) || 0;
      mcalSuplDia = supleActivo ? (a?.em||0)*d : 0;
    } else if (cat === "vaq2") {
      const a = SUPLEMENTOS[form.supl_vaq2] || null;
      const d = parseFloat(form.dosis_vaq2) || 0;
      mcalSuplDia = supleActivo ? (a?.em||0)*d : 0;
    } else if (cat === "vacas_v2s") {
      // V2S sí recibe suplemento; vacas no — promedio por animal
      const nAll = C.n || 1;
      const av2s = SUPLEMENTOS[form.supl_v2s] || null;
      const dv2s = parseFloat(form.dosis_v2s) || 0;
      const nV2sLoc = parseFloat(form.v2sN) || 0;
      mcalSuplDia = supleActivo ? (av2s?.em||0)*dv2s*nV2sLoc / Math.max(1,nAll) : 0;
    } else {
      // general — suma todos los suplementos / total animales
      const nAll = C.n || 1;
      const suplCatsG = [
        {sk:"supl_v2s",   dk:"dosis_v2s",   n: parseFloat(form.v2sN)||0},
        {sk:"supl_toros", dk:"dosis_toros", n: parseFloat(form.torosN)||0},
        {sk:"supl_vaq2",  dk:"dosis_vaq2",  n: parseFloat(form.vaq2N)||0},
        {sk:"supl_vaq1",  dk:"dosis_vaq1",  n: nVaqR},
      ];
      let totMcal = 0;
      suplCatsG.forEach(c => {
        const sup = SUPLEMENTOS[form[c.sk]];
        const dos = parseFloat(form[c.dk]) || 0;
        if (sup && dos && c.n) totMcal += sup.em*dos*c.n;
      });
