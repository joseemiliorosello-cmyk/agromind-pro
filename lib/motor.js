...
import { BIOTIPOS, getBiotipo, SUPLEMENTOS, PROD_BASE,
         CALIDAD_C4_CALIBRADA, MESES_NOM, PB_FENOL, FORM_DEF, getClima } from "./constantes";


// ─── CC PONDERADA (local, también en constantes) ─────────────────
const ccPond = (dist) => {
  let s = 0, t = 0;
...
// ─── CONSTANTES Y HELPERS ADICIONALES ───────────────────────────
...
const pbPasto     = (fenol) => PB_FENOL[fenol] ?? 10;
const FENOLOGIAS = [
...
function ccAPrenez(cc) {
  const c = Math.min(9, Math.max(1, parseFloat(cc) || 0));
  if (c >= CC_PR[0].ccP) return CC_PR[0].pr;
  if (c <= CC_PR[CC_PR.length - 1].ccP) return CC_PR[CC_PR.length - 1].pr;
  for (let i = 0; i < CC_PR.length - 1; i++) {
    if (c <= CC_PR[i].ccP && c >= CC_PR[i + 1].ccP) {
      const r = (c - CC_PR[i + 1].ccP) / (CC_PR[i].ccP - CC_PR[i + 1].ccP);
      return Math.round(CC_PR[i + 1].pr + r * (CC_PR[i].pr - CC_PR[i + 1].pr));
    }
  }
  return 20;
}
...
const NDVI_ESTAC_DEF = [0.85,0.88,0.90,0.80,0.65,0.50,0.45,0.50,0.65,0.78,0.82,0.84];
...
const GDP_MIN_INV_VAQ1_REF = Math.round(65000 / 122); // g/d — mismo cálculo que gdpMinInv en calcVaq1

// GDP potencial estacional NEA — pastizal natural sin suplemento, vaquillona típica
// Calibrado: Balbuena & Rabotnikof 2003 (INTA EEA El Colorado), Peruchena 2003
// Ene  Feb  Mar  Abr  May  Jun  Jul  Ago  Sep  Oct  Nov  Dic
const BASE_GDP_NEA = [300, 280, 250, 180,  80,  45,  35,  55, 180, 280, 320, 300];

// GDP primavera-verano por tipo de recurso — vaquillona sin suplemento
// 4 fases: [sep-oct (61d), nov-dic (61d), ene-mar (90d), abr-may (58d)]
// Fuente: Peruchena 2003; Balbuena 2003; datos de campo NEA/Chaco
const GDP_VERANO_TIPO = {
  pastizal:     [300, 330, 170, 190],  // C4 natural NEA (promedio 256 g/d)
  megatermicas: [400, 430, 230, 250],  // Gatton panic / brachiaria (promedio 318 g/d)
  mixta:        [360, 380, 205, 225],  // Gramíneas + leguminosas (promedio 289 g/d)
  verdeo:       [300, 250, 150, 185],  // Verdeo invierno (declinando en verano)
  bosque:       [200, 220, 120, 140],  // Monte nativo / bosque (promedio 174 g/d)
  c3:           [330, 280, 160, 205],  // Pasturas templadas C3
};

// Normaliza la vegetación a una clave de GDP_VERANO_TIPO
function vegKeyVerano(veg) {
  const v = (veg || "").toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "");
  if (v.includes("gatton") || v.includes("buffel") || v.includes("brachiaria") ||
      v.includes("megat")  || v.includes("gaton"))           return "megatermicas";
  if (v.includes("mixta")  || v.includes("legum"))           return "mixta";
  if (v.includes("verdeo"))                                   return "verdeo";
  if (v.includes("bosque") || v.includes("monte"))           return "bosque";
  if (v.includes("templada") || v.includes(" c3"))           return "c3";
  return "pastizal"; // pastizal natural NEA/Chaco + default
}
...
const ndviMes = (i, ndviHoy, zona) => {
  const curva  = NDVI_ESTAC[zona] || NDVI_ESTAC_DEF;
  const factor = curva[i] ?? 1.0;
  const base   = parseFloat(ndviHoy || 0.45);
  return Math.max(0.15, Math.min(0.95, base * factor));
};

function calcOfertaMensualArray(veg, ndviHoy, provincia, enso, fenolActual, zona) {
  const hist  = getClima(provincia || "Corrientes");
  const pb    = PROD_BASE[veg] || 8;
  const modE  = modENSO(enso);
  const zonaK = zona || "NEA";
  const fenolMes = [
  ...
  return hist.map((m, i) => {
    ...
    const ndviI = ndviMes(i, ndviHoy, zonaK); // NDVI real para este mes
    const kgMs  = Math.max(0,
      pb * factorN(ndviI) * factorT(t) * factorP(p) * UTIL * fAprovFenol(fenol)
    );
    return +(kgMs * mcalKgAdj(t, fenol)).toFixed(2);
  });
}

// Factor forrajero unificado: combina cantidad (disponMS), calidad (fenología) y verdor (NDVI).
// Multiplica la tasa de recuperación CC post-destete en calcTrayectoriaCC/calcDistCC.
// Rango: 0.45 (pasto escaso + encañado + NDVI bajo) → 1.25 (pasto abundante + tierno + NDVI alto).
function calcFactorForrajero(disponMS, fenologia, ndvi) {
  const nivel = disponMS?.nivelEfectivo || disponMS?.nivel || "media";
  const factCant = nivel === "baja" ? 0.60 : nivel === "alta" ? 1.10 : 0.85;
  const factCal = { menor_10: 1.15, "10_25": 1.0, "25_50": 0.82, mayor_50: 0.65 }[fenologia] || 1.0;
  const ndviFact = Math.min(1.20, Math.max(0.70, (parseFloat(ndvi) || 0.45) / 0.45));
  return Math.min(1.25, Math.max(0.45, factCant * factCal * ndviFact));
}

/**
 * Proyecta la trayectoria de CC (condición corporal) a lo largo del año reproductivo.
 * @param {object} params - Campos del Form relevantes (dist, cadena, biotipo, suplementación, etc.)
 * @returns {{ ccHoy, ccParto, ccMinLact, ccServ, mesParto, mesServ, pr, ... }} Trayectoria CC + preñez estimada
 */
function calcTrayectoriaCC(params) {
  const {
    ...
    provincia,
    factorRecupForrajero = 1.0,  // 1.0 = sin corrección; <1 = balance deficitario en período destete→servicio
  } = params;
  ...
  for (let d = 0; d < diasRecup; d++) {
    ...
    // Tasa diaria de recuperación CC: temperatura es proxy estacional, factorRecupForrajero corrige por balance real
    const tasaRecup = (t >= 25 ? 0.013 : t >= 20 ? 0.009 : t >= 15 ? 0.005 : 0.003) * factorRecupForrajero;
  ...
  // mparto: mes de parto de referencia (default: cabeza de parición)
  const calcCcServConDestete = (diasLact, mparto = mesParto) => {
    const ml    = diasLact / 30;
    const caida = Math.min(2.5, ml * (tasaCaida - reducCaida));
    const ccMin = Math.max(1, ccParto - caida);
    const mDest = (mparto + Math.round(ml)) % 12;
    const mSv   = cadenaR.ini ? cadenaR.ini.getMonth() : (mparto + 3) % 12;
    ...
    for (let d = 0; d < dRec; d++) {
      ...
      const tr = (t >= 25 ? 0.013 : t >= 20 ? 0.009 : t >= 15 ? 0.005 : 0.003) * factorRecupForrajero;
  ...
  // Escenarios para la VACA TARDÍA (parió al final del servicio)
  // Solo relevante cuando diasServ > 120 — la cola tiene mucho menos recuperación
  const dS = cadenaR.diasServ || 0;
  const mesParceTard = cadenaR.partoTard ? cadenaR.partoTard.getMonth()
    : (mesParto + Math.round(dS / 30)) % 12;
  const ccServAnticCola = dS > 120 ? calcCcServConDestete(90,  mesParceTard) : null;
  const ccServHiperCola = dS > 120 ? calcCcServConDestete(50,  mesParceTard) : null;
  const ccServTradCola  = dS > 120 ? calcCcServConDestete(180, mesParceTard) : null;
  ...
  // Recomendación específica para la vaca tardía cuando el servicio es largo
  let recDesteTardio = null;
  if (dS > 120 && ccServTradCola !== null) {
    if (ccServTradCola >= 4.5) {
      recDesteTardio = { tipo:"tradicional", label:"🟢 Vacas tardías: Tradicional OK", urgencia:"monitorear" };
    } else if (ccServAnticCola >= 4.5) {
      recDesteTardio = {
        tipo:"anticipado", label:"🔶 Vacas tardías: Anticipado (90d)",
        urgencia:"importante",
        motivo:`Servicio de ${dS}d: vaca tardía solo llega a CC ${ccServTradCola} con trad. Anticipado: CC ${ccServAnticCola}.`,
      };
    } else {
      recDesteTardio = {
        tipo:"hiperprecoz", label:"⚡ Vacas tardías: Hiperprecoz (50d) urgente",
        urgencia:"urgente",
        motivo:`Servicio de ${dS}d: vaca tardía llega a CC ${ccServHiperCola} solo con hiperprecoz.`,
      };
    }
  }
  ...
  return {
    ...
    recDestete, recDesteTardio,
    ccServAnticCola, ccServHiperCola, ccServTradCola,
    _estimada: esEstimada,
    _factorForrajero: factorRecupForrajero,
  };
}
...
function calcDistCC(params) {
  ...
  const calcCcServ = (ccDestete, mesDestete) => {
    ...
    for (let d = 0; d < diasRecup; d++) {
      ...
      const tasaRecup = (t >= 25 ? 0.012 : t >= 20 ? 0.008 : t >= 15 ? 0.004 : 0.002) * factorRecupForrajero;
...
// Calcula GDP primavera-verano ponderado por tipo de recurso de los potreros
// de vaq1 (o por vegetacion del form como fallback), ajustado por NDVI real
function gdpVeranoPotreros(potreros, vegetacion, ndvi) {
  const DIAS = [61, 61, 90, 58]; // sep-oct, nov-dic, ene-mar, abr-may
  // NDVI de referencia para primavera NEA (~0.50 en pastizal activo sep-oct)
  const ndviFact = Math.min(1.20, Math.max(0.60, (parseFloat(ndvi) || 0.45) / 0.50));

  // Potreros asignados a vaq1 (o todos los que tienen ha cargada si no hay asignación)
  const potsV = Array.isArray(potreros) && potreros.length > 0
    ? potreros.filter(p => {
        const ha   = parseFloat(p.ha) || 0;
        const cats = p.categorias || [];
        return ha > 0 && (cats.length === 0 || cats.includes("vaq1"));
      })
    : [];

  const haTotal = potsV.reduce((s, p) => s + (parseFloat(p.ha) || 0), 0);

  const baseGdp = DIAS.map((_, i) => {
    if (haTotal > 0) {
      // Promedio ponderado por ha y tipo de recurso de cada potrero
      return potsV.reduce((s, p) => {
        const ha  = parseFloat(p.ha) || 0;
        const key = vegKeyVerano(p.veg || vegetacion);
        return s + GDP_VERANO_TIPO[key][i] * ha;
      }, 0) / haTotal;
    }
    // Fallback: vegetacion del form
    return GDP_VERANO_TIPO[vegKeyVerano(vegetacion)][i];
  });

  return DIAS.map((dias, i) => ({ dias, gdp: Math.round(baseGdp[i] * ndviFact) }));
}

function calcPvEntradaVaq2(pvSalidaVaq1, ctx = {}) {
  // agosto → mayo = ~270 días en 4 fases estacionales
  // GDP por fase depende del tipo de recurso de los potreros y el NDVI real
  // Sin datos: fallback a pastizal natural NEA (256 g/d promedio)
  const fases    = gdpVeranoPotreros(ctx.potreros, ctx.vegetacion, ctx.ndvi);
  const ganancia = fases.reduce((s, f) => s + f.gdp * f.dias / 1000, 0);
  const gdpProm  = Math.round(ganancia / (fases.reduce((s, f) => s + f.dias, 0) / 1000));
  return {
    pv:      Math.round((parseFloat(pvSalidaVaq1) || 0) + ganancia),
...
function calcVaq1(pvEntrada, pvAdulta, ndvi, edadMayo, tipoDestete, disponMS, fenologia, suplReal, dosisRealKg, suplReal2, dosisRealKg2) {
  ...
  // nivelEfectivo descuenta el efecto del agua sobre el consumo (mismo criterio que vacas)
  const nivelMS    = disponMS?.nivelEfectivo || disponMS?.nivel || "media";
  const msHa       = disponMS?.msHaEfectivo  || disponMS?.msHa  || 1500;
  const fenol      = fenologia || "25_50";
  const calidadBaja = ["25_50","mayor_50"].includes(fenol);  // pasto encañado/maduro

  // Sin suplemento invernal: pérdida o mantenimiento de peso (Peruchena 2003; INTA El Colorado)
  // C4 dormante: PB < 5% → rumen limitado → -80 a 0 g/d según disponibilidad de pasto
  // El suplemento proteico es lo que genera GDP positivo (Detmann/NASSEM 2010)
  const ndviN       = parseFloat(ndvi) || 0.45;
  const factNdvV1   = Math.min(1.25, Math.max(0.70, ndviN / 0.45));
  const GDP_BASE_V1 = nivelMS === "baja" ? -80 : nivelMS === "alta" ? 0 : -35;
  // NDVI alto = algo de biomasa verde tardía = reduce la pérdida (±40 g/d por desvío)
  const gdpPasto    = Math.min(0, Math.round(GDP_BASE_V1 + (factNdvV1 - 1.0) * 40));
  ...
  const mcalSuplReal = (suplReal  ? mcalSuplemento(suplReal,  dosisRealKg  || 0) : 0)
                     + (suplReal2 ? mcalSuplemento(suplReal2, dosisRealKg2 || 0) : 0);
  ...
  const pbPastoAct = PB_FENOL[fenol] ?? 9;
  ...
  let protKg, energKg, freq, fuenteEnerg;

  if (nivelMS === "baja") {
    protKg      = Math.round(pv * 0.007 * 10) / 10;
    energKg     = Math.round(pv * 0.004 * 10) / 10;
    freq        = "Diario";
  ...
  const gdpReal   = sinSupl
    ? gdpBase  // sin suplemento = pérdida o mantenimiento (-80 a 0 g/d NEA)
  ...
  return {
    esc, protKg, energKg, freq, fuenteEnerg,
    ...
    gdpSuplAporte: boostSuplReal,
    gdpNecesario, pbPastoAct,
    pvEntrada: pv, pvFuenteDato,
  };
}

function calcVaq2(pvEntradaVaq1Sal, pvAdulta, ndvi, disponMS, fenologia, suplReal, dosisRealKg, ctx = {}) {
  ...
  // Si el vet midió el PV del animal al inicio del 2do invierno, usarlo directo.
  // De lo contrario, calcularlo sumando el verano desde la salida del 1er invierno.
  let resVerano;
  if (ctx.pvMayo2InvDirecto) {
    const ganancia = Math.round(ctx.pvMayo2InvDirecto - pvSal1);
    const diasVer  = 270;
    resVerano = {
      pv:       ctx.pvMayo2InvDirecto,
      ganancia: Math.max(0, ganancia),
      gdpProm:  Math.round(Math.max(0, ganancia) * 1000 / diasVer),
      fases:    [],
    };
  } else {
    resVerano = calcPvEntradaVaq2(pvSal1, ctx);
  }
  const pvMayo2Inv  = resVerano.pv;
  const pvMinEntore     = Math.round(pva * 0.65);
  ...
  const factNdvV2   = Math.min(1.25, Math.max(0.70, ndviN / 0.45));
  // Sin suplemento 2° invierno: igual que Vaq1 — pérdida o mantenimiento
  const GDP_BASE_V2 = nivelMS === "baja" ? -80 : nivelMS === "alta" ? 0 : -35;
  const gdpPastoInv = Math.min(0, Math.round(GDP_BASE_V2 + (factNdvV2 - 1.0) * 40));
  ...
  const pbPastoV2   = PB_FENOL[fenologia||"25_50"] ?? 6;
  ...
  // llegas = criterio de peso final al entore (pvEntore >= pvMinEntore)
  // GDP insuficiente genera recomendacion de suplemento pero no bloquea "llega"
  ...
  let protKg, energKg, fuenteEnerg, freq;
  if (gdpFaltante > 0) {
    protKg  = Math.round(pvMayo2Inv * 0.005 * 10) / 10;
    const usaSemAlg = pvMayo2Inv >= 280;
    if (usaSemAlg) {
      energKg     = Math.round(pvMayo2Inv * 0.004 * 10) / 10;
    ...
    freq = "2–3 veces/semana"; fuenteEnerg = null;
  }
  ...
  return {
    ...
    gdpInv: gdpInvReal, gdpReal: gdpInvReal, gdpPastoInv, gdpPrimavera, llegas,
    aptaEntoreAntic, pvMinEntoreAntic, pctPVAgosto,
    protKg, energKg, fuenteEnerg, freq,
    nivelMS, gdpFaltante, sinSupl: sinSupl2, mcalSuplReal,
    gdpNecesario: GDP_MIN_VAQ2, pbPastoAct: pbPastoV2,
    gdpConSuplReal,
    gdpVeranoProm: resVerano.gdpProm,
    gananciaVerano: resVerano.ganancia,
  };
}
...
/**
 * Calidad de preñez: distribuye la preñez total del servicio en cabeza/cuerpo/cola
 * según ciclos estrales de 21 días, asumiendo probabilidad de concepción constante
 * por ciclo (p) tal que (1-p)^nCiclos = 1 - preñezTotal. Cabeza = 1° ciclo (mejor CC,
 * terneros más pesados al destete), cuerpo = 2° ciclo, cola = 3° ciclo en adelante
 * (mayor riesgo de repetir vacía el año siguiente — TABLA_SUPERV).
 */
function calcCalidadPrenez(ccServ, diasServ) {
  const nCiclos = Math.max(1, Math.round((parseFloat(diasServ) || 90) / 21));
  const prenezTotal = ccAPrenez(ccServ) / 100;
  if (prenezTotal <= 0) return null;

  const p = 1 - Math.pow(1 - prenezTotal, 1 / nCiclos);
  const fCabeza = p;
  const fCuerpo = (1 - p) * p;
  const fCola   = Math.max(0, prenezTotal - fCabeza - fCuerpo);

  const cabezaPct = Math.round((fCabeza / prenezTotal) * 100);
  const cuerpoPct = Math.round((fCuerpo / prenezTotal) * 100);
  const colaPct   = 100 - cabezaPct - cuerpoPct;

  return {
    prenezTotalPct: Math.round(prenezTotal * 100),
    nCiclos,
    cabeza: { pct: cabezaPct, riesgo: "bajo",   nota: "1° ciclo — terneros más pesados al destete" },
    cuerpo: { pct: cuerpoPct, riesgo: "medio",  nota: "2° ciclo — destete intermedio" },
    cola:   { pct: colaPct,   riesgo: "alto",   nota: "3°+ ciclo — terneros livianos, mayor riesgo de repetir vacía" },
  };
}

/**
 * Calcula las fechas clave del ciclo reproductivo (parto, destete, servicio).
 * @param {string} iniServ - Fecha inicio de servicio "YYYY-MM-DD"
 * @param {string} finServ - Fecha fin de servicio "YYYY-MM-DD"
 * @returns {{ iniServ: Date, finServ: Date, diasServ: number, parto: Date, destete: Date } | null}
 */
function calcCadena(iniServ, finServ) {
  ...
  return {
    ini, fin, diasServ, esContinuo: diasServ >= 180,
    partoTemp, partoTard, diasPartoTemp, diasPartoTard,
...
function evaluarSanidad(vacunas, brucelosis, aftosa, toros, historiaAbortos, programaSanit, cadena) {
  ...
  // Vacunar 30–45d antes del servicio — solo aparece como tarea si no hay programa sanitario estructurado
...
function dZona(lat, lon) {
  ...
  if (lat>-30&&lon<=-63) return "NOA";   // Salta/Jujuy/Tucumán/Catamarca/Stgo del Estero
...
/**
 * Calcula el score global de riesgo del establecimiento (0-100).
 * Integra CC al servicio, balance invernal, vaquillonas, sanidad y agua.
 * @param {import('./types').Motor} motor - Resultado de correrMotor
 * @param {import('./types').Form} form
 * @param {string} faseCiclo - Fase del ciclo ("preparto", "servicio", etc.)
 * @returns {{ score: number, nivel: string, color: string, alertas: string[] } | null}
 */
function calcScore(motor, form, faseCiclo) {
  ...
  let scoreCC = 0;
  let descCC  = "";
  // Escala CC al servicio: ≥4.5 aceptable · 4.5 = umbral crítico · <3.5 malo.
  // (La vaca llega al servicio tras perder ~1 punto desde el parto, cuyo objetivo es 5.5.)
  if (ccServ <= 0) {
    scoreCC = 50; descCC = "Sin dato de CC al servicio";
  } else if (ccServ >= 5.0) {
    scoreCC = 100; descCC = "CC óptima al servicio (≥5.0)";
  } else if (ccServ >= 4.5) {
    scoreCC = 78 + (ccServ - 4.5) / 0.5 * 22;
    descCC = "CC en el mínimo aceptable (4.5) — justo en el umbral crítico, sin margen";
  } else if (ccServ >= 3.5) {
    scoreCC = 30 + (ccServ - 3.5) / 1.0 * 48;
    descCC = "CC crítica al servicio (<4.5) — preñez comprometida ~8-12pp por cada 0.5 puntos menos";
  } else {
    scoreCC = Math.max(0, (ccServ - 2.0) / 1.5 * 30);
    descCC = "CC MALA al servicio (<3.5) — anestro prolongado, preñez severamente reducida";
  }
  ...
  // ── TOTAL PONDERADO ──────────────────────────────────────────
  const dim = [
    { id:"cc",      nombre:"CC al servicio",    score:Math.round(scoreCC),    peso:30, descripcion:descCC,    color:scoreCC>=75?"#7ec850":scoreCC>=50?"#e8a030":"#e05530" },
    { id:"balance", nombre:"Balance invernal",   score:Math.round(scoreBal),   peso:20, descripcion:descBal,   color:scoreBal>=75?"#7ec850":scoreBal>=50?"#e8a030":"#e05530" },
    { id:"repro",   nombre:"Reproducción",       score:Math.round(scoreRepro), peso:20, descripcion:descRepro, color:scoreRepro>=75?"#7ec850":scoreRepro>=50?"#e8a030":"#e05530" },
    { id:"vaq",     nombre:"Vaquillona",         score:Math.round(scoreVaq),   peso:15, descripcion:descVaq,   color:scoreVaq>=75?"#7ec850":scoreVaq>=50?"#e8a030":"#e05530" },
    { id:"sanidad", nombre:"Sanidad",            score:Math.round(scoreSan),   peso:15, descripcion:descSan,   color:scoreSan>=75?"#7ec850":scoreSan>=50?"#e8a030":"#e05530" },
  ];
  const total = Math.round(dim.reduce((s,d) => s + d.score * d.peso / 100, 0));
  const colorTotal = total >= 75 ? "#7ec850" : total >= 50 ? "#e8a030" : "#e05530";
...
function elegirSuplEnergetico(form, emMin = 0) {
  const opciones = ['supl_vacas', 'supl1', 'supl2', 'supl3'];
  for (const key of opciones) {
    const nombre = form[key];
    if (nombre && SUPLEMENTOS[nombre] && SUPLEMENTOS[nombre].em >= emMin) {
      return SUPLEMENTOS[nombre];
    }
  }
  return SUPLEMENTOS['Sorgo grano'];
}

function elegirSuplProteico(form, pbMin) {
  const opciones = ['supl_vacas', 'supl1', 'supl2', 'supl3'];
  for (const key of opciones) {
    const nombre = form[key];
    if (nombre && SUPLEMENTOS[nombre] && SUPLEMENTOS[nombre].pb >= pbMin) {
      return SUPLEMENTOS[nombre];
    }
  }
...
function diagnosticarSistema(motor, form) {
  ...
  if (ind.ccServ > 0 && ind.ccServ < 4.5) {
    ...
    const causas   = [];
    // La vaca pierde ~1 punto de CC del parto al servicio (lactación). Por eso el
    // objetivo al parto es 5.5 → aterriza en 4.5 al servicio (crítico). <3.5 = malo.
    if (ind.ccParto < 5.5) causas.push(`CC al parto baja (${ind.ccParto} — objetivo 5.5)`);
    ...
    cuellos.push({
      ...
      icono: "🐄",
      prioridad: ind.ccServ < 3.5 ? "P1" : "P1",
      titulo: ind.ccServ < 3.5
        ? `CC al servicio MALA — ${ind.ccServ} (crítico <4.5 · malo <3.5)`
        : `CC al servicio crítica — ${ind.ccServ} (necesita ≥4.5)`,
      impacto: `Preñez actual: ~${prenezAct}% → con CC 4.5+: ~${prenezObj}% (+${prenezObj-prenezAct}pp)`,
      causas,
      causaRaiz: ind.ccParto < 5.5 ? "cc_parto" : ind.mesesLact > 3 ? "lactacion" : "forraje",
    });
    ...
    planes.push({
      ...
      pasos: [
        ...
        },
        {
          ...
          porque: `ÚNICA ventana eficiente para suplementar la vaca: todavía no tiene ternero al pie → 1 kg ${suplPre.label} = ${suplPre.em} Mcal netas que van a CC. Objetivo: llegar al parto con CC ≥5.5 (pierde ~1 punto en lactación → 4.5 al servicio).`,
  ...
  // Objetivo 5.5 al parto: pierde ~1 punto en lactación → 4.5 al servicio (mínimo).
  if (ind.ccParto > 0 && ind.ccParto < 5.5) {
    cuellos.push({
      id: "cc_parto",
      categoria: "Vaca adulta",
      icono: "🐄",
      prioridad: ind.ccParto < 4.5 ? "P1" : "P2",
      titulo: `CC al parto baja — ${ind.ccParto} (objetivo ≥5.5)`,
      impacto: `Pierde ~1 CC en lactación → llegaría a ~${(ind.ccParto - 1).toFixed(1)} al servicio. Cada 0.5 CC al parto = 25 días menos de anestro posparto`,
  ...
  if (nVaq2 > 0 && vaq2E && !vaq2E.llegas) {
    ...
    const suplEnerg2  = elegirSuplEnergetico(form);
    const pasosVaq2   = [
      {
        orden: 1,
        tipo: "suplemento",
      ...
      ...(kgEnerg2 > 0 ? [{
        orden: 2,
        tipo: "suplemento",
        titulo: `Energía — ${suplEnerg2.label}`,
  ...
  // ── CRONOLOGÍA DEL CICLO PRODUCTIVO ────────────────────────────
  // Ordena los cuellos según el MOMENTO del ciclo en que impactan (mesCiclo
  // relativo: 0 = inicio parición agosto NEA). Permite leer el sistema como
  // línea de tiempo: dónde se rompe primero y qué arrastra después.
  const FASE_CUELLO = {
    cc_parto:      { mesCiclo: 0,  fase: "Parición (ago–oct)" },
    san_brucela:   { mesCiclo: 0,  fase: "Parición (ago–oct)" },
    san_toros_rev: { mesCiclo: 2,  fase: "Pre-servicio (oct–nov)" },
    toros_cc:      { mesCiclo: 2,  fase: "Pre-servicio (oct–nov)" },
    relacion_at:   { mesCiclo: 3,  fase: "Servicio (nov–dic)" },
    cc_serv:       { mesCiclo: 3,  fase: "Servicio (nov–dic)" },
    vaq2_entore:   { mesCiclo: 3,  fase: "Servicio (nov–dic)" },
    agua:          { mesCiclo: 5,  fase: "Verano (ene–mar)" },
    san_aftosa:    { mesCiclo: 7,  fase: "Tacto (mar–abr)" },
    vaq1_peso:     { mesCiclo: 9,  fase: "Invierno recría (may–ago)" },
  };
  const cronologia = cuellos
    .map(c => {
      const f = FASE_CUELLO[c.id] || (c.id?.startsWith("stock_")
        ? { mesCiclo: 10, fase: "Invierno suplementación (jun–ago)" }
        : { mesCiclo: 6,  fase: "Ciclo general" });
      return { ...c, ...f };
    })
    .sort((a,b) => a.mesCiclo - b.mesCiclo || (ord[a.prioridad]??3) - (ord[b.prioridad]??3));
  ...
  return {
    ...
    cronologia,  // los mismos cuellos ordenados por momento del ciclo (parición→servicio→tacto→invierno)
...
/**
 * Punto de entrada principal: corre el motor de diagnóstico completo.
 * Calcula oferta forrajera, balance energético, trayectoria CC, vaquillonas, alertas y score.
 * @param {import('./types').Form} form - Estado del formulario
 * @param {import('./types').Sat | null} sat - Datos satelitales/climáticos actuales
 * @param {import('./types').Potrero[]} potreros - Lista de potreros (puede ser [])
 * @param {boolean} usaPotreros - Si true, calcula oferta por potrero asignado
 * @returns {import('./types').Motor}
 */
// Detmann/NASSEM 2010: suplemento proteico desbloquea microflora cuando PB pasto <8%
// → animal consume más pasto además del suplemento en sí
// Retorna kg MS extra de pasto consumido por vaca por día
function boostConsumoPasto(pbPasto, pbSupl, dosisKgPorAnimal) {
  if (pbPasto >= 8 || dosisKgPorAnimal <= 0) return 0;
  const deficitPB = Math.max(0, 8 - pbPasto) / 8;   // 0-1 según déficit nitrogenado
  const factorPB  = pbSupl >= 30 ? 2.0 : pbSupl >= 20 ? 1.5 : 1.0; // expeller >30% más eficiente
  return Math.min(1.5, dosisKgPorAnimal * deficitPB * factorPB);    // tope 1.5 kg extra/vaca/día
}
...
function correrMotor(form, sat, potreros, usaPotreros) {
  ...
  // Alertar si no hay datos satelitales reales
  if (!sat?.ndvi) {
    addAlerta("sat_faltante", "P2",
      "Sin datos satelitales — NDVI y temperatura usan valores históricos de la zona. Cargá la ubicación para datos reales.",
      "Oferta forrajera estimada con historial — puede diferir de la condición actual",
      "fetchSat");
  }
  ...
  // ── 1.3b OFERTA POR CATEGORÍA según potrero asignado ──────────
  // Si los potreros tienen categorías asignadas, calcular oferta
  // diferenciada por categoría — la más importante mejora del sistema
  const ofertaPorCateg = {};
  if (usaPotreros && potreros && potreros.length > 0) {
    const CATS = ["vacas","v2s","toros","vaq1","vaq2","terneros"];
    CATS.forEach(cat => {
      // Potreros asignados a esta categoría
      const potsCat = potreros.filter(p =>
        (p.categorias||[]).includes(cat) && parseFloat(p.ha) > 0
      );
      if (potsCat.length === 0) return; // sin asignación → usa global
      const haCat = potsCat.reduce((s,p) => s+(parseFloat(p.ha)||0), 0);
      // Disponibilidad promedio ponderada de los potreros de esta categoría
      let msHaCat = 0;
      potsCat.forEach(p => {
        const ha = parseFloat(p.ha)||0;
        const d = calcDisponibilidadMS(p.altPasto||"20", p.tipoPasto||"corto_denso");
        msHaCat += d.msHa * (ha / haCat);
      });
      // PB ponderado por ha de los potreros asignados a esta categoría
      const _haT2  = potsCat.reduce((s,p) => s+(parseFloat(p.ha)||0), 0);
      const _PBM   = { menor_10:12, "10_25":9, "25_50":6, mayor_50:4 };
      const pbPond = _haT2 > 0
        ? potsCat.reduce((s,p) => s+(_PBM[p.fenol||"25_50"]||6)*(parseFloat(p.ha)||0),0) / _haT2
        : 6;
      const vegCat    = potsCat[0].veg || "Pastizal natural";
      const esVerdeo  = potsCat.some(p => (p.veg||"").toLowerCase().includes("verdeo"));
      const pbEfectivo = esVerdeo ? 18 : Math.round(pbPond * 10) / 10;
      const fenolCat  = pbPond >= 10 ? "menor_10" : pbPond >= 7 ? "10_25" : pbPond >= 5 ? "25_50" : "mayor_50";

      ofertaPorCateg[cat] = {
        haCat: Math.round(haCat),
        msHa:  Math.round(msHaCat),
        pb:    pbEfectivo,
        fenol: fenolCat,
        veg:   vegCat,
        esVerdeo,
        // Factor de calidad: si PB < 7% la microflora está bloqueada
        // y el animal no puede aprovechar la fibra — Detmann/NASSEM 2010
        factorCalidad: pbEfectivo < 7 ? 0.55 : pbEfectivo < 10 ? 0.80 : 1.0,
      };
    });
  }
  ...
  const mapVerdeoMes = { junio:5, julio:6, agosto:7, septiembre:8 };
  // Verdeo desde potrero (cat:"verdeo") o desde campos standalone (legado)
  const potreroVerdeo = usaPotreros && Array.isArray(potreros)
    ? potreros.find(p => (p.veg || "").toLowerCase().includes("verdeo"))
    : null;
  if (potreroVerdeo) {
    const haV   = parseFloat(potreroVerdeo.ha) || 0;
    const tipoV = potreroVerdeo.verdeoTipo || "Avena / Raigrás / Melilotus";
    const vp    = VERDEO_PARAMS[tipoV] || VERDEO_PARAMS["Avena / Raigrás / Melilotus"];
    verdeoMesInicio = mapVerdeoMes[potreroVerdeo.verdeoDisp||"agosto"] || 7;
    verdeoAporteMcalMes = +(haV * vp.msHa * vp.mcalKg * vp.util / 90).toFixed(0);
    trazas.verdeo = { ha:haV, tipo:tipoV, msHa:vp.msHa, pb:vp.pb, mcalKg:vp.mcalKg, aporteMcalDia:verdeoAporteMcalMes, desde:verdeoMesInicio };
  } else if (form.tieneVerdeo === "si" && form.verdeoHa) {
    // fallback campos standalone (compatibilidad historial guardado)
    const haV   = parseFloat(form.verdeoHa) || 0;
    const tipoV = form.verdeoTipo || "Avena / Raigrás / Melilotus";
    const vp    = VERDEO_PARAMS[tipoV] || VERDEO_PARAMS["Avena / Raigrás / Melilotus"];
    verdeoMesInicio = mapVerdeoMes[form.verdeoDisp||"agosto"] || 7;
    verdeoAporteMcalMes = +(haV * vp.msHa * vp.mcalKg * vp.util / 90).toFixed(0);
    trazas.verdeo = { ha:haV, tipo:tipoV, msHa:vp.msHa, pb:vp.pb, mcalKg:vp.mcalKg, aporteMcalDia:verdeoAporteMcalMes, desde:verdeoMesInicio };
  }
  ...
  const supl2Vaq1Real  = form.supl2_vaq1  || "";
  const dosis2Vaq1Real = parseFloat(form.dosis2_vaq1) || 0;
  ...
  const vaq1E = calcVaq1(pvEntVaq1, form.pvVacaAdulta, ndviN,
    ...
    suplVaq1Real, dosisVaq1Real, supl2Vaq1Real, dosis2Vaq1Real);
  ...
  // pvSalidaVaq1 = PV en agosto al salir del 1er invierno (input base para el verano)
  // form.vaq2PV = PV medido por el vet al INICIO del 2do invierno (ya incluye el verano)
  //   → si está cargado, no hay que sumarle otro verano; se pasa como pvMayo2InvDirecto
  const pvSalidaVaq1 = vaq1E?.pvSal ? String(vaq1E.pvSal) : "";
  const pvMayo2InvDirecto = form.vaq2PV ? parseFloat(form.vaq2PV) : null;
  const suplVaq2Real  = form.supl_vaq2  || "";
  const dosisVaq2Real = parseFloat(form.dosis_vaq2) || 0;
  const vaq2E = calcVaq2(pvSalidaVaq1, form.pvVacaAdulta, ndviN, disponMS, form.fenologia,
    suplVaq2Real, dosisVaq2Real,
    { potreros, ndvi: ndviN, vegetacion: form.vegetacion, pvMayo2InvDirecto });
  ...
  // Factor forrajero: combina disponibilidad MS de potreros + fenología + NDVI.
  // Pondera la recuperación CC post-destete de todas las categorías de vacas.
  const factorForrajero = calcFactorForrajero(disponMS, form.fenologia, ndviN);
  ...
  const baseParams  = {
    ...
    provincia: form.provincia,
    ndvi: ndviN,
    mesTacto: form.mesTacto || "abr",
    supl1: form.supl1 || "",  dosis1: form.dosis1 || "0",
    supl2: "",                dosis2: "0",
    supl3: suplPrepartoProt,  dosis3: dosisPreparto,
    factorRecupForrajero: factorForrajero,
  };
  ...
  // Detectar inconsistencias antes de calcular
  if (!ccPondVal || ccPondVal === 0) {
    addAlerta("cc_faltante", "P1",
      "CC del rodeo no cargada — ingresá la distribución de CC en el Paso 0 para calcular la trayectoria y preñez correctamente.",
      "Sin CC no hay trayectoria confiable",
      "form.distribucionCC");
  }
  ...
  // PB ponderado del campo real (potreros cargados) para el mes actual
  // Potreros sin categoría asignada se consideran pastoreo del rodeo principal
  const _mesActual = new Date().getMonth();
  const _potsRodeo = usaPotreros && potreros && potreros.length > 0
    ? potreros.filter(p => {
        const cats = p.categorias || [];
        return parseFloat(p.ha) > 0 && (cats.length === 0 || cats.includes("vacas"));
      })
    : [];
  const _haRodeo = _potsRodeo.reduce((s,p) => s+(parseFloat(p.ha)||0), 0);
  const _pbRodeoHoy = _haRodeo > 0
    ? _potsRodeo.reduce((s,p) => {
        const pb = { menor_10:12, "10_25":9, "25_50":6, mayor_50:4 }[p.fenol||"25_50"] || 6;
        return s + pb * (parseFloat(p.ha)||0);
      }, 0) / _haRodeo
    : null;
  // Mapear PB ponderado → clave fenol para usar en los lookups del balance
  const fenolRodeoHoy = _pbRodeoHoy !== null
    ? (_pbRodeoHoy >= 10 ? "menor_10" : _pbRodeoHoy >= 7 ? "10_25" : _pbRodeoHoy >= 5 ? "25_50" : "mayor_50")
    : null;

  const MESES_NOM = ["Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"];
  const hist = getClima(form.provincia || "Corrientes");

  // Si hay un potrero con verdeo asignado, no sumar también el módulo standalone
  // para evitar contar dos veces la misma superficie
  const hayPotreroVerdeo = !!potreroVerdeo;

  const balanceMensual = MESES_NOM.map((mes, i) => {
    const climM  = hist[i] || { t:22, p:80 };
    // Mes actual: usar temperatura real (Open-Meteo) en vez del promedio histórico
    const t      = (i === _mesActual && sat?.temp != null) ? parseFloat(sat.temp) : climM.t;
    const fenol  = (fenolRodeoHoy && i === _mesActual) ? fenolRodeoHoy : fenolMes[i];
    ...
    const mesesDesdeConc = (i - mesServIni + 12) % 12; // edad gestacional desde concepción
    const enServ         = (i - mesServIni + 12) % 12 <= (mesServFin - mesServIni + 12) % 12;

    // Fracción de vacas aún en lactación este mes, por modalidad de destete
    // Corrección: en lugar de un promedio único (mesesLactN), cada grupo tiene su propia duración
    const _pT  = parseFloat(form.destTrad)  || 0;
    const _pA  = parseFloat(form.destAntic) || 0;
    const _pH  = parseFloat(form.destHiper) || 0;
    const _tot = _pT + _pA + _pH;
    // Fallback: sin destete configurado → igual que calcTrayectoriaCC: Tradicional 180d (escenario conservador)
    const wT = _tot > 0 ? _pT / _tot : 1.0;
    const wA = _tot > 0 ? _pA / _tot : 0.0;
    const wH = _tot > 0 ? _pH / _tot : 0.0;
    const fracLactH = mesesDesdeP < 2 ? wH : 0; // hiperprecoz 50d ≈ ceil(1.67) = 2 meses
    const fracLactA = mesesDesdeP < 3 ? wA : 0; // anticipado 90d = 3 meses
    const fracLactT = mesesDesdeP < 6 ? wT : 0; // tradicional 180d = 6 meses
    const fracLact  = fracLactH + fracLactA + fracLactT;
    const enLact    = fracLact > 0;
    // Preparto: último mes de gestación = ~8 meses desde la concepción (gestación ≈ 9 meses)
    const enPrepartO = !enLact && mesesDesdeConc >= 8;

    // Estado gestacional para vacas secas este mes (edad desde concepción, no desde parto)
    const estadoGest = enPrepartO        ? "Preparto (último mes)"
                     : mesesDesdeConc <= 3 ? "Gestación temprana (1–4 meses)"
                     : "Gestación media (5–7 meses)";
    const estadoVaca = fracLact >= 0.9   ? "Lactación con ternero al pie"
                     : fracLact > 0.1    ? "Lactación parcial (" + Math.round(fracLact * 100) + "% con ternero)"
                     : estadoGest;
    ...
    // Requerimiento vaca: promedio ponderado entre grupo lactando y grupo seco
    const reqLact  = reqEM(pvVaca, "Lactación con ternero al pie", form.biotipo) || 14;
    const reqGest  = reqEM(pvVaca, estadoGest, form.biotipo) || 14;
    const reqVacaI = fracLact > 0 && fracLact < 1
      ? +(fracLact * reqLact + (1 - fracLact) * reqGest).toFixed(1)
      : (fracLact >= 1 ? reqLact : reqGest);
    const reqToroI = reqEM(pvVaca*1.3, "Vaca seca sin ternero", form.biotipo) || 14;
    const pvVaq1p  = pvEntVaq1 || Math.round(pvVaca*0.40);
    const pvVaq2p  = vaq2E?.pvMayo2Inv || Math.round(pvVaca*0.65);
    ...
    // Demanda directa del ternero al pie (consumo de pasto, no de leche materna)
    // Comienza ~60d, crece hasta 5.5 Mcal/día a los 5 meses (NRC 2000 / Bavera 2006)
    // La vaca ya cubre la energía de la leche vía factor 1.90 — esto es consumo ADICIONAL
    const pctParicion   = tray?.pr ? (tray.pr / 100) : 0.80;
    const nTerneros     = Math.round(nVacas * pctParicion * 0.90); // 90% supervivencia
    const edadTernMes   = enLact ? mesesDesdeP : -1;
    const reqTernI      = edadTernMes < 2 ? 0
                        : edadTernMes < 3 ? 1.0
                        : edadTernMes < 4 ? 2.5
                        : edadTernMes < 5 ? 4.0
                        : 5.5;
    // Solo los terneros aún al pie consumen pasto (ponderado por fracción no destetada)
    const dTerneros     = Math.round(nTerneros * fracLact) * reqTernI;

    const demanda = dVacas + dToros + dV2s + dVaq2 + dVaq1 + dTerneros;
    ...
    const pvVaq2R  = vaq2E?.pvMayo2Inv || Math.round(pvVaca * 0.65);
    ...
    const factQualI      = pbPasI >= 9 ? 1.10 : pbPasI >= 7 ? 1.00 : 0.90;
    const factNdvI       = Math.min(1.25, Math.max(0.75, ndviI / 0.45));
    const gdpPastoMesVaq = Math.round(BASE_GDP_NEA[i] * factQualI * factNdvI);
    ...
    // Usar PB real del potrero asignado a cada categoría si está disponible
    const pbV1 = ofertaPorCateg?.vaq1?.pb ?? pbPasI;
    const pbV2 = ofertaPorCateg?.vaq2?.pb ?? pbPasI;
    const factV1 = ofertaPorCateg?.vaq1?.factorCalidad ?? 1.0;
    const factV2 = ofertaPorCateg?.vaq2?.factorCalidad ?? 1.0;
    const eficV1 = pbV1 < 7 ? 0.55 : 0.42;
    const eficV2 = pbV2 < 7 ? 0.55 : 0.42;
    // Ajustar GDP base de vaquillona según calidad del potrero asignado
    const gdpPastoV1 = Math.round(gdpPastoMesVaq * factV1);
    const gdpPastoV2 = Math.round(gdpPastoMesVaq * factV2);
    ...
    const gdpVaq1Mes = gdpPastoV1 + boostVaq1;
    const gdpVaq2Mes = gdpPastoV2 + boostVaq2;

    // CC movilizada en lactación — solo las vacas aún lactando movilizan reservas
    const ccAporte = fracLact > 0
      ? Math.round(caidaCC * MCAL_CC * nVacas * fracLact / Math.max(1, mesesLactN*30)) : 0;

    // Creep feeding — ternero come sin competir con la madre
    // Aumenta GDP ternero 700→750 g/d · libera 0.5 Mcal/día de la vaca
    const tieneCreep = form.tieneCreep === "si";
    const creepDosis = parseFloat(form.creepDosis) || 0;
    const creepAporte = (fracLact > 0 && tieneCreep && creepDosis > 0)
      ? Math.round(creepDosis * 3.1 * nVacas * 0.90) // 3.1 Mcal/kg, 90% consumo
      : 0;

    // Verdeo — no sumar si ya hay un potrero con verdeo (misma superficie)
    // tieneVerdeo es string: comparar === "si" (antes "no" pasaba como truthy)
    const verdeoAp = ((form.tieneVerdeo === "si" || hayPotreroVerdeo) && verdeoAporteMcalMes > 0)
    ...
    return {
      ...
      creepAporte: creepAporte||0,
      verdeoAporte:+verdeoAp,
      ofertaTotal:+ofertaTotal.toFixed(0),
      // Oferta pasto per-animal por categoría (para GraficosBalance)
      ofPastoPerVaca:     nVacas         > 0 ? +(ofPastoVacas *factCargaMes/nVacas).toFixed(1)                  : 0,
      ofPastoPerV2s:      nV2s           > 0 ? +(ofPastoV2s   *factCargaMes/nV2s).toFixed(1)                    : 0,
      ofPastoPerVaq1:     nVaq1          > 0 ? +(ofPastoVaq1  *factCargaMes/nVaq1).toFixed(1)                   : 0,
      ofPastoPerVaq2:     nVaq2          > 0 ? +(ofPastoVaq2  *factCargaMes/nVaq2).toFixed(1)                   : 0,
      ofPastoPerVacasV2s: (nVacas+nV2s)  > 0 ? +((ofPastoVacas+ofPastoV2s)*factCargaMes/(nVacas+nV2s)).toFixed(1) : 0,
      // Demanda
      dVacas:+dVacas.toFixed(0), dToros:+dToros.toFixed(0),
      dV2s:+dV2s.toFixed(0), dVaq2:+dVaq2.toFixed(0), dVaq1:+dVaq1.toFixed(0),
      dTerneros:+dTerneros.toFixed(0),
      ...
      // Fisiología
      estadoVaca, estadoGest, enLact, fracLact:+fracLact.toFixed(2), enServ, enPrepartO,
  ...
  const peorDeficitMcal = deficitInv.length > 0 ? Math.min(...deficitInv.map(i => balanceMensual[i].balance)) : 0;
  const deficitAcumInv = deficitInv.reduce((s, i) => s + Math.abs(balanceMensual[i]?.balance ?? 0), 0);
  if (deficitInv.length > 0) {
    addAlerta("balance_inv", deficitInv.length === 3 ? "P1" : "P2",
      `Déficit invernal en ${deficitInv.length}/3 meses clave — peor mes: ${peorDeficitMcal.toLocaleString()} Mcal/día`,
      `Sin corrección: caída CC estimada ${(-peorDeficitMcal / MCAL_CC / nVacas * 30).toFixed(1)} unidades adicionales sobre lo proyectado`,
      "balanceMensual invernal");
  }

  // ── CORRECCIÓN CC × BALANCE REAL PERÍODO DESTETE→SERVICIO ─────────
  // El primer cálculo de tray usó temperatura como proxy de calidad forrajera.
  // Ahora que tenemos balanceMensual, re-calibrar si el balance es deficitario
  // en los meses de recuperación CC (destete→servicio).
  if (tray && balanceMensual?.length === 12) {
    const mD = tray.mesDestete ?? 0;
    const mS = tray.mesServ    ?? 3;
    const nMeses = Math.max(1, Math.round((tray.diasRecup || 60) / 30));
    const mesesRecup = [];
    for (let i = 1; i <= Math.min(nMeses, 6); i++) {
      mesesRecup.push((mS - i + 12) % 12);
    }
    const balProm = mesesRecup.reduce((s, mi) => s + (balanceMensual[mi]?.balance ?? 0), 0) / mesesRecup.length;
    const balPromPerVaca = nVacas > 0 ? balProm / nVacas : 0;
    const factorBalance = balPromPerVaca >= 0 ? 1.0 : balPromPerVaca >= -3 ? 0.75 : 0.50;
    // Factor final = el MÁS LIMITANTE entre oferta forrajera (potreros+fenología+NDVI)
    // y balance energético real (oferta − demanda del rodeo). Nunca se relaja la
    // corrección: si el balance da 0.75 pero el pasto está escaso (0.60), gana 0.60.
    const factorRecupForrajero = Math.min(factorForrajero, factorBalance);
    tray._factorForrajero = factorRecupForrajero;
    if (factorRecupForrajero < factorForrajero) {
      const trayAdj = calcTrayectoriaCC({ ...baseParams, factorRecupForrajero });
      tray.ccServ          = trayAdj.ccServ;
      tray.pr              = trayAdj.pr;
      tray.ccServAntic     = trayAdj.ccServAntic;
      tray.ccServHiper     = trayAdj.ccServHiper;
      tray.ccServTrad      = trayAdj.ccServTrad;
      tray.prAntic         = trayAdj.prAntic;
      tray.prHiper         = trayAdj.prHiper;
      tray.gananciaPrAntic = trayAdj.gananciaPrAntic;
      tray.gananciaPrHiper = trayAdj.gananciaPrHiper;
      tray.recDestete      = trayAdj.recDestete;
      tray.recDesteTardio  = trayAdj.recDesteTardio;
    }
  }
  ...
  Object.entries(demandaAlim).forEach(([alim, kgNec]) => {
    const item    = stockAlim.find(s => (s.alimento||"").toLowerCase().trim() === alim.toLowerCase().trim());
    ...
    // No alertar por stock vacío: el productor carga lo que va a comprar, no lo que ya tiene
  });
  ...
  // Tendencia CC desde tacto anterior (campo ccAnterior en FORM_DEF)
  const ccAnteriorVal = parseFloat(form.ccAnterior) || 0;
  if (ccAnteriorVal > 0 && ccPondVal > 0) {
    const tendCC = +(ccPondVal - ccAnteriorVal).toFixed(2);
    if (tendCC < -0.3) {
      addAlerta("cc_bajando", "P1",
        `CC bajó ${Math.abs(tendCC).toFixed(1)} unidades desde el tacto anterior (${ccAnteriorVal} → ${ccPondVal}). La pérdida es mayor a lo esperado. Revisar balance y carga.`,
        "Pérdida CC acelerada", "form.ccAnterior");
    } else if (tendCC > 0.2) {
      addAlerta("cc_mejorando", "P3",
        `CC mejoró +${tendCC.toFixed(1)} unidades desde el tacto anterior — las correcciones están funcionando.`,
        "Tendencia positiva", "form.ccAnterior");
    }
  }
  let ccDesvio = null;
  ...
  return {
    // Nivel 1
    cadena, disponMS, ndviN, tempHoy, p30, enso, factorForrajero,
    evalAgua, factorAgua, factorCarga, cargaEV_ha,
    ofertaMensual: ofertaAjustada,
    verdeoAporteMcalMes, verdeoMesInicio,
    nVacas, nToros, nV2s, nVaq2, nVaq1, totalEV,
    // Nivel 2
    tcSave, pvEntVaq1, vaq1E, pvSalidaVaq1, pvEntradaVaq2: vaq2E?.pvMayo2Inv, vaq2E,
    impactoCola,
    tray, dist, ccPondVal: ccPondVal || 0, ccPondValOk: ccPondVal > 0, baseParams,
    ofertaPorCateg,
    ...
    mesesDeficit, peorDeficitMcal, deficitAcumInv,
    trazas,
  };
}
...
const ndviBase = {
  ...
  "NOA":              0.44,
  "Pampa Húmeda":     0.58,
  "Paraguay Oriental":0.50,
  "Chaco Paraguayo":  0.42,
  "Brasil (Cerrado)": 0.55,
  "Bolivia (Llanos)": 0.46,
};

// NDVI histórico de referencia por zona y mes (media climatológica 2001-2020)
// Índices 0-11 = Ene-Dic · Fuente: MODIS MCD43A4 pastizales NEA/NOA/Pampa
const NDVI_MENSUAL = {
  "NEA":               [0.68,0.65,0.59,0.53,0.47,0.40,0.37,0.39,0.46,0.55,0.63,0.67],
  "NOA":               [0.54,0.52,0.47,0.41,0.33,0.27,0.25,0.28,0.36,0.45,0.51,0.54],
  "Pampa Húmeda":      [0.62,0.60,0.54,0.47,0.39,0.31,0.29,0.32,0.41,0.51,0.58,0.62],
  "Paraguay Oriental": [0.65,0.63,0.57,0.51,0.44,0.37,0.34,0.37,0.44,0.53,0.61,0.65],
  "Chaco Paraguayo":   [0.57,0.55,0.50,0.44,0.37,0.30,0.28,0.31,0.38,0.47,0.54,0.57],
  "Brasil (Cerrado)":  [0.61,0.59,0.54,0.47,0.39,0.32,0.30,0.33,0.41,0.51,0.58,0.62],
  "Bolivia (Llanos)":  [0.59,0.57,0.52,0.45,0.37,0.30,0.28,0.31,0.39,0.49,0.56,0.59],
};

async function fetchSat(lat, lon, zona, prov, enso, cb) {
  try {
    // past_days=92 cubre 3 meses (p30 + p90) + forecast_days=7 para pronóstico
    ...
    const p30  = Math.round(precPas.slice(-30).reduce((a,b)=>a+(b||0),0));
    const p90  = Math.round(precPas.reduce((a,b)=>a+(b||0),0));
    ...
    const mesHoyFetch = new Date().getMonth();
    const ndviRef     = NDVI_MENSUAL[zona] || NDVI_MENSUAL["NEA"];
    // Factor estacional: ajusta el NDVI estimado según el mes del año
    // (C4 dormido en invierno → NDVI bajo aunque llueva; activo en verano → NDVI alto)
    const ndviHistProm = ndviRef.reduce((a,b) => a+b, 0) / 12;
    const factEstac    = ndviRef[mesHoyFetch] / ndviHistProm;
    const ndviAdj = Math.min(0.95, Math.max(0.15, nb * factEstac * (0.6 + p30/300) * m)).toFixed(2);
    const ndviHist    = +ndviRef[mesHoyFetch].toFixed(2);
    const ndviDelta   = +(parseFloat(ndviAdj) - ndviHist).toFixed(2);
    const cond        = ndviAdj > 0.60 ? "Excelente" : ndviAdj > 0.45 ? "Buena" : ndviAdj > 0.30 ? "Regular" : "Crítica";
    const ndviCateg   = ndviDelta >  0.07 ? "Sobre lo normal"
                      : ndviDelta > -0.05 ? "Normal"
                      : ndviDelta > -0.10 ? "Bajo lo normal"
                      :                      "Muy por debajo";
    ...
    cb({
      ...
      p7, p30, p90, deficit: Math.round(p30 - et07*4.3), ndvi: ndviAdj, condForr: cond, et07,
      ndviHist, ndviDelta, ndviCateg,
      // Pronóstico
...
const fmtFecha = (d) => {
  ...
  const fecha = d instanceof Date ? d : new Date(d + "T12:00:00");
  if (isNaN(fecha)) return "—";
  return fecha.toLocaleDateString("es-AR", { day:"2-digit", month:"short", year:"2-digit" });
};
const smf = (v, bajo, alto) => v >= alto ? "#7ec850" : v >= bajo ? "#e8a030" : "#e05530";

// ═══════════════════════════════════════════════════════════════════
// BALANCE POR CATEGORIA — consumo real + movilizacion CC
// Base: Rosello et al. 2025 (INTA EEA Colonia Benitez, ISSN 3008-8631)
//       Detmann et al. 2010 (boost proteico)
//       NRC 2000 (requerimientos y consumo voluntario)
//       Minson 1990 (calidad y consumo)
//       Houghton 1990 (movilizacion CC)
// ═══════════════════════════════════════════════════════════════════

// Digestibilidad por vegetacion + fenologia (% MS)
// Fuente: Rosello et al. 2025, Tabla 3; Peruchena 2003
const DIG_POR_VEG_FEN = {
  "Pastizal natural":       { menor_10: 60, "10_25": 56, "25_50": 52, mayor_50: 48 },
  "Megatermicas C4":        { menor_10: 65, "10_25": 60, "25_50": 55, mayor_50: 45 },
  "Megatérmicas C4":        { menor_10: 65, "10_25": 60, "25_50": 55, mayor_50: 45 },
  "Pasturas templadas C3":  { menor_10: 65, "10_25": 65, "25_50": 65, mayor_50: 65 },
  "Mixta gramineas+leguminosas": { menor_10: 62, "10_25": 58, "25_50": 55, mayor_50: 50 },
  "Mixta gramíneas+leguminosas": { menor_10: 62, "10_25": 58, "25_50": 55, mayor_50: 50 },
  "Verdeo de invierno":     { menor_10: 68, "10_25": 65, "25_50": 60, mayor_50: 55 },
  "Bosque nativo / monte":  { menor_10: 40, "10_25": 40, "25_50": 40, mayor_50: 40 },
};

const MCAL_POR_PUNTO_CC = 150; // Houghton et al. 1990; NRC 2000 Tabla 4.3

function factorAccesibilidad(dispMS_ha) {
  const d = parseFloat(dispMS_ha) || 0;
  if (d >= 3000) return 1.00;
  if (d >= 1500) return 0.90;
  if (d >= 800)  return 0.75;
  if (d >= 400)  return 0.55;
  return 0.35; // pastizal muy degradado
}

function factorSequia(lluvia30) {
  const p = parseFloat(lluvia30) || 60;
  if (p >= 80) return 1.00;
  if (p >= 40) return 0.95;
  if (p >= 20) return 0.85;
  return 0.75;
...
function calcMovilizacionCC(tray, _mes, pvAdulta, estadoFisiolMes) {
  if (!tray || !tray.ccParto || !tray.ccMinLact) return 0;
  if (pvAdulta <= 0) return 0;
  if (!estadoFisiolMes || !estadoFisiolMes.includes("Lactaci")) return 0;
  const ccParto = parseFloat(tray.ccParto);
  const ccMin   = parseFloat(tray.ccMinLact);
  const mesesLact = parseFloat(tray.mesesLact) || 6;
  if (mesesLact <= 0) return 0;
  const perdidaCC = Math.max(0, ccParto - ccMin);
  const mcalTotales = perdidaCC * MCAL_POR_PUNTO_CC * (pvAdulta / 400);
  const mcalDia = mcalTotales / (mesesLact * 30);
  return +mcalDia.toFixed(1);
}

// ─── Fenologia esperada segun ciclo estacional de la zona ───
// Cita: Peruchena 2003; Rosello et al. 2025 p.5 (ciclo pastizal natural NEA)
// NOA y Pampa Humeda: ciclo similar — calibrar por zona en ronda futura
function fenologiaPorMes(mes, zona) {
  const ciclo = {
    8:"menor_10", 9:"menor_10",                          // Sep-Oct: rebrote
    10:"10_25",  11:"10_25",  0:"10_25",  1:"10_25",    // Nov-Feb: crecimiento
    2:"25_50",   3:"25_50",   4:"25_50",                 // Mar-May: maduracion
    5:"mayor_50", 6:"mayor_50", 7:"mayor_50",            // Jun-Ago: encanado
  };
  return ciclo[mes] || "25_50";
}

// ─── Balance por categoria — la funcion maestra ───
/**
 * Balance energético mensual para una categoría animal específica.
 * @param {import('./types').Motor} motor - Resultado de correrMotor
 * @param {import('./types').Form} form
 * @param {import('./types').Sat | null} sat
 * @param {"general" | "vacas_v2s" | "vaq1" | "vaq2"} cat - Categoría a calcular
 * @returns {import('./types').BalancePorCategoria}
 */
function balancePorCategoria(motor, form, sat, cat) {
  const MESES = 12;
  const res = [];

  const veg       = form.vegetacion || "Pastizal natural";
  const ndviZona  = NDVI_ESTAC[form.zona || "NEA"] || NDVI_ESTAC["NEA"];
  const hasPot    = (form.potreros || []).length > 0 && form.usaPotreros;

  // Datos por categoria
  const categorias = {
    "general":    { n: (parseFloat(form.vacasN)||0) + (parseFloat(form.v2sN)||0) + (parseFloat(form.vaq1N)||0) + (parseFloat(form.vaq2N)||0) + (parseFloat(form.torosN)||0),
                    pv: parseFloat(form.pvVacaAdulta)||320,
                    consPot: 2.8, label: "General (todo el rodeo)" },
    "vacas_v2s":  { n: (parseFloat(form.vacasN)||0) + (parseFloat(form.v2sN)||0),
                    pv: parseFloat(form.pvVacaAdulta)||320,
                    consPot: 2.6, label: "Vacas + V2S" },
    "vaq1":       { n: Math.round((parseFloat(form.vacasN)||0) * (parseFloat(form.pctReposicion)||20) / 100),
                    pv: 180, consPot: 2.4, label: "Vaquillona 1° invierno" },
    "vaq2":       { n: parseFloat(form.vaq2N)||0,
                    pv: 250, consPot: 2.8, label: "Vaquillona 2° invierno" },
  };

  const C = categorias[cat] || categorias["general"];
  const mesHoy = new Date().getMonth();

  const nVaqR = Math.round((parseFloat(form.vacasN)||0)*(parseFloat(form.pctReposicion)||20)/100);
  const mesesSupl = (form.suplMeses || ["5","6","7"]).map(Number);

  // ── Datos para F1 y F4 (ajuste oferta pasto con datos reportados) ──
  // F1: accesibilidad — biomasa reportada (altPasto + tipoPasto del form → kg MS/ha)
  // modulada mensualmente por estacionalidad C4 NEA
  const dispMS_base = motor?.disponMS?.msHa ?? 1500;
  const FACT_FENOL_DISP = { menor_10: 0.80, "10_25": 1.00, "25_50": 0.90, mayor_50: 0.50 };
  // F4: sequía — lluvia histórica mensual de la provincia reportada (getClima)
  const histClima = getClima(form.provincia || "Corrientes");

  for (let i = 0; i < MESES; i++) {
    // ── Oferta de pasto: motor precomputado (NDVI, temperatura, agua, fenología) ──
    // + corrección F1 (accesibilidad por biomasa reportada) y F4 (sequía histórica mensual)
    // F2 (digestibilidad) y F3 (PB) ya están en el motor vía mcalKg y cvPct — no se repiten
    const mBal = motor?.balanceMensual?.[i];
    const ofPastoBase =
        cat === "vaq1"        ? (mBal?.ofPastoPerVaq1       ?? 0)
      : cat === "vaq2"        ? (mBal?.ofPastoPerVaq2       ?? 0)
      : cat === "vacas_v2s"   ? (mBal?.ofPastoPerVacasV2s   ?? 0)
      :                         (mBal?.ofPastoVaca           ?? 0);

    // F1: biomasa disponible este mes = base reportada × factor estacional fenología
    const fenolMes   = mBal?.fenol || "25_50";
    const dispMS_mes = dispMS_base * (FACT_FENOL_DISP[fenolMes] ?? 0.80);
    const F1         = factorAccesibilidad(dispMS_mes);
    // F4: lluvia histórica mensual → estrés hídrico sobre el pasto
    const p30Hist    = histClima[i]?.p ?? 60;
    const F4         = factorSequia(p30Hist);

    const ofPastoDia = +(ofPastoBase * F1 * F4).toFixed(1);

    // Calidad del pasto este mes (del motor)
    const pbMes = mBal?.pbPas ?? 8;

    // ── Suplementación por categoría ──
    const supleActivo = mesesSupl.includes(i);
    let mcalSuplDia;
    // Cada categoría tiene 2 slots: principal (supl_/dosis_) y complemento (supl2_/dosis2_)
    const mcalSuplPorAnimal = (pref) => {
      const a1 = SUPLEMENTOS[form["supl_"+pref]] || null;
      const d1 = parseFloat(form["dosis_"+pref]) || 0;
      const a2 = SUPLEMENTOS[form["supl2_"+pref]] || null;
      const d2 = parseFloat(form["dosis2_"+pref]) || 0;
      return (a1?.em||0)*d1 + (a2?.em||0)*d2;
    };
    if (cat === "vaq1") {
      mcalSuplDia = supleActivo ? mcalSuplPorAnimal("vaq1") : 0;
    } else if (cat === "vaq2") {
      mcalSuplDia = supleActivo ? mcalSuplPorAnimal("vaq2") : 0;
    } else if (cat === "vacas_v2s") {
      // Vacas y V2S pueden tener suplementos distintos — promedio ponderado por animal
      const nAll = C.n || 1;
      const nVacasLoc = parseFloat(form.vacasN) || 0;
      const nV2sLoc   = parseFloat(form.v2sN) || 0;
      const totMcal = mcalSuplPorAnimal("vacas")*nVacasLoc + mcalSuplPorAnimal("v2s")*nV2sLoc;
      mcalSuplDia = supleActivo ? totMcal / Math.max(1, nAll) : 0;
    } else {
      // general — suma todos los suplementos (principal+complemento) / total animales
      const nAll = C.n || 1;
      const suplCatsG = [
        {pref:"vacas",   n: parseFloat(form.vacasN)||0},
        {pref:"v2s",     n: parseFloat(form.v2sN)||0},
        {pref:"toros",   n: parseFloat(form.torosN)||0},
        {pref:"vaq2",    n: parseFloat(form.vaq2N)||0},
        {pref:"vaq1",    n: nVaqR},
      ];
      let totMcal = 0;
      suplCatsG.forEach(c => {
        if (c.n) totMcal += mcalSuplPorAnimal(c.pref)*c.n;
      });
      mcalSuplDia = supleActivo ? totMcal / Math.max(1, nAll) : 0;
    }

    // ── CC movilizada como oferta (solo vacas_v2s y general) ──
    // Solo las vacas que siguen lactando movilizan CC — escalar por fracLact
    let mcalCC = 0;
    if (cat === "vacas_v2s" || cat === "general") {
      const fracLactMes = mBal?.fracLact ?? 1.0;
      const ccBase = calcMovilizacionCC(motor?.tray, i, C.pv, "Lactación con ternero al pie");
      mcalCC = +(ccBase * fracLactMes).toFixed(1);
    }

    // ── Verdeo (si aplica a esta categoría) ──
    let mcalVerdeo = 0;
    const potreroV = (form.potreros || []).find(p => (p.veg || "").toLowerCase().includes("verdeo"));
    const hayVerdeo = potreroV || (form.tieneVerdeo === "si" && parseFloat(form.verdeoHa) > 0);
    if (hayVerdeo && motor?.verdeoAporteMcalMes !== undefined) {
      const destino = potreroV?.verdeoDestino || form.verdeoDestinoVaq || "todo";
      const aplica  = destino === "todo" ||
                      (destino === "vaq1" || destino === "si") && cat === "vaq1" ||
                      destino === "vaq2" && cat === "vaq2" ||
                      destino === "v2s"  && cat === "vacas_v2s";
      if (aplica) {
        const aporte = motor.verdeoAporteMcalMes;
        const desde  = motor.verdeoMesInicio ?? 7;
        mcalVerdeo = (i >= desde && i <= desde+2) ? (typeof aporte === "number" ? aporte : 0) : 0;
        // Para verdeo por potrero: escalar al per-animal de la categoría
        if (mcalVerdeo > 0 && cat !== "general") {
          const nCat = cat === "vaq1" ? nVaqR
                     : cat === "vaq2" ? (parseFloat(form.vaq2N)||0)
                     : C.n;
          mcalVerdeo = nCat > 0 ? mcalVerdeo / Math.max(1, nCat) : 0;
        } else if (mcalVerdeo > 0 && cat === "general") {
          mcalVerdeo = mcalVerdeo / Math.max(1, C.n);
        }
      }
    }

    const ofertaTotal = ofPastoDia + mcalSuplDia + mcalVerdeo + mcalCC;

    // ── Demanda por categoría ──
    // Para vacas/V2S: usar reqVacaI del motor (ya ponderado por fracLact y destete)
    // Para vaquillonas: reqEM("vaq1inv"/"vaq2inv") está calibrado contra el GDP objetivo
    // de invierno (gdpNecesario/GDP_MIN_VAQ2). Escalar por el GDP real de este mes
    // (gdpVaq1Mes/gdpVaq2Mes, ya variable por pastura/NDVI/suplemento) para que la
    // demanda refleje el estado fisiológico real mes a mes en lugar de quedar fija.
    let demandaTotal;
    if (cat === "vaq1") {
      const reqBase   = reqEM(C.pv, "vaq1inv", form.biotipo) ?? C.pv * 0.077 * 1.3;
      const gdpMes    = mBal?.gdpVaq1Mes ?? GDP_MIN_INV_VAQ1_REF;
      const factGdp   = Math.min(1.8, Math.max(0.5, gdpMes / GDP_MIN_INV_VAQ1_REF));
      demandaTotal = reqBase * factGdp;
    } else if (cat === "vaq2") {
      const reqBase   = reqEM(C.pv, "vaq2inv", form.biotipo) ?? C.pv * 0.077 * 1.2;
      const gdpMes    = mBal?.gdpVaq2Mes ?? GDP_MIN_VAQ2;
      const factGdp   = Math.min(1.8, Math.max(0.5, gdpMes / GDP_MIN_VAQ2));
      demandaTotal = reqBase * factGdp;
    } else {
      // vacas_v2s y general: reqVacaI ya incorpora fracLact (ponderado por grupo de destete)
      demandaTotal = mBal?.reqVacaI ?? reqEM(C.pv, mBal?.estadoVaca || "Gestación media (5–7 meses)", form.biotipo) ?? C.pv * 0.077 * 1.2;
    }

    const balance = ofertaTotal - demandaTotal;

    let gdp = null;
    if (cat === "vaq1") {
      gdp = mBal?.gdpVaq1Mes ?? null;
    } else if (cat === "vaq2") {
      gdp = mBal?.gdpVaq2Mes ?? null;
    }

    res.push({
      mes: i,
      oferta: {
        pasto:          +ofPastoDia.toFixed(1),
        suplemento:     +mcalSuplDia.toFixed(1),
        verdeo:         +mcalVerdeo.toFixed(1),
        movilizacionCC: +mcalCC.toFixed(1),
        total:          +ofertaTotal.toFixed(1),
      },
      demanda: +demandaTotal.toFixed(1),
      balance: +balance.toFixed(1),
      gdp,
      estadoVaca: (cat === "vacas_v2s" || cat === "general") ? (mBal?.estadoVaca || null) : null,
      pbPas: pbMes,
      p30: sat?.p30 || null,
    });
  }
...
export {
  balancePorCategoria,
  getBiotipo, calcOfertaMensualArray, calcOfPasto, reqEM, evaluarSincronia,
  ...
  ccACategSuperv, calcSupervivencia, calcCalidadPrenez, calcCadena, calcConsumoAgua,
  evaluarAgua, evaluarSanidad, validarManual, diasHelada,
  calcDisp, dZona, dProv, correrMotor, calcFactorForrajero,
  ...
  calcScore, diagnosticarSistema, calcImpactoCola, calcFaseCiclo,
  fetchSat, fmtFecha, smf, getClima, FENOLOGIAS, ccAPrenez,
};
