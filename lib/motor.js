...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  for (let i = 0; i < MESES; i++) {
    // ── Oferta de pasto: usa el motor precomputado con todos los potreros ──
    // El motor ya calculó correctamente con potreros, ha, veg, fenología, temperatura
    ...
    const ofPastoDia =
      ...
      :                         (mBal?.ofPastoVaca           ?? 0); // general: per-vaca
    ...
    if (cat === "vacas_v2s" || cat === "general") {
      const estadoMes = mBal?.estadoVaca || "";
      mcalCC = calcMovilizacionCC(motor?.tray, i, C.pv, estadoMes);
    }
