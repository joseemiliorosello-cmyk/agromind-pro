...
import { BIOTIPOS, getBiotipo, SUPLEMENTOS, PROD_BASE,
         CALIDAD_C4_CALIBRADA, MESES_NOM, FORM_DEF, getClima } from "./constantes";
...
// Promedio meses invernales May-Ago (índices 4-7): base para calcVaq1/calcVaq2
const BASE_GDP_INV = Math.round([4,5,6,7].reduce((s,i)=>s+BASE_GDP_NEA[i],0)/4); // ≈ 54 g/d
...
function calcVaq1(pvEntrada, pvAdulta, ndvi, edadMayo, tipoDestete, disponMS, fenologia, suplReal, dosisRealKg) {
  ...
  const factNivelV1 = nivelMS === "baja" ? 0.10 : nivelMS === "alta" ? 1.40 : 1.00;
  const gdpPasto    = Math.max(0, Math.round(BASE_GDP_INV * factNivelV1 * factNdvV1));
...
function calcVaq2(pvEntradaVaq1Sal, pvAdulta, ndvi, disponMS, fenologia, suplReal, dosisRealKg) {
  ...
  const factNivV2   = nivelMS === "baja" ? 0.10 : nivelMS === "alta" ? 1.40 : 1.00;
  const gdpPastoInv = Math.max(0, Math.round(BASE_GDP_INV * factNivV2 * factNdvV2));
...
function calcCostoSupl(kgDia, nAnimales, dias, supl) {
  const precio   = supl?.precioRefUsd || 0;
  const kgTotal  = Math.round(kgDia * nAnimales * dias);
  const costoUsd = Math.round(kgTotal * precio);
  return {
    kgTotal,
    costoUsd,
    costoUsdAnimal: +((kgDia * dias * precio).toFixed(1)),
    precioRefUsd: precio,
    label: `${(kgTotal / 1000).toFixed(1)}t × $${precio}/kg ref. ≈ $${costoUsd} USD`,
  };
}
...
function diagnosticarSistema(motor, form) {
  ...
  const PRECIO_TERNERO = parseFloat(form.precioTerneroUsd) || 320; // USD/ternero al destete
    ...
    planes.push({
      ...
      pasos: [
        ...
        },
        {
          ...
          costo: calcCostoSupl(kgPrep, nVacas, 60, suplPre),
        },
      ],
      proyeccion: {
      ...
      costoResumen: (() => {
        const prenezConPlan = Math.min(85, prenezAct + Math.round((prenezObj - prenezAct) * 0.65));
        const ternerosMas   = Math.round((prenezConPlan - prenezAct) / 100 * (nVacas || 0));
        const costoUsd      = calcCostoSupl(kgPrep, nVacas, 60, suplPre).costoUsd;
        const valorTerneros = ternerosMas * PRECIO_TERNERO;
        return {
          costoTotalUsd: costoUsd,
          ternerosMas,
          valorTernerosMasUsd: valorTerneros,
          costoPorTerneroExtraUsd: ternerosMas > 0 ? Math.round(costoUsd / ternerosMas) : null,
          roi: costoUsd > 0 ? +(valorTerneros / costoUsd).toFixed(1) : null,
          rentable: valorTerneros > costoUsd,
          label: ternerosMas > 0
            ? `+${ternerosMas} terneros · costo/ternero extra ≈ $${Math.round(costoUsd / ternerosMas)} USD · ROI ${+(valorTerneros / costoUsd).toFixed(1)}x`
            : `Costo campaña ≈ $${costoUsd} USD (mejora de CC, no preñez directa)`,
        };
      })(),
    });
  }
  ...
  if (nVaq1 > 0 && vaq1E && !vaq1E.mensaje) {
    ...
    if (deficitPV > 0 || esCalidadBaja || esCantidadBaja) {
      ...
      pasosVaq1.push({
        ...
        diasTotal: 90,
        costo: calcCostoSupl(kgProt, nVaq1, 90, suplProt),
      });
      ...
      if (esCantidadBaja && kgEnerg > 0) {
        pasosVaq1.push({
          ...
          momento: "Mayo–Agosto",
          diasTotal: 90,
          costo: calcCostoSupl(kgEnerg, nVaq1, 90, suplEnerg),
        });
      }

      const costoVaq1Usd = pasosVaq1.reduce((s, p) => s + (p.costo?.costoUsd || 0), 0);
      planes.push({
        id: "plan_vaq1",
        categoria: "Vaquillona 1° invierno",
        icono: "🐮",
        ...
        costoResumen: {
          costoTotalUsd: costoVaq1Usd,
          nota: "Inversión en crecimiento — impacto en preñez del año siguiente",
          label: `$${costoVaq1Usd} USD campaña invierno · ${(nVaq1 || 0)} cab. · $${nVaq1 > 0 ? Math.round(costoVaq1Usd / nVaq1) : 0}/cab`,
        },
      });
    }
  }
    ...
    const pasosVaq2   = [
      {
        ...
        diasTotal: 110,
        costo: calcCostoSupl(kgProt2, nVaq2, 110, suplProt),
      },
      ...(kgEnerg2 > 0 ? [{
        orden: 2,
        tipo: "suplemento",
        ...
        momento: "Mayo–entore",
        diasTotal: 110,
        costo: calcCostoSupl(kgEnerg2, nVaq2, 110, suplEnerg2),
      }] : []),
    ];
    const costoVaq2Usd = pasosVaq2.reduce((s, p) => s + (p.costo?.costoUsd || 0), 0);
    planes.push({
      id: "plan_vaq2",
      categoria: "Vaquillona 2° invierno",
      icono: "🐂",
      ...
      costoResumen: {
        costoTotalUsd: costoVaq2Usd,
        nota: "Inversión en entore — la vaq2 que llega con peso correcto preña a la primera",
        label: `$${costoVaq2Usd} USD campaña · ${(nVaq2 || 0)} cab. · $${nVaq2 > 0 ? Math.round(costoVaq2Usd / nVaq2) : 0}/cab`,
      },
    });
  }
  ...
  if (ind.ccToros && ind.ccToros < 5.0) {
    ...
    const costoTorosPaso = calcCostoSupl(kgToro, nToros, dias, suplToro);
    const ppMejora       = Math.min(12, Math.round((5.5 - ind.ccToros) / 0.12 * 10));
    const ternTorosMas   = Math.round(ppMejora / 100 * (nVacas || 0));
    const valorTernToros = ternTorosMas * PRECIO_TERNERO;
    planes.push({
      id: "plan_toros",
      categoria: "Toros",
      icono: "🐂",
      ...
      costoResumen: {
        costoTotalUsd: costoTorosPaso.costoUsd,
        ternerosMas: ternTorosMas,
        valorTernerosMasUsd: valorTernToros,
        costoPorTerneroExtraUsd: ternTorosMas > 0 ? Math.round(costoTorosPaso.costoUsd / ternTorosMas) : null,
        roi: costoTorosPaso.costoUsd > 0 ? +(valorTernToros / costoTorosPaso.costoUsd).toFixed(1) : null,
        rentable: valorTernToros > costoTorosPaso.costoUsd,
        label: ternTorosMas > 0
          ? `+${ternTorosMas} terneros · costo/ternero extra ≈ $${Math.round(costoTorosPaso.costoUsd / ternTorosMas)} USD · ROI ${+(valorTernToros / costoTorosPaso.costoUsd).toFixed(1)}x`
          : `Costo preparo servicio ≈ $${costoTorosPaso.costoUsd} USD`,
      },
    });
  }
  ...
  // ── Resumen económico total de la campaña ─────────────────────
  const costoTotalCampanaUsd = planes.reduce((s, p) => s + (p.costoResumen?.costoTotalUsd || 0), 0);
  const ternerosTotalesMas   = planes.reduce((s, p) => s + (p.costoResumen?.ternerosMas   || 0), 0);
  const valorTernerosTotales = ternerosTotalesMas * PRECIO_TERNERO;
  ...
  return {
    ...
    costoTotalCampana: {
      costoUsd:              costoTotalCampanaUsd,
      ternerosMas:           ternerosTotalesMas,
      valorTernerosMasUsd:   valorTernerosTotales,
      roi:                   costoTotalCampanaUsd > 0 ? +(valorTernerosTotales / costoTotalCampanaUsd).toFixed(1) : null,
      rentable:              valorTernerosTotales > costoTotalCampanaUsd,
      precioTerneroUsado:    PRECIO_TERNERO,
      label: costoTotalCampanaUsd > 0
        ? `Campaña completa ≈ $${costoTotalCampanaUsd} USD · +${ternerosTotalesMas} terneros · ROI ${costoTotalCampanaUsd > 0 ? +(valorTernerosTotales / costoTotalCampanaUsd).toFixed(1) : "—"}x`
        : "Sin intervenciones de suplementación requeridas",
    },
    ...
    recs: planes.flatMap(p => p.pasos.map(paso => ({
      ...
      solucion: { tipo:"supl", alimento: paso.alimento, kgDia: paso.kgDia, costo: paso.costo },
      impactoBase: p.proyeccion?.base,
      impactoConRec: p.proyeccion?.conPlan,
      costoResumen: p.costoResumen,
    }))),
  };
}
...
function correrMotor(form, sat, potreros, usaPotreros) {
  ...
  if (usaPotreros && potreros && potreros.length > 0) {
    ...
    CATS.forEach(cat => {
      ...
      // Calidad estimada del potrero (PB) según tipo de vegetación y fenología
      const potPrincipal = potsCat[0];
      const fenolCat = potPrincipal.fenol || "25_50";
      const vegCat   = potPrincipal.veg   || "Pastizal natural";
      const pbCat    = { menor_10:12, "10_25":9, "25_50":6, mayor_50:4 }[fenolCat] || 6;
      const esVerdeo = vegCat.toLowerCase().includes("verdeo");
      const pbEfectivo = esVerdeo ? 18 : pbCat; // verdeo tiene PB alto
  ...
  const balanceMensual = MESES_NOM.map((mes, i) => {
    ...
    const enPrepartO = !enLact && mesesDesdeP >= 8;       // 2 meses antes del próximo parto
...
async function fetchSat(lat, lon, zona, prov, enso, cb) {
  try {
    ...
    // NDVI histórico de referencia para el mes actual
...
// Umbral de PB en el pasto bajo el cual la suplementacion proteica es critica
// Cita: Detmann et al. 2010 (R. Bras. Zootec. 39 supl.:195-212)
const UMBRAL_PB_CRITICO = 7.0;

// Dosis minima de PB aportada por suplemento para activar boost proteico
// (g PB / animal / dia) — Detmann 2010, NRC 2000
const PB_MIN_BOOST = 100;

// Mcal de energia neta por punto de CC en vaca adulta 400 kg
// Cita: Houghton et al. 1990; NRC 2000 Tabla 4.3
const MCAL_POR_PUNTO_CC = 150;

// CC minima fisiologica — por debajo no hay mas movilizacion posible
// Cita: Short et al. 1990, J. Anim. Sci. 68:799
const CC_MINIMA_MOVILIZACION = 2.5;

// ─── Factor 1: Accesibilidad por disponibilidad (kgMS/ha) ───
function factorAccesibilidad(dispMS_ha) {
  ...
  return 0.35; // Rosello et al. 2025: pastizal muy degradado
}

// ─── Factor 2: Calidad (digestibilidad) ───
function factorCalidad(digPct) {
  const d = parseFloat(digPct) || 55;
  if (d >= 65) return 1.00;
  if (d >= 60) return 0.90;
  if (d >= 55) return 0.75;
  if (d >= 50) return 0.60; // invierno NEA tipico
  return 0.45;              // paja, rastrojo
}

// ─── Factor 3: PB del pasto — con boost si hay suplemento proteico ───
function factorPB(pbPasto, aportePBSupl_gDia) {
  const pb = parseFloat(pbPasto) || 7;
  const aporteSupl = parseFloat(aportePBSupl_gDia) || 0;
  // Si PB pasto < 7% pero hay suplemento proteico suficiente, F3 vuelve a 1.00
  if (pb < UMBRAL_PB_CRITICO && aporteSupl >= PB_MIN_BOOST) {
    return 1.00; // boost proteico activado (Detmann 2010)
  }
...
// ─── Factor 4: Sequia (lluvia 30 dias) ───
...
// ─── Consumo real (kg MS / animal / dia) ───
// pvAnimal: peso vivo (kg)
// consPotencial_pctPV: 2.5-3.0 segun categoria
// Aplica los 4 factores y devuelve kg MS y Mcal
function calcConsumoReal(pvAnimal, consPotencial_pctPV, dispMS_ha, digPct, pbPasto, aportePBSupl, lluvia30) {
  const pv = parseFloat(pvAnimal) || 0;
  const consPot = parseFloat(consPotencial_pctPV) || 2.5;

  const F1 = factorAccesibilidad(dispMS_ha);
  const F2 = factorCalidad(digPct);
  const F3 = factorPB(pbPasto, aportePBSupl);
  const F4 = factorSequia(lluvia30);

  // Boost adicional a la calidad efectiva si F3 esta boosteado
  // (microflora activada mejora digestibilidad aparente)
  const F2_efectivo = (pbPasto < UMBRAL_PB_CRITICO && aportePBSupl >= PB_MIN_BOOST)
    ? Math.min(1.00, F2 + 0.10)
    : F2;

  const consRealPctPV = consPot * F1 * F2_efectivo * F3 * F4;
  const kgMS = pv * consRealPctPV / 100;

  // EM del pasto segun digestibilidad (4.4 Mcal/kg MS × digestibilidad × 0.82)
  // Cita: NRC 2000; ARC 1980
  const EM_pasto = 4.4 * (digPct / 100) * 0.82;
  const mcalDia = kgMS * EM_pasto;

  return {
    kgMS: +kgMS.toFixed(2),
    mcalDia: +mcalDia.toFixed(2),
    pctPV: +consRealPctPV.toFixed(2),
    factores: { F1, F2: F2_efectivo, F3, F4 },
    boostActivo: (pbPasto < UMBRAL_PB_CRITICO && aportePBSupl >= PB_MIN_BOOST),
    EM_pasto: +EM_pasto.toFixed(2),
  };
}

// ─── Movilizacion de CC — aporte endogeno Mcal/dia ───
// Solo vacas adultas durante lactacion y solo si CC > 2.5
// Distribuye la perdida de CC linealmente en los meses de lactacion
// Cita: Houghton et al. 1990; NRC 2000 Tabla 4.3
function calcMovilizacionCC(tray, mes, pvAdulta, estadoFisiolMes) {
  if (!tray || !tray.ccParto || !tray.ccMinLact) return 0;
  if (pvAdulta <= 0) return 0;

  // Solo aporta durante lactacion (no preparto, no seca)
  ...
  if (ccMin < CC_MINIMA_MOVILIZACION) return 0; // ya toco piso fisiologico
...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  for (let i = 0; i < MESES; i++) {
    ...
    // Para vaquillonas: reqEM con su estado fijo de recría
    let demandaTotal;
    if (cat === "vaq1") {
      demandaTotal = reqEM(C.pv, "vaq1inv", form.biotipo) ?? C.pv * 0.077 * 1.3;
    } else if (cat === "vaq2") {
      demandaTotal = reqEM(C.pv, "vaq2inv", form.biotipo) ?? C.pv * 0.077 * 1.2;
    } else {
...
export {
  balancePorCategoria, calcConsumoReal, calcMovilizacionCC, factorAccesibilidad, factorCalidad, factorPB, factorSequia,
getBiotipo,
