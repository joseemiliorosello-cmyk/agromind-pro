...
function correrMotor(form, sat, potreros, usaPotreros) {
  ...
  const hayPotreroVerdeo = usaPotreros && Array.isArray(potreros) &&
    potreros.some(p => (p.veg || "").toLowerCase().includes("verdeo"));
...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  for (let i = 0; i < MESES; i++) {
    ...
    let mcalVerdeo = 0;
    if (form.tieneVerdeo && form.verdeoHa > 0) {
      const destino = form.verdeoDestinoVaq || "todo";
      const aplica = (destino === "todo") ||
                     (destino === "vaq1" && cat === "vaq1") ||
                     (destino === "vaq2" && cat === "vaq2") ||
                     (destino === "v2s" && cat === "vacas_v2s");
      if (aplica && (motor?.verdeoAporteMcalMes?.[i] !== undefined)) {
        mcalVerdeo = motor.verdeoAporteMcalMes[i];
      }
    }
