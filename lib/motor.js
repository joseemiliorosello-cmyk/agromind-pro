...
function correrMotor(form, sat, potreros, usaPotreros) {
  ...
  const balanceMensual = MESES_NOM.map((mes, i) => {
    ...
    const _tot = _pT + _pA + _pH || 100;
    const fracLactH = mesesDesdeP < 2 ? _pH / _tot : 0; // hiperprecoz 50d ≈ ceil(1.67) = 2 meses
    const fracLactA = mesesDesdeP < 3 ? _pA / _tot : 0; // anticipado 90d = 3 meses
    const fracLactT = mesesDesdeP < 6 ? _pT / _tot : 0; // tradicional 180d = 6 meses
