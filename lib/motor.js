...
function correrMotor(form, sat, potreros, usaPotreros) {
  ...
  const balanceMensual = MESES_NOM.map((mes, i) => {
    ...
    const factV1 = ofertaPorCateg?.vaq1?.factorCalidad ?? (pbPasI < 7 ? 0.55 : 1.0);
    const factV2 = ofertaPorCateg?.vaq2?.factorCalidad ?? (pbPasI < 7 ? 0.55 : 1.0);
...
function balancePorCategoria(motor, form, sat, cat) {
  ...
  for (let i = 0; i < MESES; i++) {
    const ndviMes = ndviZona[i] || 0.6;
    // Fenologia dinamica: ciclo estacional por zona; el mes actual usa lo observado en campo
    const fenolEsperado = fenologiaPorMes(i, form.zona || "NEA");
    const fenol = (i === mesHoy && form.fenologia) ? form.fenologia : fenolEsperado;

    // Digestibilidad: base fenologica con modulacion NDVI suave ±10%
    // Fenologia determina la base; NDVI solo ajusta segun verdor relativo — Peruchena 2003
    const digMap = DIG_POR_VEG_FEN[veg] || DIG_POR_VEG_FEN["Pastizal natural"];
    const ndviFactor = 0.90 + (ndviMes * 0.2); // ndvi 0.5→1.00, 0.8→1.06, 1.0→1.10
    const digMes = (digMap[fenol] || 52) * Math.min(1.10, Math.max(0.90, ndviFactor));
    const digFinal = Math.max(40, Math.min(70, digMes));

    // PB del pasto segun vegetacion y fenologia
    const pbBase = 10;
    const pbMes = pbBase * (fenol === "mayor_50" ? 0.4 : fenol === "25_50" ? 0.6 : fenol === "10_25" ? 0.8 : 1.0);

    // Disponibilidad MS
    const dispMS = (motor && motor.disponMS) || (2000 * ndviMes);

    // Solo contar suplemento si estamos en los meses seleccionados
    const mesesSupl = (form.suplMeses || ["5","6","7"]).map(Number);
    const supleActivo = mesesSupl.includes(i);

    // Suplementacion proteica de esta categoria
    const nVaqR = Math.round((parseFloat(form.vacasN)||0)*(parseFloat(form.pctReposicion)||20)/100);
    let mcalSuplDia, pbSuplDia;
    if (cat === "vaq1") {
      const a = SUPLEMENTOS[form.supl_vaq1] || null;
      const d = parseFloat(form.dosis_vaq1) || 0;
      mcalSuplDia = supleActivo ? (a?.em||0)*d : 0;
      pbSuplDia   = supleActivo ? (a?.pb||0)*d*10 : 0;
    } else if (cat === "vaq2") {
      ...
      // Vacas no reciben suplemento (su herramienta es el destete); V2S sí
      const nAll = C.n || 1;
      const av2s = SUPLEMENTOS[form.supl_v2s] || null;
      const dv2s = parseFloat(form.dosis_v2s) || 0;
      const nV2sLoc = parseFloat(form.v2sN) || 0;
      const totMcal = supleActivo ? (av2s?.em||0)*dv2s*nV2sLoc : 0;
      const totPb   = supleActivo ? (av2s?.pb||0)*dv2s*10*nV2sLoc : 0;
      mcalSuplDia = totMcal / Math.max(1, nAll);
      pbSuplDia   = totPb   / Math.max(1, nAll);
    } else {
      // general — suma todos los suplementos por categoría / total animales
      const nAll = C.n || 1;
      const suplCatsG = [
      ...
      let totMcal = 0, totPb = 0;
      suplCatsG.forEach(c => {
        const sup = SUPLEMENTOS[form[c.sk]];
        const dos = parseFloat(form[c.dk]) || 0;
        if (sup && dos && c.n) { totMcal += sup.em*dos*c.n; totPb += sup.pb*dos*10*c.n; }
      });
      mcalSuplDia = supleActivo ? totMcal / Math.max(1, nAll) : 0;
      pbSuplDia   = supleActivo ? totPb   / Math.max(1, nAll) : 0;
    }

    // Consumo real del pasto
    const cons = calcConsumoReal(C.pv, C.consPot, dispMS, digFinal, pbMes, pbSuplDia, sat?.p30 || 60);

    // Movilizacion CC (solo vacas_v2s y general)
    let mcalCC = 0;
    if (cat === "vacas_v2s" || cat === "general") {
      const estadoMes = motor?.balanceMensual?.[i]?.estado || "";
      mcalCC = calcMovilizacionCC(motor?.tray, i, C.pv, estadoMes);
    }

    // Verdeo (si aplica por destino a esta categoria)
    let mcalVerdeo = 0;
    const potreroV = (form.potreros || []).find(p => (p.veg || "").toLowerCase().includes("verdeo"));
    const hayVerdeo = potreroV
      ? true
      : (form.tieneVerdeo === "si" && parseFloat(form.verdeoHa) > 0);
    ...
    // Oferta total
    const ofertaTotal = cons.mcalDia + mcalSuplDia + mcalVerdeo + mcalCC;

    // Demanda — vaquillonas usan clave especifica de reqEM; vacas usan estado mensual del motor
    const estadoFisiol = cat === "vaq1" ? "vaq1inv"
      : cat === "vaq2" ? "vaq2inv"
      : (motor?.balanceMensual?.[i]?.estado || "Vaca seca sin ternero");
    const demandaUnit = (typeof reqEM === 'function')
      ? reqEM(C.pv, estadoFisiol, form.biotipo)
      : (C.pv * 0.077 * 1.2);
    const demandaTotal = demandaUnit; // por animal/dia

    const balance = ofertaTotal - demandaTotal;

    // Si es vaq1 o vaq2, calcular GDP
    let gdp = null;
    if (cat === "vaq1" || cat === "vaq2") {
      // GDP g/dia = balance / 5 Mcal per kg (NRC 2000)
      gdp = Math.round((balance / 5.0) * 1000);
    }

    res.push({
      mes: i,
      oferta: {
        pasto: +cons.mcalDia.toFixed(1),
      ...
      gdp: gdp,
      ndvi: +ndviMes.toFixed(2),
      p30: sat?.p30 || null,
      consRealPctPV: cons.pctPV,
      boostProteico: cons.boostActivo,
      factoresConsumo: cons.factores,
    });
  }
